...
SSOK_DoF1:
    ...
    ; Win+F1 Ã¢ ÀÚÃ¼°¡ È°¼ºÈ­µÈ »óÅÂ¿¡¼­ ´Ù½Ã ´­·¯µµ
    ; ¿ø·¡ ÀÛ¾÷ Ã¢À» ÀÒÁö ¾Êµµ·Ï ´ë»ó HWND¸¦ º¸È£ÇÕ´Ï´Ù.
    QICurrentTargetHwnd := WinExist("A")
    if (QICurrentTargetHwnd != "" && QICurrentTargetHwnd != QIQuickHwnd)
        QILastTargetHwnd := QICurrentTargetHwnd
...
; =========================================================
QI_ShowGui:
    ; Win+F1 ¿¬¼Ó ÀÔ·Â/Å¸ÀÌ¸Ó°¡ GUI »ý¼º Áß ³¢¾îµé¸é °°Àº vº¯¼ö ÄÁÆ®·ÑÀÌ
    ; Áßº¹ »ý¼ºµÉ ¼ö ÀÖÀ¸¹Ç·Î »ý¼º ±¸°£Àº ÇÑ ¹ø¸¸ ½ÇÇàµÇ°Ô º¸È£ÇÕ´Ï´Ù.
    if (QIQuickBuilding)
        return
    QIQuickBuilding := 1
    Critical, On
    ...
    Gui, QIQuick:Destroy
    ; Destroy ÈÄ New·Î ¿ÏÀüÈ÷ »õ GUI¸¦ ¸¸µé¾î ÀÌÀü ÄÁÆ®·Ñ ¿¬°áÀ» ³²±âÁö ¾Ê½À´Ï´Ù.
    Gui, QIQuick:New, +AlwaysOnTop +ToolWindow +HwndQIQuickHwnd
    ...
    ; 10¹ø - »ó´ÜÀº ³¯Â¥ ±â´É, ÇÏ´ÜÀº ÇÐ±³ÁÖ¼Ò/±Ý¾× ±â´É ¼ø¼­·Î ¹èÄ¡
    Gui, QIQuick:Add, Text, x15 y540 w18 h24 c003366, 10.
    Gui, QIQuick:Font, s9 norm c000000, Malgun Gothic

    ; »ó´Ü: ¿À´ÃºÎÅÍ ³¯Â¥ °ü·Ã ±â´É ¼ø¼­
    ...
    ; ÇÏ´Ü: ÇÐ±³ÁÖ¼Ò ¡æ ¼ýÀÚ-±Ý¾× ¡æ ¿ø ¡æ Ãµ¿ø ¡æ ¸¸¿ø ¡æ ¹é¸¸¿ø ¡æ ¾ï¿ø
    ; ÇÐ±³ÁÖ¼Ò - ¸ÞÀÎ È­¸é¿¡ ÀÔ·ÂµÈ ±â°ü¸íÀ» ¹Ù·Î »ç¿ë
    ; ³ªÀÌ½º ÇÐ±³Á¤º¸ API¿¡¼­ ÁÖ¼Ò / ÀüÈ­ / ÆÑ½º / È¨ÆäÀÌÁö¸¦ Ã£¾Æ ÇöÀç ÀÛ¾÷Ã¢¿¡ ÀÔ·Â
    Gui, QIQuick:Add, Button, x35  y569 w100 h28 gQI_SchoolAddressInput, ÇÐ±³ÁÖ¼Ò
    Gui, QIQuick:Add, Button, x140 y569 w90  h28 gQI_MoneyTextToggle, ¼ýÀÚ-±Ý¾×
    Gui, QIQuick:Add, Button, x235 y569 w60  h28 gQI_MoneyUnitWon, ¿ø
    Gui, QIQuick:Add, Button, x300 y569 w70  h28 gQI_MoneyUnitThousand, Ãµ¿ø
    Gui, QIQuick:Add, Button, x375 y569 w70  h28 gQI_MoneyUnitTenThousand, ¸¸¿ø
    Gui, QIQuick:Add, Button, x450 y569 w85  h28 gQI_MoneyUnitMillion, ¹é¸¸¿ø
    Gui, QIQuick:Add, Button, x540 y569 w70  h28 gQI_MoneyUnitHundredMillion, ¾ï¿ø

    ; 11¹ø Æ¯¼ö¹®ÀÚ - ¡à ¡Û ¡¤ ¡Ø¸¦ ¸Ç ¾Õ¿¡ µÎ°í ³ª¸ÓÁö´Â ±âÁ¸ ¼ø¼­ À¯Áö
    Gui, QIQuick:Add, Text, x15 y635 w18 h24 c003366, 11.
    QISymbolMap := StrSplit("¡à|¡Û|¡¤|¡Ø|¡î|¢Ñ|¢¡|¢¹|¢º|¡Ü|¡Ý|¡á|¢Ã|¡¸|¡¹|¡¼|¡½|¡²|¡³|¡¶|¡·|¡º|¡»|¡Þ|¡ß|¡Ú|¡â|¡ã|¡ä|¡å|¡ç|¡è|¡é|¡ê|¢Ï|¥°|¥±|¥²|¥³|¥´|¨ç|¨è|¨é|¨ê|¨ë|¡É|§®|§¯|§°|§³|§¨|§±|§¸|¢ß|¡À|¡¿", "|")
...
QI_LoadOneUrlPreview(slot)
{
    ...
    ; ÀÌ¹Ì ¸¸µé¾î µÐ 12~15¹ø Picture ÄÁÆ®·ÑÀÇ ÀÌ¹ÌÁö¸¸ ±³Ã¼ÇÕ´Ï´Ù.
    ; »õ ÄÁÆ®·ÑÀ» °ãÃÄ ¸¸µé¸é ±âº» URL ÀÌ¹ÌÁö°¡ Ç¥½ÃµÇÁö ¾Ê´Â °æ¿ì°¡ ÀÖ½À´Ï´Ù.
    ctrlName := "QIImage" . slot . "Ctrl"
    GuiControl, QIQuick:, %ctrlName%, *w150 *h75 %cachePath%
}
...
; =========================================================
; 10¹ø ¼ýÀÚ-±Ý¾× Åä±Û
; - 1È¸: ±Ý12,232¿ø(±ÝÀÏ¸¸ÀÌÃµÀÌ¹é»ï½ÊÀÌ¿ø)
; - ´Ù½Ã ´©¸§: 12,232¿ø
; - ´Ù½Ã ´©¸§: ÇÑ±Û ±Ý¾×À» ´Ù½Ã ºÙÀÓ
; =========================================================
QI_MoneyTextToggle:
    QI_ToggleSelectedMoneyText()
return

QI_ToggleSelectedMoneyText()
{
    global QILastTargetHwnd, QIPinMode
    ...
    if (!SSOK_Tool_CopyClipboardText(selectedText, 0.4, 2))
    {
        Clipboard := oldClip
        ToolTip, º¯È¯ÇÒ ±Ý¾×À» ¸ÕÀú ºí·Ï ÁöÁ¤ÇØÁÖ¼¼¿ä.
        SetTimer, QI_RemoveToolTip, -1500
        QI_RestorePinnedGuiFocus()
        return false
    }

    selectedText := Trim(selectedText, " `t`r`n")
    if (selectedText = "")
    {
        Clipboard := oldClip
        ToolTip, º¯È¯ÇÒ ±Ý¾×À» ¸ÕÀú ºí·Ï ÁöÁ¤ÇØÁÖ¼¼¿ä.
    ...
    ; ÀÌ¹Ì ÇÑ±Û ±Ý¾×ÀÌ ºÙ¾î ÀÖÀ¸¸é ¼ýÀÚ ±Ý¾×¸¸ ³²±é´Ï´Ù.
    hasKoreanMoney := RegExMatch(selectedText, "\(\s*±Ý[^)]*¿ø\s*\)")

    if (RegExMatch(selectedText, "([0-9][0-9,]*)\s*¿ø", m))
    {
        rawNum := StrReplace(m1, ",", "")
        amountWon := rawNum + 0
        parseOK := true
    }
    else
        amountWon := QI_ParseMoneyTextToWon(selectedText, parseOK)

    if (!parseOK)
    {
        Clipboard := oldClip
        ToolTip, ¼±ÅÃÇÑ ±Ý¾×À» ÀÎ½ÄÇÏÁö ¸øÇß½À´Ï´Ù.
        SetTimer, QI_RemoveToolTip, -1700
        QI_RestorePinnedGuiFocus()
        return false
    }

    absWon := Abs(Round(amountWon))
    rawNumber := Format("{:.0f}", absWon)
    formattedNum := SSOK_CalcAddCommas(rawNumber)
    if (amountWon < 0)
        formattedNum := "-" . formattedNum

    if (hasKoreanMoney)
        convertedText := formattedNum . "¿ø"
    else
    {
        resultHan := QI_NumToKoreanMoney(rawNumber)
        if (amountWon < 0)
            resultHan := "¸¶ÀÌ³Ê½º" . resultHan
        convertedText := "±Ý" . formattedNum . "¿ø(±Ý" . resultHan . "¿ø)"
    }
    ...
    if (!SSOK_Tool_SetClipboardTextWithWait(convertedText, 0.7, 3))
    {
        Clipboard := oldClip
        QI_RestorePinnedGuiFocus()
        return false
...
QI_NumToKoreanMoney(num)
{
    digits := ["", "ÀÏ", "ÀÌ", "»ï", "»ç", "¿À", "À°", "Ä¥", "ÆÈ", "±¸"]
    smallUnits := ["", "½Ê", "¹é", "Ãµ"]
    largeUnits := ["", "¸¸", "¾ï", "Á¶", "°æ"]

    num := RegExReplace(num, "[^0-9]", "")
    ...
    result := ""
    len := StrLen(num)
    groupCount := Ceil(len / 4)

    Loop, %groupCount%
    {
        groupIndex := A_Index - 1
        endPos := len - (groupIndex * 4)
        startPos := endPos - 3
        if (startPos < 1)
            startPos := 1
        groupText := SubStr(num, startPos, endPos - startPos + 1)
        groupResult := ""
        ...
        {
            p := A_Index
            d := SubStr(groupText, p, 1) + 0
            if (d = 0)
                continue
            unitIndex := groupLen - p
            groupResult .= digits[d + 1] . smallUnits[unitIndex + 1]
        }

        if (groupResult != "")
            result := groupResult . largeUnits[groupIndex + 1] . result
    }
...
; =========================================================
; 10¹ø ±Ý¾× ´ÜÀ§ º¯È¯
; - ¼±ÅÃÇÑ ±Ý¾×ÀÇ ½°Ç¥ À§Ä¡°¡ Àß¸øµÇ¾î ÀÖ¾îµµ ¼ýÀÚ¸¦ º¹¿ø
; - 1¹é¸¸¿ø / 1Ãµ¸¸¿ø / 1¾ï¿ø µî ÇÑ±Û ´ÜÀ§ ÀÔ·Âµµ ¿ø ´ÜÀ§·Î ÇØ¼®
; - ¼±ÅÃÇÑ ¹öÆ° ´ÜÀ§ ¹Ì¸¸Àº Àý»ç
; =========================================================
QI_MoneyUnitWon:
    QI_ConvertSelectedMoneyUnit("¿ø")
return

QI_MoneyUnitThousand:
    QI_ConvertSelectedMoneyUnit("Ãµ¿ø")
return

QI_MoneyUnitTenThousand:
    QI_ConvertSelectedMoneyUnit("¸¸¿ø")
return

QI_MoneyUnitMillion:
    QI_ConvertSelectedMoneyUnit("¹é¸¸¿ø")
return

QI_MoneyUnitHundredMillion:
    QI_ConvertSelectedMoneyUnit("¾ï¿ø")
return

QI_ConvertSelectedMoneyUnit(targetUnit)
{
    global QILastTargetHwnd, QIPinMode
    ...
    convertedText := QI_FormatMoneyUnit(amountWon, targetUnit)
    if (convertedText = "")
    {
        Clipboard := oldClip
        QI_RestorePinnedGuiFocus()
    ...
    {
        Clipboard := oldClip
        ToolTip, º¯È¯ÇÑ ±Ý¾×À» Å¬¸³º¸µå¿¡ ´ãÁö ¸øÇß½À´Ï´Ù.
        SetTimer, QI_RemoveToolTip, -1500
        QI_RestorePinnedGuiFocus()
        return false
    }
...
QI_ParseMoneyTextToWon(text, ByRef ok)
{
    ok := false
    s := Trim(text, " `t`r`n")

    ; Àß¸ø ÂïÈù ½°Ç¥´Â À§Ä¡¿Í °ü°è¾øÀÌ ¸ðµÎ Á¦°ÅÇÕ´Ï´Ù.
    s := StrReplace(s, ",", "")
    s := StrReplace(s, " ", "")
    s := StrReplace(s, "`t", "")
    s := RegExReplace(s, "^±Ý")
    ...
    {
        part := SubStr(s, pos)
        if (!RegExMatch(part, "^([0-9]+(?:\.[0-9]+)?)(¾ï|Ãµ¸¸|¹é¸¸|½Ê¸¸|¸¸|Ãµ|¹é|½Ê)?", m))
            return 0
...
QI_FormatMoneyUnit(amountWon, targetUnit)
{
    if (targetUnit = "¿ø")
        divisor := 1
    else if (targetUnit = "Ãµ¿ø")
        divisor := 1000
    else if (targetUnit = "¸¸¿ø")
        divisor := 10000
    else if (targetUnit = "¹é¸¸¿ø")
        divisor := 1000000
    else if (targetUnit = "¾ï¿ø")
        divisor := 100000000
    else
        return ""

    if (amountWon >= 0)
        converted := Floor(amountWon / divisor)
    else
        converted := Ceil(amountWon / divisor)

    convertedText := Format("{:.0f}", converted)
    return SSOK_CalcAddCommas(convertedText) . targetUnit
}
...
QI_DateToday:
    Gosub, QI_MakeTodayDate
    ; ¿À´Ã ¹öÆ°Àº ÇöÀç ½Ã°£ ¾øÀÌ ¿À´Ã ³¯Â¥ + ¿äÀÏ¸¸ ÀÔ·ÂÇÕ´Ï´Ù.
    QIInputText := QIYear . ". " . QIMonth . ". " . QIDay . ".(" . QIYoil . ")"
    Gosub, QI_PasteText
return
...
QI_DateFormatToggle:
    ; ³¯Â¥ Çü½ÄÀº ¼±ÅÃ ¿µ¿ª ÀüÃ¼¸¦ ±×´ë·Î °¡Á®¿Í ¿©·¯ ³¯Â¥¸¦ ÇÑ²¨¹ø¿¡ º¯È¯ÇÕ´Ï´Ù.
    ; Ã¹ ¹øÂ°·Î ÀÎ½ÄµÈ ³¯Â¥ÀÇ ÇöÀç Çü½ÄÀ» ±âÁØÀ¸·Î ´ÙÀ½ Çü½ÄÀ» Á¤ÇÏ°í,
    ; ¼±ÅÃ ¿µ¿ª ¾ÈÀÇ ¸ðµç ³¯Â¥¸¦ °°Àº Çü½ÄÀ¸·Î ÅëÀÏÇÕ´Ï´Ù.
    if (!QI_DateGetSelection(QISelectedDateText, QISelectedOldClip, false))
    {
    ...
    if (!QI_BatchFormatDates(QISelectedDateText, QIFormattedDateText, QIChangedDateCount))
    {
        ToolTip, ¼±ÅÃ ¿µ¿ª¿¡¼­ ³¯Â¥ Çü½ÄÀ» ÀÎ½ÄÇÏÁö ¸øÇß½À´Ï´Ù.
        SetTimer, QI_RemoveToolTip, -1500
        return
    }

    QIInputText := QIFormattedDateText
    Gosub, QI_PasteText
return

QI_DateGetSelection(ByRef outText, ByRef oldClip, trimSelection := true)
{
    ...
    if (trimSelection)
        ...
        outText := QIDateSelectionRaw

    Clipboard := oldClip
    return (Trim(outText, " `t`r`n") != "")
}

; ¼±ÅÃ ¿µ¿ª ¾ÈÀÇ ¿©·¯ ³¯Â¥¸¦ ÇÑ ¹ø¿¡ °°Àº Çü½ÄÀ¸·Î º¯È¯
; ¼øÈ¯: 2026. 9. 21. -> 2026. 9. 21.(¿ù) -> 2026.9.21. -> 2026-09-21 -> 2026. 9. 21.
; Ãß°¡ ÀÎ½Ä: 2026.9.21(¸¶Áö¸· Á¡ ¾øÀ½), 2026/9/21, 2026³â 9¿ù 21ÀÏ
; ¿äÀÏÀÌ ÀÌ¹Ì ºÙÀº ³¯Â¥µµ ÇÏ³ªÀÇ ³¯Â¥·Î ÅëÂ°·Î ÀÎ½ÄÇÏ¿© ÀÏ°ý º¯È¯
QI_BatchFormatDates(text, ByRef outText, ByRef changedCount)
{
    changedCount := 0
    outText := text

    ; ³¯Â¥°¡ ¹®Àå ¾È¿¡ ¿©·¯ °³ ÀÖ¾îµµ °¢°¢ Ã£½À´Ï´Ù.
    ; Á¡(.) ³¯Â¥´Â ¸¶Áö¸· Á¡ÀÌ ¾ø¾îµµ ÀÎ½ÄÇÏ¸ç, (¿ù) / (¿ù¿äÀÏ) °°Àº ¿äÀÏµµ ÇÔ²² Àâ½À´Ï´Ù.
    ; ¾ÕµÚ°¡ ¼ýÀÚ·Î ¹Ù·Î ÀÌ¾îÁö´Â °æ¿ì´Â ³¯Â¥·Î Àß¸ø ÀÎ½ÄÇÏÁö ¾Ê½À´Ï´Ù.
    pattern := "O)(?<!\d)(\d{4}\s*\.\s*\d{1,2}\s*\.\s*\d{1,2}\s*\.?(?:\s*\([ÀÏ¿ùÈ­¼ö¸ñ±ÝÅä](?:¿äÀÏ)?\))?|\d{4}-\d{1,2}-\d{1,2}|\d{4}/\d{1,2}/\d{1,2}|\d{4}\s*³â\s*\d{1,2}\s*¿ù\s*\d{1,2}\s*ÀÏ)(?!\d)"

    matches := []
    firstStyle := ""
    searchPos := 1

    while (searchPos := RegExMatch(text, pattern, dm, searchPos))
    {
        rawDate := dm.Value(0)
        if (QI_ParseBatchDate(rawDate, dy, dmo, dd, dstyle))
        {
            if (firstStyle = "")
                firstStyle := dstyle

            matches.Push({pos: dm.Pos(0), len: dm.Len(0), y: dy, m: dmo, d: dd})
        }

        nextPos := dm.Pos(0) + dm.Len(0)
    ...
    ; Ã¹ ³¯Â¥ÀÇ ÇöÀç Çü½ÄÀ» ±âÁØÀ¸·Î ¸ðµç ³¯Â¥¸¦ °°Àº ´ÙÀ½ Çü½ÄÀ¸·Î ÅëÀÏÇÕ´Ï´Ù.
    ; ¸¶Áö¸· Á¡ÀÌ ¾ø´Â Á¡(.) Çü½ÄÀº ÀÔ·Â¿ëÀ¸·Î¸¸ ÀÎÁ¤ÇÏ°í, Ã¹ Å¬¸¯¿¡¼­´Â °ø¹é Á¡ Çü½ÄÀ¸·Î Á¤±ÔÈ­ÇÕ´Ï´Ù.
    if (firstStyle = "spaced")
        targetStyle := "weekday"
    else if (firstStyle = "weekday")
        targetStyle := "compact"
    else if (firstStyle = "compact")
        targetStyle := "dash"
    else if (firstStyle = "dash")
        targetStyle := "spaced"
    else
        targetStyle := "spaced"
    ...
    {
        result .= SubStr(text, cursor, item.pos - cursor)
        result .= QI_FormatDateStyle(item.y, item.m, item.d, targetStyle)
...
QI_ParseBatchDate(dateText, ByRef y, ByRef m, ByRef d, ByRef style)
{
    dateText := Trim(dateText, " `t`r`n")
    style := ""

    ; ¿äÀÏ Æ÷ÇÔ Çü½ÄÀ» °¡Àå ¸ÕÀú °Ë»çÇØ¾ß ³¯Â¥ µÚÀÇ (¿ù)±îÁö ÇÑ µ¢¾î¸®·Î Ã³¸®µË´Ï´Ù.
    if (RegExMatch(dateText, "^(\d{4})\s*\.\s*(\d{1,2})\s*\.\s*(\d{1,2})\s*\.\s*\([ÀÏ¿ùÈ­¼ö¸ñ±ÝÅä](?:¿äÀÏ)?\)$", md))
        style := "weekday"
    else if (RegExMatch(dateText, "^(\d{4})\.\s*(\d{1,2})\.\s*(\d{1,2})\.$", md))
    {
        ; Á¡ »çÀÌ¿¡ °ø¹éÀÌ ÀÖÀ¸¸é spaced, ¾øÀ¸¸é compact·Î ±¸ºÐÇÕ´Ï´Ù.
        if (RegExMatch(dateText, "\d\.\s+\d"))
            style := "spaced"
        else
            style := "compact"
    }
    else if (RegExMatch(dateText, "^(\d{4})\s*\.\s*(\d{1,2})\s*\.\s*(\d{1,2})$", md))
        style := "dot_nofinal"
    else if (RegExMatch(dateText, "^(\d{4})-(\d{1,2})-(\d{1,2})$", md))
        style := "dash"
    else if (RegExMatch(dateText, "^(\d{4})/(\d{1,2})/(\d{1,2})$", md))
        style := "slash"
    else if (RegExMatch(dateText, "^(\d{4})\s*³â\s*(\d{1,2})\s*¿ù\s*(\d{1,2})\s*ÀÏ$", md))
        style := "korean"
    else
        return false
...
QI_FormatDateStyle(y, m, d, style)
{
    if (style = "weekday")
    {
        stamp := Format("{:04}{:02}{:02}000000", y, m, d)
...
QI_ParseSimpleDate(dateText, ByRef y, ByRef m, ByRef d, ByRef style)
{
    ...
    ; ´ÜÀÏ ³¯Â¥ ±â´Éµµ ¸¶Áö¸· Á¡ÀÌ ¾ø´Â 2026.9.21 ¹× ¿äÀÏ Æ÷ÇÔ ³¯Â¥¸¦ ÀÎ½ÄÇÕ´Ï´Ù.
    if (RegExMatch(dateText, "^(\d{4})\s*\.\s*(\d{1,2})\s*\.\s*(\d{1,2})\s*\.\s*\([ÀÏ¿ùÈ­¼ö¸ñ±ÝÅä](?:¿äÀÏ)?\)$", md))
        style := "weekday"
    else if (RegExMatch(dateText, "^(\d{4})\s*\.\s*(\d{1,2})\s*\.\s*(\d{1,2})\s*\.?$", md))
        style := "dot"
    else if (RegExMatch(dateText, "^(\d{4})-(\d{1,2})-(\d{1,2})$", md))
        style := "dash"
    else
...
QI_ShowSpecialPicker:
    ...
    QISymbolMap := StrSplit("¡î|¢Ñ|¢¡|¢¹|¢º|¡Ü|¡Ý|¡á|¢Ã|¡¸|¡¹|¡¼|¡½|¡²|¡³|¡¶|¡·|¡º|¡»|¡Þ|¡ß|¡Ú|¡â|¡ã|¡ä|¡å|¡ç|¡è|¡é|¡ê|¢Ï|¥°|¥±|¥²|¥³|¥´|¨ç|¨è|¨é|¨ê|¨ë|¡É|§®|§¯|§°|§³|§¨|§±|§¸|¢ß|¡À|¡¿", "|")
