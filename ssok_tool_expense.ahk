...
SSOK_Expense_BudgetDashboard_Clear:
    ...
    GuiControl, SSOKBudgetDashboard:, SSOK_BudgetDashboardFileDrop, »ç¾÷°ü¸®Ä«µå(Çö¾×) Excel ÆÄÀÏÀ» ¿©±â¿¡ ³õ¾ÆÁÖ¼¼¿ä
...
SSOK_Expense_BudgetDashboard_ExportOverall:
    SSOK_Expense_BudgetDashboard_ExportListView("SSOK_BudgetDashboardMainLV", "ÀüÃ¼ÇöÈ²"
        , "±¸ºÐ|»ó¼¼|¿¹»êÇö¾×|¿øÀÎÇàÀ§¾×|¿øÀÎÇàÀ§ ÁýÇà·ü|¿¹»êÀÜ¾×|Áö±Þ¾×|Áö±Þ ÁýÇà·ü|ÁöÃâÀÜ¾×|¸ñÀûÀç¿ø|ÀÏ¹ÝÀç¿ø"
...
SSOK_Expense_BudgetDashboard_ExportPurposeUser:
    SSOK_Expense_BudgetDashboard_ExportListView("SSOK_BudgetDashboardPurposeUserLV", "¸ñÀûÀç¿ø_¼öÀÍÀÚ"
        , "¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|¿øÀÎÇàÀ§¾×|¿øÀÎÇàÀ§ ÁýÇà·ü|¿¹»êÀÜ¾×|Áö±Þ¾×|Áö±Þ ÁýÇà·ü|ÁöÃâÀÜ¾×|´ã´çÀÚ"
...
SSOK_Expense_BudgetDashboard_ExportOver:
    SSOK_Expense_BudgetDashboard_ExportListView("SSOK_BudgetDashboardOverLV", "ÃÊ°úÁýÇà_°ü¸®"
        , "»óÅÂ|Àç¿ø|¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|¿øÀÎÇàÀ§¾×|¿øÀÎÇàÀ§ ÁýÇà·ü|¿¹»êÀÜ¾×|Áö±Þ¾×|Áö±Þ ÁýÇà·ü|ÁöÃâÀÜ¾×", "SSOK_BudgetDashboardOverStatus")
return
...
{
    entries := SSOK_Expense_BudgetDashboard_BuildOverEntries()
    ...
    for _, row in entries
    {
        ...
        LV_Add("", isOver ? "ÃÊ°ú" : "°ü·Ã", row.resourceType, row.business, row.subItem, row.calc, row.cost
            , SSOK_Expense_FormatNumber(row.current), SSOK_Expense_FormatNumber(row.commit)
            , SSOK_Expense_BudgetDashboard_FormatRate(row.current, row.commit), SSOK_Expense_FormatNumber(row.current - row.commit)
            , SSOK_Expense_FormatNumber(row.paid), SSOK_Expense_BudgetDashboard_FormatRate(row.current, row.paid)
            , SSOK_Expense_FormatNumber(row.current - row.paid))
        current += row.current, commit += row.commit, paid += row.paid
        if (isOver)
            overCount++, excess += row.commit - row.current
    }
    widths := [65,75,120,140,210,110,100,100,95,100,100,95,100]
...
SSOK_Expense_BudgetDashboard_ExportCost:
    SSOK_Expense_BudgetDashboard_ExportListView("SSOK_BudgetDashboardCostLV", "¿ø°¡Åë°èºñ¸ñº°_ÇÕ°è"
        , "¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|¿øÀÎÇàÀ§¾×|¿øÀÎÇàÀ§ ÁýÇà·ü|¿¹»êÀÜ¾×|Áö±Þ¾×|Áö±Þ ÁýÇà·ü|ÁöÃâÀÜ¾×|¸ñÀûÀç¿ø|ÀÏ¹ÝÀç¿ø", "SSOK_BudgetDashboardCostStatus")
return
...
SSOK_Expense_BudgetDashboard_FillCost()
{
    ...
    {
        LV_Add("", entry.label, SSOK_Expense_FormatNumber(entry.current)
            , SSOK_Expense_FormatNumber(entry.commit), SSOK_Expense_BudgetDashboard_FormatRate(entry.current, entry.commit)
            , SSOK_Expense_FormatNumber(entry.current - entry.commit), SSOK_Expense_FormatNumber(entry.paid)
            , SSOK_Expense_BudgetDashboard_FormatRate(entry.current, entry.paid), SSOK_Expense_FormatNumber(entry.current - entry.paid)
...
SSOK_Expense_BudgetDashboard_Show()
{
    ...
    fileDisplay := FileExist(SSOK_BudgetDashboardFilePath) ? SSOK_BudgetDashboardFilePath : "»ç¾÷°ü¸®Ä«µå(Çö¾×) Excel ÆÄÀÏÀ» ¿©±â¿¡ ³õ¾ÆÁÖ¼¼¿ä"
...
SSOK_Expense_BudgetDashboard_HandleDrop(dropText, targetControl := "")
{
    ...
    GuiControl, SSOKBudgetDashboard:, SSOK_BudgetDashboardStatus, ÆÄÀÏÀ» ÀÐ°í ºÐ¼®ÇÏ°í ÀÖ½À´Ï´Ù...
...
SSOK_Expense_BudgetDashboard_LoadFile(filePath)
{
    ...
    {
        GuiControl, SSOKBudgetDashboard:, SSOK_BudgetDashboardStatus, ºÐ¼® ½ÇÆÐ
        if (errMsg = "")
    ...
    Loop, %rowTotal%
    {
        ...
        Loop, %colTotal%
        {
            ...
            if (key = "¼¼ºÎ»ç¾÷")
                ...
                tmpDraft := c
            else if InStr(key, "¿øÀÎÇàÀ§¾×")
                tmpCommit := c
            else if InStr(key, "Áö±Þ¾×")
                tmpPaid := c
            else if (key = "Á¤»êÀç¿ø")
                tmpSettle := c
        ...
        if (tmpBusiness && tmpCost && tmpCalc && tmpCurrent && tmpDraft && tmpCommit && tmpPaid)
        {
            headerRow := r
...
SSOK_Expense_BudgetDashboard_ShowDashboard()
{
    ...
    Gui, SSOKBudgetDashboard:Font, s13 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, Text, x18 y10 w500 h28 c005BAC, ¿¹»ê DashBoard
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, Text, x560 y12 w920 h20 Right c555555, % "»êÃâ³»¿ª " . p.rowCount . "Çà  |  ÃÊ°úÁýÇà " . p.overCount . "°Ç  |  " . p.verifyText . "  |  " . p.resourceVerifyText
    Gui, SSOKBudgetDashboard:Add, Text, x560 y32 w920 h18 Right c777777, %sourceName%
    ...
    SSOK_Expense_BudgetDashboard_AddSplitKpi(18, 50, 202, "¿¹»êÇö¾×"
    ...
    SSOK_Expense_BudgetDashboard_AddSplitKpi(228, 50, 202, "¿øÀÎÇàÀ§¾×"
    ...
    SSOK_Expense_BudgetDashboard_AddSplitKpi(438, 50, 202, "¿øÀÎÇàÀ§ ÁýÇà·ü"
    ...
    SSOK_Expense_BudgetDashboard_AddSplitKpi(648, 50, 202, "¿¹»êÀÜ¾×"
        , SSOK_Expense_FormatNumber(p.current - p.commit) . "¿ø"
        , "ÀÏ¹Ý " . SSOK_Expense_FormatNumber(p.general - p.generalCommit) . "¿ø"
        , "¸ñÀû " . SSOK_Expense_FormatNumber(p.settle - p.settleCommit) . "¿ø", balanceColor)
    SSOK_Expense_BudgetDashboard_AddSplitKpi(858, 50, 202, "Áö±Þ¾×"
    ...
    SSOK_Expense_BudgetDashboard_AddSplitKpi(1068, 50, 202, "Áö±Þ ÁýÇà·ü"
        , SSOK_Expense_BudgetDashboard_FormatRate(p.current, p.paid)
        , "ÀÏ¹Ý " . SSOK_Expense_BudgetDashboard_FormatRate(p.general, p.generalPaid)
        , "¸ñÀû " . SSOK_Expense_BudgetDashboard_FormatRate(p.settle, p.settlePaid), "00856A")
    SSOK_Expense_BudgetDashboard_AddSplitKpi(1278, 50, 202, "ÁöÃâÀÜ¾×"
        , SSOK_Expense_FormatNumber(p.current - p.paid) . "¿ø"
        , "ÀÏ¹Ý " . SSOK_Expense_FormatNumber(p.general - p.generalPaid) . "¿ø"
        , "¸ñÀû " . SSOK_Expense_FormatNumber(p.settle - p.settlePaid) . "¿ø", paidBalanceColor)

    Gui, SSOKBudgetDashboard:Font, s9 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, Tab3, x18 y136 w1462 h786 vSSOK_BudgetDashboardTab gSSOK_Expense_BudgetDashboard_SelectSection, ÀüÃ¼ ÇöÈ²|¸ñÀûÀç¿ø °ü¸®|ÀÏ¹ÝÀç¿ø °ü¸®|¾÷¹«ÃßÁøºñ °ü¸®|ÃÊ°úÁýÇà °ü¸®|¿ø°¡Åë°èºñ¸ñº° ÇÕ°è

    ; ÀüÃ¼ ÇöÈ²
    Gui, SSOKBudgetDashboard:Tab, 1, 1
    Gui, SSOKBudgetDashboard:Font, s8 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, Text, x32 y177 w36 h24 +0x200, º¸±â
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    ...
    SSOK_Expense_BudgetDashboard_AddChoiceButtons("SSOK_BudgetDashboardViewChoice", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|»êÃâ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", 32, 206, 116, "view")
    Gui, SSOKBudgetDashboard:Font, s8 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, Text, x260 y177 w36 h24 +0x200, °Ë»ö
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, Edit, x296 y175 w250 h24 vSSOK_BudgetDashboardSearch gSSOK_Expense_BudgetDashboard_Refresh
    Gui, SSOKBudgetDashboard:Font, s8 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, Text, x558 y177 w220 h24 +0x200, ¿­ Á¦¸ñ Å¬¸¯: ¿À¸§/³»¸²Â÷¼ø
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    ...
    SSOK_Expense_BudgetDashboard_AddChoiceButtons("SSOK_BudgetDashboardResourceChoice", "ÀüÃ¼|ÀÏ¹ÝÀç¿ø|¸ñÀûÀç¿ø", "ÀüÃ¼|ÀÏ¹ÝÀç¿ø|¸ñÀûÀç¿ø", 796, 175, 50, "resource")
    Gui, SSOKBudgetDashboard:Add, CheckBox, x950 y177 w100 h24 vSSOK_BudgetDashboardHideZero gSSOK_Expense_BudgetDashboard_Refresh, 0¿ø Çà Á¦¿Ü
    Gui, SSOKBudgetDashboard:Add, CheckBox, x1054 y177 w105 h24 vSSOK_BudgetDashboardOverOnly gSSOK_Expense_BudgetDashboard_Refresh, ÃÊ°úÁýÇà¸¸
    Gui, SSOKBudgetDashboard:Add, Button, x1168 y172 w86 h30 gSSOK_Expense_BudgetDashboard_CopyView, Ç¥ º¹»ç
    Gui, SSOKBudgetDashboard:Add, Button, x1260 y172 w102 h30 gSSOK_Expense_BudgetDashboard_ExportOverall, Excel ÃßÃâ
    Gui, SSOKBudgetDashboard:Add, Button, x1368 y172 w98 h30 gSSOKBudgetDashboardGuiClose, ´Ý±â
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, CheckBox, x520 y207 w500 h24 vSSOK_BudgetDashboardSubtotal gSSOK_Expense_BudgetDashboard_Refresh, Àç¿ø ±¸ºÐº° ºÎºÐÇÕ°è º¸±â (ÇØÁ¦: ¼±ÅÃÇÑ º¸±â)
    Gui, SSOKBudgetDashboard:Add, ListView, x32 y268 w1434 h564 Grid -Multi NoSort gSSOK_Expense_BudgetDashboard_HeaderClick vSSOK_BudgetDashboardMainLV, ±¸ºÐ|»ó¼¼|¿¹»êÇö¾×|¿øÀÎÇàÀ§¾×|¿øÀÎÇàÀ§ ÁýÇà·ü|¿¹»êÀÜ¾×|Áö±Þ¾×|Áö±Þ ÁýÇà·ü|ÁöÃâÀÜ¾×|¸ñÀûÀç¿ø|ÀÏ¹ÝÀç¿ø
    SSOK_Expense_BudgetDashboard_AddTopSummary("SSOK_BudgetDashboardMainLV", [150,240,105,105,105,105,105,95,105,105,105], 32, 240, 1434, 592)
    Gui, SSOKBudgetDashboard:Add, Text, x32 y838 w1434 h34 +0x200 vSSOK_BudgetDashboardListStatus Hidden c555555,
    Gui, SSOKBudgetDashboard:Add, Button, x1266 y876 w96 h28 gSSOK_Expense_BudgetDashboard_BackToInput, ´Ù¸¥ ÆÄÀÏ

    ; ¸ñÀûÀç¿ø: ¸ñÀû»ç¾÷ / ±¹°íº¸Á¶±Ý / ¼öÀÍÀÚ
    Gui, SSOKBudgetDashboard:Tab, 2, 1
    Gui, SSOKBudgetDashboard:Font, s8 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x46 y176 w260 vSSOK_BudgetDashboardPurposeSubTab gSSOK_Expense_BudgetDashboard_SelectSection, ÀüÃ¼||¸ñÀû»ç¾÷|±¹°íº¸Á¶±Ý|¼öÀÍÀÚ
    SSOK_Expense_BudgetDashboard_AddChoiceButtons("SSOK_BudgetDashboardPurposeSubTab", "ÀüÃ¼|¸ñÀû»ç¾÷ºñ|±¹°íº¸Á¶±Ý|¼öÀÍÀÚºÎ´ã±Ý", "ÀüÃ¼|¸ñÀû»ç¾÷|±¹°íº¸Á¶±Ý|¼öÀÍÀÚ", 46, 174, 100, "section")
    Gui, SSOKBudgetDashboard:Add, Text, x560 y176 w890 h28 +0x200 c005BAC vSSOK_BudgetDashboardPurposeTotal,

    ; ¸ñÀûÀç¿ø > ¸ñÀû»ç¾÷
    Gui, SSOKBudgetDashboard:Tab, 2, 1
    Gui, SSOKBudgetDashboard:Font, s8 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, Text, x46 y214 w36 h24 +0x200 vSSOK_BudgetDashboardPurposeAllLabel, º¸±â
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x84 y212 w140 vSSOK_BudgetDashboardPurposeAllView gSSOK_Expense_BudgetDashboard_ResourceViewRefresh, ¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°||¿ø°¡Åë°èºñ¸ñº°
    SSOK_Expense_BudgetDashboard_AddChoiceButtons("SSOK_BudgetDashboardPurposeAllView", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", 84, 210, 116, "view")
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x560 y212 w190 vSSOK_BudgetDashboardPurposeAllSort gSSOK_Expense_BudgetDashboard_ResourceViewRefresh, ÀÌ¸§¼ø||Çö¾× Å«¼ø|¿øÀÎÇàÀ§ Å«¼ø|Áö±Þ¾× Å«¼ø|ÀÜ¾× Å«¼ø|ÁýÇà·ü ³ôÀº¼ø|ÁýÇà·ü ³·Àº¼ø
    Gui, SSOKBudgetDashboard:Add, ListView, x46 y274 w1406 h527 Grid -Multi NoSort gSSOK_Expense_BudgetDashboard_HeaderClick vSSOK_BudgetDashboardPurposeAllLV, ¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|¿øÀÎÇàÀ§¾×|¿øÀÎÇàÀ§ ÁýÇà·ü|¿¹»êÀÜ¾×|Áö±Þ¾×|Áö±Þ ÁýÇà·ü|ÁöÃâÀÜ¾×|´ã´çÀÚ
    SSOK_Expense_BudgetDashboard_AddTopSummary("SSOK_BudgetDashboardPurposeAllLV", [150,150,220,130,100,100,100,100,100,100,100,150], 46, 246, 1406, 555)
    ...
    Gui, SSOKBudgetDashboard:Tab, 2, 1
    Gui, SSOKBudgetDashboard:Font, s8 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, Text, x46 y214 w36 h24 +0x200 vSSOK_BudgetDashboardPurposeProjectLabel, º¸±â
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x84 y212 w140 vSSOK_BudgetDashboardPurposeProjectView gSSOK_Expense_BudgetDashboard_ResourceViewRefresh, ¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°||¿ø°¡Åë°èºñ¸ñº°
    SSOK_Expense_BudgetDashboard_AddChoiceButtons("SSOK_BudgetDashboardPurposeProjectView", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", 84, 210, 116, "view")
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x560 y212 w190 vSSOK_BudgetDashboardPurposeProjectSort gSSOK_Expense_BudgetDashboard_ResourceViewRefresh, ÀÌ¸§¼ø||Çö¾× Å«¼ø|¿øÀÎÇàÀ§ Å«¼ø|Áö±Þ¾× Å«¼ø|ÀÜ¾× Å«¼ø|ÁýÇà·ü ³ôÀº¼ø|ÁýÇà·ü ³·Àº¼ø
    Gui, SSOKBudgetDashboard:Add, ListView, x46 y274 w1406 h527 Grid -Multi NoSort gSSOK_Expense_BudgetDashboard_HeaderClick vSSOK_BudgetDashboardPurposeProjectLV, ¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|¿øÀÎÇàÀ§¾×|¿øÀÎÇàÀ§ ÁýÇà·ü|¿¹»êÀÜ¾×|Áö±Þ¾×|Áö±Þ ÁýÇà·ü|ÁöÃâÀÜ¾×|´ã´çÀÚ
    SSOK_Expense_BudgetDashboard_AddTopSummary("SSOK_BudgetDashboardPurposeProjectLV", [150,150,220,130,100,100,100,100,100,100,100,150], 46, 246, 1406, 555)
    ...
    Gui, SSOKBudgetDashboard:Tab, 2, 1
    Gui, SSOKBudgetDashboard:Font, s8 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, Text, x46 y214 w36 h24 +0x200 vSSOK_BudgetDashboardPurposeNationalLabel, º¸±â
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x84 y212 w140 vSSOK_BudgetDashboardPurposeNationalView gSSOK_Expense_BudgetDashboard_ResourceViewRefresh, ¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°||¿ø°¡Åë°èºñ¸ñº°
    SSOK_Expense_BudgetDashboard_AddChoiceButtons("SSOK_BudgetDashboardPurposeNationalView", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", 84, 210, 116, "view")
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x560 y212 w190 vSSOK_BudgetDashboardPurposeNationalSort gSSOK_Expense_BudgetDashboard_ResourceViewRefresh, ÀÌ¸§¼ø||Çö¾× Å«¼ø|¿øÀÎÇàÀ§ Å«¼ø|Áö±Þ¾× Å«¼ø|ÀÜ¾× Å«¼ø|ÁýÇà·ü ³ôÀº¼ø|ÁýÇà·ü ³·Àº¼ø
    Gui, SSOKBudgetDashboard:Add, ListView, x46 y274 w1406 h527 Grid -Multi NoSort gSSOK_Expense_BudgetDashboard_HeaderClick vSSOK_BudgetDashboardPurposeNationalLV, ¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|¿øÀÎÇàÀ§¾×|¿øÀÎÇàÀ§ ÁýÇà·ü|¿¹»êÀÜ¾×|Áö±Þ¾×|Áö±Þ ÁýÇà·ü|ÁöÃâÀÜ¾×|´ã´çÀÚ
    SSOK_Expense_BudgetDashboard_AddTopSummary("SSOK_BudgetDashboardPurposeNationalLV", [150,150,220,130,100,100,100,100,100,100,100,150], 46, 246, 1406, 555)
    ...
    Gui, SSOKBudgetDashboard:Tab, 2, 1
    Gui, SSOKBudgetDashboard:Font, s8 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, Text, x46 y214 w36 h24 +0x200 vSSOK_BudgetDashboardPurposeUserLabel, º¸±â
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x84 y212 w140 vSSOK_BudgetDashboardPurposeUserView gSSOK_Expense_BudgetDashboard_ResourceViewRefresh, ¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°||¿ø°¡Åë°èºñ¸ñº°
    SSOK_Expense_BudgetDashboard_AddChoiceButtons("SSOK_BudgetDashboardPurposeUserView", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", 84, 210, 116, "view")
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x560 y212 w190 vSSOK_BudgetDashboardPurposeUserSort gSSOK_Expense_BudgetDashboard_ResourceViewRefresh, ÀÌ¸§¼ø||Çö¾× Å«¼ø|¿øÀÎÇàÀ§ Å«¼ø|Áö±Þ¾× Å«¼ø|ÀÜ¾× Å«¼ø|ÁýÇà·ü ³ôÀº¼ø|ÁýÇà·ü ³·Àº¼ø
    Gui, SSOKBudgetDashboard:Add, ListView, x46 y274 w1406 h527 Grid -Multi NoSort gSSOK_Expense_BudgetDashboard_HeaderClick vSSOK_BudgetDashboardPurposeUserLV, ¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|¿øÀÎÇàÀ§¾×|¿øÀÎÇàÀ§ ÁýÇà·ü|¿¹»êÀÜ¾×|Áö±Þ¾×|Áö±Þ ÁýÇà·ü|ÁöÃâÀÜ¾×|´ã´çÀÚ
    SSOK_Expense_BudgetDashboard_AddTopSummary("SSOK_BudgetDashboardPurposeUserLV", [150,150,220,130,100,100,100,100,100,100,100,150], 46, 246, 1406, 555)
    ...
    Gui, SSOKBudgetDashboard:Font, s8 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x46 y176 w260 vSSOK_BudgetDashboardGeneralSubTab gSSOK_Expense_BudgetDashboard_SelectSection, ÀüÃ¼||¹Ì·¡|Æ¯¼º|ÅëÇÕ|ÀÏ¹Ý
    SSOK_Expense_BudgetDashboard_AddChoiceButtons("SSOK_BudgetDashboardGeneralSubTab", "ÀüÃ¼|¹Ì·¡|Æ¯¼º|ÅëÇÕ|ÀÏ¹Ý", "ÀüÃ¼|¹Ì·¡|Æ¯¼º|ÅëÇÕ|ÀÏ¹Ý", 46, 174, 100, "section")
    Gui, SSOKBudgetDashboard:Add, Text, x560 y176 w890 h28 +0x200 c005BAC vSSOK_BudgetDashboardGeneralTotal,

    ; ÀÏ¹ÝÀç¿ø > ¹Ì·¡
    Gui, SSOKBudgetDashboard:Tab, 3, 1
    Gui, SSOKBudgetDashboard:Font, s8 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, Text, x46 y214 w36 h24 +0x200 vSSOK_BudgetDashboardGeneralAllLabel, º¸±â
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x84 y212 w140 vSSOK_BudgetDashboardGeneralAllView gSSOK_Expense_BudgetDashboard_ResourceViewRefresh, ¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°||¿ø°¡Åë°èºñ¸ñº°
    SSOK_Expense_BudgetDashboard_AddChoiceButtons("SSOK_BudgetDashboardGeneralAllView", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", 84, 210, 116, "view")
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x560 y212 w190 vSSOK_BudgetDashboardGeneralAllSort gSSOK_Expense_BudgetDashboard_ResourceViewRefresh, ÀÌ¸§¼ø||Çö¾× Å«¼ø|¿øÀÎÇàÀ§ Å«¼ø|Áö±Þ¾× Å«¼ø|ÀÜ¾× Å«¼ø|ÁýÇà·ü ³ôÀº¼ø|ÁýÇà·ü ³·Àº¼ø
    Gui, SSOKBudgetDashboard:Add, ListView, x46 y274 w1406 h527 Grid -Multi NoSort gSSOK_Expense_BudgetDashboard_HeaderClick vSSOK_BudgetDashboardGeneralAllLV, ¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|¿øÀÎÇàÀ§¾×|¿øÀÎÇàÀ§ ÁýÇà·ü|¿¹»êÀÜ¾×|Áö±Þ¾×|Áö±Þ ÁýÇà·ü|ÁöÃâÀÜ¾×|´ã´çÀÚ
    SSOK_Expense_BudgetDashboard_AddTopSummary("SSOK_BudgetDashboardGeneralAllLV", [150,150,220,130,100,100,100,100,100,100,100,150], 46, 246, 1406, 555)
    ...
    Gui, SSOKBudgetDashboard:Tab, 3, 1
    Gui, SSOKBudgetDashboard:Font, s8 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, Text, x46 y214 w36 h24 +0x200 vSSOK_BudgetDashboardFutureLabel, º¸±â
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x84 y212 w140 vSSOK_BudgetDashboardFutureView gSSOK_Expense_BudgetDashboard_ResourceViewRefresh, ¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°||¿ø°¡Åë°èºñ¸ñº°
    SSOK_Expense_BudgetDashboard_AddChoiceButtons("SSOK_BudgetDashboardFutureView", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", 84, 210, 116, "view")
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x560 y212 w190 vSSOK_BudgetDashboardFutureSort gSSOK_Expense_BudgetDashboard_ResourceViewRefresh, ÀÌ¸§¼ø||Çö¾× Å«¼ø|¿øÀÎÇàÀ§ Å«¼ø|Áö±Þ¾× Å«¼ø|ÀÜ¾× Å«¼ø|ÁýÇà·ü ³ôÀº¼ø|ÁýÇà·ü ³·Àº¼ø
    Gui, SSOKBudgetDashboard:Add, ListView, x46 y274 w1406 h527 Grid -Multi NoSort gSSOK_Expense_BudgetDashboard_HeaderClick vSSOK_BudgetDashboardFutureLV, ¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|¿øÀÎÇàÀ§¾×|¿øÀÎÇàÀ§ ÁýÇà·ü|¿¹»êÀÜ¾×|Áö±Þ¾×|Áö±Þ ÁýÇà·ü|ÁöÃâÀÜ¾×|´ã´çÀÚ
    SSOK_Expense_BudgetDashboard_AddTopSummary("SSOK_BudgetDashboardFutureLV", [150,150,220,130,100,100,100,100,100,100,100,150], 46, 246, 1406, 555)
    ...
    Gui, SSOKBudgetDashboard:Tab, 3, 1
    Gui, SSOKBudgetDashboard:Font, s8 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, Text, x46 y214 w36 h24 +0x200 vSSOK_BudgetDashboardSpecialLabel, º¸±â
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x84 y212 w140 vSSOK_BudgetDashboardSpecialView gSSOK_Expense_BudgetDashboard_ResourceViewRefresh, ¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°||¿ø°¡Åë°èºñ¸ñº°
    SSOK_Expense_BudgetDashboard_AddChoiceButtons("SSOK_BudgetDashboardSpecialView", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", 84, 210, 116, "view")
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x560 y212 w190 vSSOK_BudgetDashboardSpecialSort gSSOK_Expense_BudgetDashboard_ResourceViewRefresh, ÀÌ¸§¼ø||Çö¾× Å«¼ø|¿øÀÎÇàÀ§ Å«¼ø|Áö±Þ¾× Å«¼ø|ÀÜ¾× Å«¼ø|ÁýÇà·ü ³ôÀº¼ø|ÁýÇà·ü ³·Àº¼ø
    Gui, SSOKBudgetDashboard:Add, ListView, x46 y274 w1406 h527 Grid -Multi NoSort gSSOK_Expense_BudgetDashboard_HeaderClick vSSOK_BudgetDashboardSpecialLV, ¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|¿øÀÎÇàÀ§¾×|¿øÀÎÇàÀ§ ÁýÇà·ü|¿¹»êÀÜ¾×|Áö±Þ¾×|Áö±Þ ÁýÇà·ü|ÁöÃâÀÜ¾×|´ã´çÀÚ
    SSOK_Expense_BudgetDashboard_AddTopSummary("SSOK_BudgetDashboardSpecialLV", [150,150,220,130,100,100,100,100,100,100,100,150], 46, 246, 1406, 555)
    ...
    Gui, SSOKBudgetDashboard:Tab, 3, 1
    Gui, SSOKBudgetDashboard:Font, s8 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, Text, x46 y214 w36 h24 +0x200 vSSOK_BudgetDashboardIntegratedLabel, º¸±â
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x84 y212 w140 vSSOK_BudgetDashboardIntegratedView gSSOK_Expense_BudgetDashboard_ResourceViewRefresh, ¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°||¿ø°¡Åë°èºñ¸ñº°
    SSOK_Expense_BudgetDashboard_AddChoiceButtons("SSOK_BudgetDashboardIntegratedView", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", 84, 210, 116, "view")
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x560 y212 w190 vSSOK_BudgetDashboardIntegratedSort gSSOK_Expense_BudgetDashboard_ResourceViewRefresh, ÀÌ¸§¼ø||Çö¾× Å«¼ø|¿øÀÎÇàÀ§ Å«¼ø|Áö±Þ¾× Å«¼ø|ÀÜ¾× Å«¼ø|ÁýÇà·ü ³ôÀº¼ø|ÁýÇà·ü ³·Àº¼ø
    Gui, SSOKBudgetDashboard:Add, ListView, x46 y274 w1406 h527 Grid -Multi NoSort gSSOK_Expense_BudgetDashboard_HeaderClick vSSOK_BudgetDashboardIntegratedLV, ¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|¿øÀÎÇàÀ§¾×|¿øÀÎÇàÀ§ ÁýÇà·ü|¿¹»êÀÜ¾×|Áö±Þ¾×|Áö±Þ ÁýÇà·ü|ÁöÃâÀÜ¾×|´ã´çÀÚ
    SSOK_Expense_BudgetDashboard_AddTopSummary("SSOK_BudgetDashboardIntegratedLV", [150,150,220,130,100,100,100,100,100,100,100,150], 46, 246, 1406, 555)
    ...
    Gui, SSOKBudgetDashboard:Tab, 3, 1
    Gui, SSOKBudgetDashboard:Font, s8 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, Text, x46 y214 w36 h24 +0x200 vSSOK_BudgetDashboardGeneralLabel, º¸±â
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x84 y212 w140 vSSOK_BudgetDashboardGeneralView gSSOK_Expense_BudgetDashboard_ResourceViewRefresh, ¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°||¿ø°¡Åë°èºñ¸ñº°
    SSOK_Expense_BudgetDashboard_AddChoiceButtons("SSOK_BudgetDashboardGeneralView", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", 84, 210, 116, "view")
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x560 y212 w190 vSSOK_BudgetDashboardGeneralSort gSSOK_Expense_BudgetDashboard_ResourceViewRefresh, ÀÌ¸§¼ø||Çö¾× Å«¼ø|¿øÀÎÇàÀ§ Å«¼ø|Áö±Þ¾× Å«¼ø|ÀÜ¾× Å«¼ø|ÁýÇà·ü ³ôÀº¼ø|ÁýÇà·ü ³·Àº¼ø
    Gui, SSOKBudgetDashboard:Add, ListView, x46 y274 w1406 h527 Grid -Multi NoSort gSSOK_Expense_BudgetDashboard_HeaderClick vSSOK_BudgetDashboardGeneralLV, ¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|¿øÀÎÇàÀ§¾×|¿øÀÎÇàÀ§ ÁýÇà·ü|¿¹»êÀÜ¾×|Áö±Þ¾×|Áö±Þ ÁýÇà·ü|ÁöÃâÀÜ¾×|´ã´çÀÚ
    SSOK_Expense_BudgetDashboard_AddTopSummary("SSOK_BudgetDashboardGeneralLV", [150,150,220,130,100,100,100,100,100,100,100,150], 46, 246, 1406, 555)
    ...
    Gui, SSOKBudgetDashboard:Font, s8 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x46 y176 w260 vSSOK_BudgetDashboardWorkSubTab gSSOK_Expense_BudgetDashboard_SelectSection, ÀüÃ¼||ÀÏ¹Ý|¸ñÀû
    SSOK_Expense_BudgetDashboard_AddChoiceButtons("SSOK_BudgetDashboardWorkSubTab", "ÀüÃ¼|ÀÏ¹Ý|¸ñÀû", "ÀüÃ¼|ÀÏ¹Ý|¸ñÀû", 46, 174, 100, "section")
    Gui, SSOKBudgetDashboard:Add, Text, x560 y176 w890 h28 +0x200 c005BAC vSSOK_BudgetDashboardWorkTotal,

    Gui, SSOKBudgetDashboard:Tab, 4, 1
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, ListView, x46 y244 w1406 h567 Grid -Multi NoSort gSSOK_Expense_BudgetDashboard_HeaderClick vSSOK_BudgetDashboardWorkAllLV, ¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|¿øÀÎÇàÀ§¾×|¿øÀÎÇàÀ§ ÁýÇà·ü|¿¹»êÀÜ¾×|Áö±Þ¾×|Áö±Þ ÁýÇà·ü|ÁöÃâÀÜ¾×|´ã´çÀÚ
    SSOK_Expense_BudgetDashboard_AddTopSummary("SSOK_BudgetDashboardWorkAllLV", [160,160,235,130,95,95,105,95,95,95,95,170], 46, 216, 1406, 595)
    ...
    Gui, SSOKBudgetDashboard:Tab, 4, 1
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, ListView, x46 y244 w1406 h567 Grid -Multi NoSort gSSOK_Expense_BudgetDashboard_HeaderClick vSSOK_BudgetDashboardWorkGeneralLV, ¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|¿øÀÎÇàÀ§¾×|¿øÀÎÇàÀ§ ÁýÇà·ü|¿¹»êÀÜ¾×|Áö±Þ¾×|Áö±Þ ÁýÇà·ü|ÁöÃâÀÜ¾×|´ã´çÀÚ
    SSOK_Expense_BudgetDashboard_AddTopSummary("SSOK_BudgetDashboardWorkGeneralLV", [160,160,235,130,95,95,105,95,95,95,95,170], 46, 216, 1406, 595)
    ...
    Gui, SSOKBudgetDashboard:Tab, 4, 1
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, ListView, x46 y244 w1406 h567 Grid -Multi NoSort gSSOK_Expense_BudgetDashboard_HeaderClick vSSOK_BudgetDashboardWorkPurposeLV, ¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|¿øÀÎÇàÀ§¾×|¿øÀÎÇàÀ§ ÁýÇà·ü|¿¹»êÀÜ¾×|Áö±Þ¾×|Áö±Þ ÁýÇà·ü|ÁöÃâÀÜ¾×|´ã´çÀÚ
    SSOK_Expense_BudgetDashboard_AddTopSummary("SSOK_BudgetDashboardWorkPurposeLV", [160,160,235,130,95,95,105,95,95,95,95,170], 46, 216, 1406, 595)
    ...
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, Text, x32 y176 w1160 h26, ÃÊ°úÁýÇà »êÃâ³»¿ª°ú °°Àº ¼¼ºÎ»ç¾÷¡¤¼¼ºÎÇ×¸ñ¡¤¿ø°¡Åë°èºñ¸ñÀÇ ÀüÃ¼ ³»¿ªÀ» ÇÔ²² Ç¥½ÃÇÕ´Ï´Ù.
    Gui, SSOKBudgetDashboard:Add, Button, x1270 y172 w168 h30 gSSOK_Expense_BudgetDashboard_ExportOver, ÀÌ ÅÇ Excel ÃßÃâ
    Gui, SSOKBudgetDashboard:Add, ListView, x32 y240 w1434 h592 Grid -Multi NoSort gSSOK_Expense_BudgetDashboard_HeaderClick vSSOK_BudgetDashboardOverLV, »óÅÂ|Àç¿ø|¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|¿øÀÎÇàÀ§¾×|¿øÀÎÇàÀ§ ÁýÇà·ü|¿¹»êÀÜ¾×|Áö±Þ¾×|Áö±Þ ÁýÇà·ü|ÁöÃâÀÜ¾×
    SSOK_Expense_BudgetDashboard_AddTopSummary("SSOK_BudgetDashboardOverLV", [65,75,120,140,210,110,100,100,95,100,100,95,100], 32, 212, 1434, 620)
    ...
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x32 y175 w140 vSSOK_BudgetDashboardCostResource gSSOK_Expense_BudgetDashboard_CostRefresh, ÀüÃ¼||ÀÏ¹ÝÀç¿ø|¸ñÀûÀç¿ø
    SSOK_Expense_BudgetDashboard_AddChoiceButtons("SSOK_BudgetDashboardCostResource", "ÀüÃ¼|ÀÏ¹ÝÀç¿ø|¸ñÀûÀç¿ø", "ÀüÃ¼|ÀÏ¹ÝÀç¿ø|¸ñÀûÀç¿ø", 32, 173, 85, "resource")
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x305 y175 w180 vSSOK_BudgetDashboardCostSort gSSOK_Expense_BudgetDashboard_CostRefresh, Çö¾× Å«¼ø||ÀÌ¸§¼ø|¿øÀÎÇàÀ§ Å«¼ø|Áö±Þ¾× Å«¼ø|ÀÜ¾× Å«¼ø|ÁýÇà·ü ³ôÀº¼ø|ÁýÇà·ü ³·Àº¼ø
    Gui, SSOKBudgetDashboard:Add, Text, x500 y177 w620 h24, ÀÎ°Çºñ ¹× ¹ýÁ¤ºÎ´ã±ÝÀº ÀÎ°Çºñ·Î ÅëÇÕÇÕ´Ï´Ù.
    Gui, SSOKBudgetDashboard:Add, Button, x1270 y172 w168 h30 gSSOK_Expense_BudgetDashboard_ExportCost, ÀÌ ÅÇ Excel ÃßÃâ
    Gui, SSOKBudgetDashboard:Add, ListView, x32 y240 w1434 h592 Grid -Multi NoSort gSSOK_Expense_BudgetDashboard_HeaderClick vSSOK_BudgetDashboardCostLV, ¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|¿øÀÎÇàÀ§¾×|¿øÀÎÇàÀ§ ÁýÇà·ü|¿¹»êÀÜ¾×|Áö±Þ¾×|Áö±Þ ÁýÇà·ü|ÁöÃâÀÜ¾×|¸ñÀûÀç¿ø|ÀÏ¹ÝÀç¿ø
    SSOK_Expense_BudgetDashboard_AddTopSummary("SSOK_BudgetDashboardCostLV", [300,120,120,120,120,120,120,120,120,120], 32, 212, 1434, 620)
    Gui, SSOKBudgetDashboard:Add, Text, x32 y838 w1434 h54 vSSOK_BudgetDashboardCostStatus Hidden,

    Gui, SSOKBudgetDashboard:Tab
    Gui, SSOKBudgetDashboard:Show, w1498 h940 Center, ¿¹»ê DashBoard
    SSOK_Expense_BudgetDashboard_RefreshList()
    ...
    for _, resource in ["¸ñÀûÀç¿ø", "ÀÏ¹ÝÀç¿ø", "¾÷¹«ÃßÁøºñ"]
    {
        ...
        for _, row in rows
            current += row.current, commit += row.commit, paid += row.paid
        summary := resource . " ÀüÃ¼ ÇÕ°è  |  Çö¾× " . SSOK_Expense_FormatNumber(current)
    ...
    if (category = "±¹°í»ç¾÷" || category = "¸ñÀû»ç¾÷")
        for _, row in SSOK_BudgetDashboardParsed.rows
            if (row.resourceType = "¸ñÀûÀç¿ø" && row.purposeCategory = category)
            {
                ...
                businessBalances[row.business] += row.current - row.paid
            }
    styles := []
    Loop, % LV_GetCount()
    {
...
{
    global SSOK_BudgetDashboardRowColors, SSOK_BudgetDashboardFonts
...
SSOK_Expense_BudgetDashboard_SetTopSummary(listViewName, firstAmountColumn, current, commit, paid, purpose := "", general := "")
{
    ...
    values := [SSOK_Expense_FormatNumber(current), SSOK_Expense_FormatNumber(commit)
        , SSOK_Expense_BudgetDashboard_FormatRate(current, commit), SSOK_Expense_FormatNumber(current - commit)
        , SSOK_Expense_FormatNumber(paid), SSOK_Expense_BudgetDashboard_FormatRate(current, paid)
        , SSOK_Expense_FormatNumber(current - paid)]
    if (purpose != "" || general != "")
...
SSOK_Expense_BudgetDashboard_AddSplitKpi(x, y, w, title, totalText, generalText, purposeText, valueColor)
{
    ...
    valueW := w - 16
    Gui, SSOKBudgetDashboard:Font, s10 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, Text, x%valueX% y%valueY% w%valueW% h24 Center c%valueColor%, %totalText%
    detailY := y + 45
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    detailText := generalText . "`n" . purposeText
    Gui, SSOKBudgetDashboard:Add, Text, x%valueX% y%detailY% w%valueW% h30 Center c555555, %detailText%
}
...
SSOK_Expense_BudgetDashboard_RefreshList()
{
    ...
    for _, entry in entries
    {
        ...
        LV_Add("", entry.label
            ...
            , SSOK_Expense_FormatNumber(entry.paid)
            , SSOK_Expense_BudgetDashboard_FormatRate(entry.current, entry.paid)
            , SSOK_Expense_FormatNumber(entry.current - entry.paid)
    ...
    SSOK_Expense_BudgetDashboard_SetTopSummary("SSOK_BudgetDashboardMainLV", 3, shownCurrent, shownCommit, shownPaid, shownPurpose, shownGeneral)
    LV_ModifyCol(1, 150)
    LV_ModifyCol(2, 240)
    LV_ModifyCol(3, 105)
    LV_ModifyCol(4, 105)
...
SSOK_Expense_BudgetDashboard_BuildEntries(viewMode, resourceMode := "ÀüÃ¼", mergeLabor := false)
{
    global SSOK_BudgetDashboardParsed
    entries := []
    p := SSOK_BudgetDashboardParsed
    ...
    for _, row in p.rows
    {
        ...
        if !IsObject(groups[key])
            groups[key] := {label:label, detailBase:detailBase, detail:"", current:0, draft:0, commit:0, paid:0, settle:0, general:0, settleCommit:0, generalCommit:0, settlePaid:0, generalPaid:0, count:0}
...
SSOK_Expense_BudgetDashboard_FormatResourceManagementColumns()
{
    widths := [150,150,220,130,100,100,100,100,100,100,100,150]
    for col, width in widths
        LV_ModifyCol(col, (col >= 5 && col <= 11 ? "Right " : "") . width)
}
...
SSOK_Expense_BudgetDashboard_AppendManager(currentText, manager)
{
    ...
    for _, existing in StrSplit(currentText, ",")
    {
        if (Trim(existing) = manager)
            return currentText
    }
    return currentText . ", " . manager
}
...
SSOK_Expense_BudgetDashboard_CopyCurrentView()
{
    ...
    out := "±¸ºÐ`t»ó¼¼`tÇö¾×(A)`t¿øÀÎÇàÀ§¾×(B)`tÁýÇà·ü(B/A)`t¿¹»êÀÜ¾×(A-B)`tÁö±Þ¾×(C)`tÁýÇà·ü(C/A)`tÁöÃâÀÜ¾×(A-C)`t¸ñÀûÀç¿ø`tÀÏ¹ÝÀç¿ø`r`n"
    Loop, %rowCount%
    {
        rowNo := A_Index
        line := ""
