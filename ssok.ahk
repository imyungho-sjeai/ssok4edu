...
global SSOK_VERSION := "260924"
global SSOK_VERSION_DATE := "2026.9.24"
...
; Win + F4 : ÀÚÁÖ ¿©´Â ÆÄÀÏ Å°¿öµå °Ë»ö
; - Å°¿öµå 1~40Àº ssok.ini [F5QuickFiles]¿¡ ÀúÀå
...
SSOK_ShowQuickFileMenu:
    ...
    Gui, SSOKQuickFile:Add, Text, x20 y10 w640 h32 c005BAC Center, ÀÚÁÖ »ç¿ëÇÏ´Â ÆÄÀÏ ¹Ù·Î ¿­±â
    Gui, SSOKQuickFile:Font, s8 norm, Malgun Gothic
    Gui, SSOKQuickFile:Add, Button, x535 y14 w120 h24 gSSOK_QF_ShowAppRecentMenu, ÃÖ±Ù¹®¼­
    Gui, SSOKQuickFile:Add, Text, x20 y43 w640 h18 c555555 Center, ¹öÆ°Àº ÆÄÀÏ ¿­±â, ¼öÁ¤Àº °Ë»ö¾î ÀúÀå, Æú´õ¿­±â´Â Ã£Àº ÆÄÀÏÀÇ Æú´õ¸¦ ¿±´Ï´Ù.

    ; ´Ù¸¥ÆÄÀÏ °Ë»öÀº ÀÚÁÖ ¾²´Â ¹öÆ°º¸´Ù À§¿¡ µÓ´Ï´Ù. ±âÁ¸ °Ë»ö ¹æ½ÄÀº ±×´ë·Î À¯ÁöÇÕ´Ï´Ù.
    Gui, SSOKQuickFile:Font, s9 norm, Malgun Gothic
    Gui, SSOKQuickFile:Add, Text, x20 y70 w90 h22 c333333, ´Ù¸¥ÆÄÀÏ °Ë»ö
    Gui, SSOKQuickFile:Add, Edit, x112 y66 w390 h28 vSSOK_QF_CustomKeyword
    Gui, SSOKQuickFile:Add, Button, x510 y65 w145 h30 gSSOK_QF_CustomOpen Default, °Ë»ö

    ; ÀÚÁÖ »ç¿ëÇÏ´Â ÆÄÀÏ °Ë»ö¾î: 5¿­ x 8Çà = 40Ä­
    ; ºóÄ­(+)À» ´©¸£¸é ÇØ´ç Ä­ÀÇ °Ë»ö¾î¸¦ ¹Ù·Î ¼³Á¤ÇÒ ¼ö ÀÖ½À´Ï´Ù.
    Loop, 40
    {
        _qfIdx := A_Index
        _qfRow := Floor((_qfIdx - 1) / 5)
        _qfCol := Mod(_qfIdx - 1, 5)
        _qfX := 14 + (_qfCol * 132)
        _qfY := 104 + (_qfRow * 44)
        _qfSubY := _qfY + 27
        _qfNumX := _qfX + 3
        _qfEditX := _qfX + 40
        _qfFolderX := _qfX + 90
        _qfKeyword := SSOK_QF_Text%_qfIdx%
        _qfButtonText := Trim(_qfKeyword)
        if (_qfButtonText = "")
            _qfButtonText := "+"

        Gui, SSOKQuickFile:Font, s9 norm, Malgun Gothic
        Gui, SSOKQuickFile:Add, Button, x%_qfX% y%_qfY% w126 h27 vSSOK_QF_FileBtn%_qfIdx% gSSOK_QF_GridOpen, %_qfButtonText%
        Gui, SSOKQuickFile:Font, s7 norm, Malgun Gothic
        if (_qfIdx <= 10)
        {
            _qfNumLabel := (_qfIdx = 10) ? "0" : _qfIdx
            Gui, SSOKQuickFile:Add, Text, x%_qfNumX% y%_qfSubY% w30 h15 c777777, %_qfNumLabel%
        }
        ; Win+F5Ã³·³ º¸Á¶ ±â´ÉÀº ¹öÆ° Å×µÎ¸® ¾øÀÌ ÀÛÀº ±ÛÀÚ ¸µÅ©·Î Ç¥½ÃÇÕ´Ï´Ù.
        Gui, SSOKQuickFile:Font, s6 norm c888888, Malgun Gothic
        Gui, SSOKQuickFile:Add, Text, x%_qfEditX% y%_qfSubY% w25 h15 Right vSSOK_QF_EditBtn%_qfIdx% gSSOK_QF_GridEdit, ¼öÁ¤
        Gui, SSOKQuickFile:Add, Text, x%_qfFolderX% y%_qfSubY% w36 h15 Right vSSOK_QF_FolderBtn%_qfIdx% gSSOK_QF_GridFolder, Æú´õ¿­±â
    }

    ; ÇÏ´Ü SettingÀº ±âÁ¸ ±â´É/Ç×¸ñÀ» ±×´ë·Î À¯ÁöÇÕ´Ï´Ù.
    Gui, SSOKQuickFile:Font, s10 bold c005BAC, Malgun Gothic
    Gui, SSOKQuickFile:Add, Text, x20 y464 w120 h24, Setting
    Gui, SSOKQuickFile:Font, s8 norm, Malgun Gothic
    Gui, SSOKQuickFile:Add, Progress, x20 y490 w635 h1 BackgroundD5DDE8 cD5DDE8
    Gui, SSOKQuickFile:Add, Text, x20 y503 w80 h22 c333333, °Ë»ö Æú´õ ¼³Á¤
    Gui, SSOKQuickFile:Add, Checkbox, x105 y501 w68 h22 vSSOK_QF_SearchDesktop Checked%SSOK_QF_SearchDesktop% gSSOK_QF_SaveFolderOptions, ¹ÙÅÁÈ­¸é
    Gui, SSOKQuickFile:Add, Checkbox, x177 y501 w68 h22 vSSOK_QF_SearchDownloads Checked%SSOK_QF_SearchDownloads% gSSOK_QF_SaveFolderOptions, ´Ù¿î·Îµå
    Gui, SSOKQuickFile:Add, Checkbox, x249 y501 w48 h22 vSSOK_QF_SearchDocuments Checked%SSOK_QF_SearchDocuments% gSSOK_QF_SaveFolderOptions, ¹®¼­
    Gui, SSOKQuickFile:Add, Checkbox, x301 y501 w90 h22 vSSOK_QF_SearchDriveD Checked%SSOK_QF_SearchDriveD% gSSOK_QF_SaveDriveDOption, D: µå¶óÀÌºê
    Gui, SSOKQuickFile:Add, Checkbox, x393 y501 w90 h22 vSSOK_QF_SearchDriveE Checked%SSOK_QF_SearchDriveE% gSSOK_QF_SaveDriveEOption, E: µå¶óÀÌºê
    Gui, SSOKQuickFile:Add, Checkbox, x485 y501 w90 h22 vSSOK_QF_SearchDriveF Checked%SSOK_QF_SearchDriveF% gSSOK_QF_SaveDriveFOption, F: µå¶óÀÌºê
    ...
    Gui, SSOKQuickFile:Add, Progress, x20 y536 w635 h1 BackgroundD5DDE8 cD5DDE8
    Gui, SSOKQuickFile:Add, Text, x20 y549 w80 h22 c333333, °Ë»ö ÆÄÀÏ ¼³Á¤
    Gui, SSOKQuickFile:Add, Checkbox, x130 y547 w84 h22 vSSOK_QF_ExtHwp Checked%SSOK_QF_ExtHwp% gSSOK_QF_SaveFolderOptions, hwp(ÇÑ±Û)
    Gui, SSOKQuickFile:Add, Checkbox, x220 y547 w84 h22 vSSOK_QF_ExtXls Checked%SSOK_QF_ExtXls% gSSOK_QF_SaveFolderOptions, xls(¿¢¼¿)
    Gui, SSOKQuickFile:Add, Checkbox, x310 y547 w84 h22 vSSOK_QF_ExtDoc Checked%SSOK_QF_ExtDoc% gSSOK_QF_SaveFolderOptions, doc(¿öµå)
    Gui, SSOKQuickFile:Add, Checkbox, x400 y547 w96 h22 vSSOK_QF_ExtPpt Checked%SSOK_QF_ExtPpt% gSSOK_QF_SaveFolderOptions, PPT(½½¶óÀÌµå)
    Gui, SSOKQuickFile:Add, Checkbox, x504 y547 w50 h22 vSSOK_QF_ExtTxt Checked%SSOK_QF_ExtTxt% gSSOK_QF_SaveFolderOptions, txt
    Gui, SSOKQuickFile:Add, Checkbox, x560 y547 w50 h22 vSSOK_QF_ExtPdf Checked%SSOK_QF_ExtPdf% gSSOK_QF_SaveFolderOptions, pdf

    Gui, SSOKQuickFile:Add, Text, x20 y585 w635 h18 Right c999999, ÀúÀÛ±Ç: ¼¼Á¾Æ¯º°ÀÚÄ¡½Ã±³À°Ã» ÁÖ¹«°ü ÀÌ¸íÈ£
    SSOK_GetSidebarAttachedGuiPos(680, 615, SSOK_QF_WinX, SSOK_QF_WinY)
    Gui, SSOKQuickFile:Show, x%SSOK_QF_WinX% y%SSOK_QF_WinY% w680 h615, SSOK ÀÚÁÖ ¿©´Â ÆÄÀÏ
return
...
SSOK_QF_SetDefaults:
    ...
    Loop, 28
    {
        _qfDefaultIdx := A_Index + 12
        SSOK_QF_Text%_qfDefaultIdx% := ""
    }
return
...
SSOK_QF_LoadKeywords:
    ...
    {
        _qfLoadIdx := A_Index
        IniRead, _qfLoadValue, %SSOK_QF_Ini%, F5QuickFiles, Keyword%_qfLoadIdx%, __SSOK_MISSING__
        if (_qfLoadValue = "__SSOK_MISSING__" || _qfLoadValue = "ERROR" || SSOK_QF_IsBrokenKeyword(_qfLoadValue))
            continue
        ; °ú°Å 1¹ø ¾È³»¹®Àº »õ ±âº» °Ë»ö¾î 'ÁÖ¼Ò·Ï'À¸·Î ÇÑ ¹ø¸¸ ÀüÈ¯ÇÕ´Ï´Ù.
        if (_qfLoadIdx = 1 && _qfLoadValue = "¿©±â¿¡ ÀÚÁÖ»ç¿ëÇÏ´Â ÆÄÀÏ¸íÀ» °£·«È÷ ÀÔ·ÂÇÏ¼¼¿ä. ¹Ù·Î Ã£¾Æ ½ÇÇàÇÒ ¼ö ÀÖ½À´Ï´Ù")
            _qfLoadValue := "ÁÖ¼Ò·Ï"
        SSOK_QF_Text%_qfLoadIdx% := _qfLoadValue
    }
return
...
; Win+F4 40Ä­ °Ë»ö¾î ±âº»°ª/ÀúÀå º¸Á¶
SSOK_QF_DefaultKeyword(index)
{
    if (index = 1)
        return "ÁÖ¼Ò·Ï"
    if (index = 2)
...
SSOK_QF_PrepareKeywordsForSave()
{
    global
    SSOK_QF_Ini := SSOK_IniFile
    Loop, 40
    {
        _idx := A_Index
        _current := SSOK_QF_Text%_idx%
        if (_current != "")
            continue

        _existing := "__SSOK_MISSING__"
        if FileExist(SSOK_QF_Ini)
            IniRead, _existing, %SSOK_QF_Ini%, F5QuickFiles, Keyword%_idx%, __SSOK_MISSING__

        if (_existing != "__SSOK_MISSING__" && _existing != "ERROR" && !SSOK_QF_IsBrokenKeyword(_existing))
        {
            if (_idx = 1 && _existing = "¿©±â¿¡ ÀÚÁÖ»ç¿ëÇÏ´Â ÆÄÀÏ¸íÀ» °£·«È÷ ÀÔ·ÂÇÏ¼¼¿ä. ¹Ù·Î Ã£¾Æ ½ÇÇàÇÒ ¼ö ÀÖ½À´Ï´Ù")
                _existing := "ÁÖ¼Ò·Ï"
            SSOK_QF_Text%_idx% := _existing
        }
        else if (_idx <= 12)
            SSOK_QF_Text%_idx% := SSOK_QF_DefaultKeyword(_idx)
    }
}

SSOK_QF_BuildIniText()
{
    global
    SSOK_QF_PrepareKeywordsForSave()
    _text := "[F5QuickFiles]`r`n"
    Loop, 40
    {
        _idx := A_Index
        _text .= "Keyword" . _idx . "=" . SSOK_QF_Text%_idx% . "`r`n"
    }
    return _text
}
...
SSOK_SaveUnifiedIni()
{
    ...
    ; Win+F4 °Ë»ö¾î 1~40À» ±âÁ¸ INI °ª ¿ì¼±À¸·Î º¸Á¸ÇÕ´Ï´Ù.
    SSOK_QF_PrepareKeywordsForSave()

    ; ºü¸¥¸µÅ©´Â ¸ÞÀÎ¸Þ´º¿Í ¿¬µ¿µÇ´Â 1~5¹ø¸¸ °ü¸®ÇÕ´Ï´Ù.
    Loop, 5
    {
        _urlIdx := A_Index
    ...
    ; ºü¸¥¸µÅ© 1~5¸¸ ÀúÀå (¸ÞÀÎ¸Þ´º 5°³ ¹öÆ°°ú ¿¬µ¿)
    SaveText .= "`r`n"
    ...
    ; Win+F5 ÀÏ¹Ý ¾÷¹« ¸µÅ©: °íÁ¤ ½½·Ô ¹øÈ£º° ÀÌ¸§/URL¸¸ º¸Á¸
    SaveText .= SSOK_QU_BuildGeneralIniText()
...
; Win+F4 ¹öÆ°Çü °Ë»ö¾î ½½·Ô °øÅë Ã³¸®
SSOK_QF_GetGridIndex(controlName)
{
    if RegExMatch(controlName, "(\d+)$", _m)
        return _m1 + 0
    return 0
}

SSOK_QF_GetStoredKeyword(index)
{
    global
    return SSOK_QF_Text%index%
}

SSOK_QF_GridOpen:
    _qfGridIndex := SSOK_QF_GetGridIndex(A_GuiControl)
    if (_qfGridIndex < 1 || _qfGridIndex > 40)
        return
    _qfGridKeyword := SSOK_QF_GetStoredKeyword(_qfGridIndex)
    if (Trim(_qfGridKeyword) = "")
    {
        SSOK_QF_ShowSlotEditor(_qfGridIndex)
        return
    }
    Gui, SSOKQuickFile:Destroy
    SSOK_OpenFirstMatchingFile(_qfGridKeyword)
return
...
SSOK_QF_GridFolder:
    ...
    SSOK_QF_OpenFolderForKeyword(_qfGridKeyword)
return

SSOK_QF_ShowSlotEditor(index)
{
    global
    SSOK_QF_EditSlotIndex := index
    _keyword := SSOK_QF_Text%index%
    Gui, SSOKQFSlotEdit:Destroy
    Gui, SSOKQFSlotEdit:+OwnerSSOKQuickFile +AlwaysOnTop +ToolWindow -MinimizeBox
    Gui, SSOKQFSlotEdit:Color, F7FBFF
    Gui, SSOKQFSlotEdit:Font, s10 bold c005BAC, Malgun Gothic
    Gui, SSOKQFSlotEdit:Add, Text, x16 y14 w330 h24 Center, ÀÚÁÖ Ã£´Â ÆÄÀÏ °Ë»ö¾î ¼³Á¤
    Gui, SSOKQFSlotEdit:Font, s9 norm c222222, Malgun Gothic
    Gui, SSOKQFSlotEdit:Add, Text, x18 y53 w62 h22 Right, °Ë»ö¾î
    Gui, SSOKQFSlotEdit:Add, Edit, x88 y49 w250 h26 vSSOK_QF_SlotKeyword, %_keyword%
    Gui, SSOKQFSlotEdit:Font, s8 norm c666666, Malgun Gothic
    Gui, SSOKQFSlotEdit:Add, Text, x88 y80 w250 h18, ÆÄÀÏ¸í¿¡ Æ÷ÇÔµÈ ÂªÀº °Ë»ö¾î¸¦ ÀÔ·ÂÇÏ¼¼¿ä.
    Gui, SSOKQFSlotEdit:Font, s9 norm c222222, Malgun Gothic
    Gui, SSOKQFSlotEdit:Add, Button, x186 y108 w72 h27 gSSOK_QF_SlotEditorSave Default, ÀúÀå
    Gui, SSOKQFSlotEdit:Add, Button, x266 y108 w72 h27 gSSOKQFSlotEditGuiClose, Ãë¼Ò
    Gui, SSOKQFSlotEdit:Show, w360 h150, SSOK ÆÄÀÏ °Ë»ö¾î ¼³Á¤
}

SSOK_QF_SlotEditorSave:
    Gui, SSOKQFSlotEdit:Submit, NoHide
    _qfEditIndex := SSOK_QF_EditSlotIndex + 0
    if (_qfEditIndex < 1 || _qfEditIndex > 40)
        return
    ; 1~12ÀÇ ±âÁ¸ ±âº» °Ë»ö¾î´Â À¯ÁöÇÏµÇ »ç¿ëÀÚ°¡ »õ °ªÀ¸·Î ¹Ù²Ù´Â °ÍÀº Çã¿ëÇÕ´Ï´Ù.
    SSOK_QF_Text%_qfEditIndex% := Trim(SSOK_QF_SlotKeyword)
    SSOK_SaveUnifiedIni()
    Gui, SSOKQFSlotEdit:Destroy
    Gosub, SSOK_ShowQuickFileMenu
return

SSOKQFSlotEditGuiEscape:
SSOKQFSlotEditGuiClose:
    Gui, SSOKQFSlotEdit:Destroy
return
...
; =========================================================
; Win + F5 : ¾÷¹« »çÀÌÆ® ¹Ù·Î°¡±â
; - ºü¸¥ ¸µÅ©´Â 5°³·Î °íÁ¤ÇÏ¿© ¸ÞÀÎ¸Þ´º 5°³ ¹öÆ°°ú ¿¬°á
; - ±âÁ¸ 6~12¹ø ¸µÅ©´Â »ç¿ëÀÚ ¸µÅ© Ä­À¸·Î ±×´ë·Î º¸Á¸¡¤ÀÌ°ü
; - ºÐ·ù ¾øÀÌ 5¿­ ¹öÆ°ÆÇ¿¡¼­ ¹Ù·Î ¿­°í, °¢ Ä­¿¡¼­ ¹Ù·Î ¼öÁ¤
; =========================================================

SSOK_ShowQuickUrlMenu:
    Gosub, SSOK_QU_LoadUrls
    SSOK_QU_EnsureGeneralDataLoaded()
    SSOK_QU_LoadEduRegionPrefs()

    ; Win+F5¸¦ ´©¸£¸é ÆíÁý È­¸éÀÌ ¾Æ´Ï¶ó ¹Ù·Î Å¬¸¯ °¡´ÉÇÑ ´ë½Ãº¸µå¸¦ Ç¥½ÃÇÕ´Ï´Ù.
    ; °íÁ¤ »óÅÂ´Â »õ·Î ¿­ ¶§¸¶´Ù ±âÁ¸ µ¿ÀÛÃ³·³ ±âº»°ª "ÇØÁ¦"ÀÔ´Ï´Ù.
    SSOK_QU_PinMode := 0
    SSOK_QU_DashboardSearch := ""
    SSOK_QU_RenderDashboard("")
return

; -----------------------------------------------------------------
; Win+F5 ¸ÞÀÎ ¹Ù·Î°¡±â ÆÇ
; - Ã¹ ÁÙ 5°³´Â ¸ÞÀÎ¸Þ´º¿Í ¿¬°áµÈ ºü¸¥¸µÅ©
; - ¾Æ·¡ ºó Ä­Àº »ç¿ëÀÚ°¡ Á÷Á¢ Á¦¸ñ/URLÀ» Ãß°¡
; - ºÐ·ù/¸µÅ©°ü¸® È­¸é ¾øÀÌ °¢ Ä­¿¡¼­ ¹Ù·Î ¿­±â/¼öÁ¤
; -----------------------------------------------------------------
SSOK_QU_RenderDashboard(searchText := "")
{
    global

    ; ºÐ·ù/°ü¸® È­¸é ¾øÀÌ Win+F5 ÀÚÃ¼¿¡¼­ ¹Ù·Î ¿­°í ¹Ù·Î ¼öÁ¤ÇÏ´Â 5¿­ ¹öÆ°ÆÇÀÔ´Ï´Ù.
    SSOK_QU_EnsureGeneralDataLoaded()
    ...
    Gui, SSOKQuickUrl:Font, s15 bold c005BAC, Malgun Gothic
    Gui, SSOKQuickUrl:Add, Text, x85 y10 w425 h30 Center, ÀÚÁÖ°¡´Â »çÀÌÆ® URL ¹Ù·Î ¿­±â

    Gui, SSOKQuickUrl:Font, s9 bold c222222, Malgun Gothic
    Gui, SSOKQuickUrl:Add, Radio, x515 y13 w70 h24 vSSOK_QU_PinOnChoice gSSOK_QU_PinOn +0x1000 Group, °íÁ¤
    Gui, SSOKQuickUrl:Add, Radio, x590 y13 w70 h24 vSSOK_QU_PinOffChoice gSSOK_QU_PinOff +0x1000, ÇØÁ¦

    ; ¸ÞÀÎ È­¸é URL °Ë»ö°ú °°Àº µ¿ÀÛ: ÁÖ¼Ò¸é ¹Ù·Î ¿­°í, °Ë»ö¾î¸é À¥ °Ë»öÇÕ´Ï´Ù.
    Gui, SSOKQuickUrl:Font, s8 norm c555555, Malgun Gothic
    Gui, SSOKQuickUrl:Add, Text, x15 y49 w52 h24 +0x200, URL°Ë»ö
    Gui, SSOKQuickUrl:Font, s9 norm c222222, Malgun Gothic
    Gui, SSOKQuickUrl:Add, Edit, x70 y47 w500 h27 vSSOK_QU_GridSearch
    Gui, SSOKQuickUrl:Font, s8 bold c222222, Malgun Gothic
    Gui, SSOKQuickUrl:Add, Button, x578 y47 w87 h27 gSSOK_QU_GridSearchOpen Default, ¿­±â
    Gui, SSOKQuickUrl:Add, Progress, x15 y82 w650 h1 BackgroundD9E4EC cD9E4EC

    ; 1Çà 5Ä­Àº ºü¸¥¸µÅ©. ¾Æ·¡´Â ±âº» 35Ä­(7Çà)À» »ç¿ëÀÚ ¸µÅ©·Î »ç¿ëÇÕ´Ï´Ù.
    ; ±âÁ¸ ÀÏ¹Ý ¸µÅ©°¡ 35°³º¸´Ù ¸¹À¸¸é ±âÁ¸ µ¥ÀÌÅÍ°¡ ¼ûÁö ¾Êµµ·Ï ÇÊ¿äÇÑ Çà¸¸ ÀÚµ¿ Áõ°¡ÇÕ´Ï´Ù.
    _generalRows := 7
    if (SSOK_QU_LinkCount > 35)
        _generalRows := Ceil(SSOK_QU_LinkCount / 5.0)
    _totalRows := 1 + _generalRows
    _totalSlots := _totalRows * 5
    _gridStartY := 91
    ...
    Loop, %_totalSlots%
    {
        _slot := A_Index
        _col := Mod(_slot - 1, 5)
        _row := Floor((_slot - 1) / 5)
        _x := _marginX + (_col * (_cellW + _cellGap))
        _y := _gridStartY + (_row * _rowH)

        if (_slot <= 5)
        {
            _name := SSOK_QU_Name%_slot%
            _url := SSOK_QU_Url%_slot%
        }
        else
        {
            _generalIndex := _slot - 5
            if (_generalIndex <= SSOK_QU_LinkCount)
            {
                _name := SSOK_QU_LinkName%_generalIndex%
                _url := SSOK_QU_LinkUrl%_generalIndex%
            }
            else
            {
                _name := ""
        ...
        ; ºó Ä­µµ ½ÇÁ¦ ¹öÆ°À¸·Î µÓ´Ï´Ù. ºó ¹öÆ°À» ´©¸£¸é ¹Ù·Î ¼³Á¤Ã¢ÀÌ ¶å´Ï´Ù.
        _caption := Trim(_name)
        if (_caption = "")
            _caption := " "
        _btnCtrl := "SSOK_QU_GridBtn" . _slot
        _btnOpts := "x" . _x . " y" . _y . " w" . _cellW . " h31 v" . _btnCtrl . " gSSOK_QU_GridOpen"
        Gui, SSOKQuickUrl:Font, s8.5 bold c222222, Malgun Gothic
        Gui, SSOKQuickUrl:Add, Button, %_btnOpts%, %_caption%

        ; ºü¸¥¸µÅ© 5°³¸¸ ¿ÞÂÊ ¾Æ·¡¿¡ 1~5 ¼ýÀÚ¸¦ ¾ÆÁÖ ÀÛ°Ô Ç¥½ÃÇÕ´Ï´Ù.
        _subY := _y + 32
        Gui, SSOKQuickUrl:Font, s6 norm c888888, Malgun Gothic
        if (_slot <= 5)
        {
            _numOpts := "x" . (_x + 4) . " y" . _subY . " w18 h12 Center"
            Gui, SSOKQuickUrl:Add, Text, %_numOpts%, %_slot%
        }

        ; ¸ðµç Ä­ ¿À¸¥ÂÊ ¾Æ·¡ÀÇ ÀÛÀº '¼öÁ¤'À» ´­·¯ ¹Ù·Î Á¦¸ñ/URL¸¸ ÆíÁýÇÕ´Ï´Ù.
        _editCtrl := "SSOK_QU_GridEditLink" . _slot
        _editOpts := "x" . (_x + _cellW - 38) . " y" . _subY . " w36 h12 Right v" . _editCtrl . " gSSOK_QU_GridEdit"
        Gui, SSOKQuickUrl:Add, Text, %_editOpts%, ¼öÁ¤
    }

    _gridBottom := _gridStartY + (_totalRows * _rowH)

    ; ---------------------------------------------------------------
    ; ÇÏ´Ü ±³À°Ã» ½Ã½ºÅÛ ¹Ù·Î°¡±â - ±âÁ¸ ±â´É ±×´ë·Î À¯Áö
    ; ---------------------------------------------------------------
    ...
    _eduLineY := _gridBottom + 3
    _lineOpts := "x15 y" . _eduLineY . " w650 h1 BackgroundD9E4EC cD9E4EC"
    Gui, SSOKQuickUrl:Add, Progress, %_lineOpts%
    _eduHeadY := _eduLineY + 8
    Gui, SSOKQuickUrl:Font, s8.5 bold c005BAC, Malgun Gothic
    _headOpts := "x15 y" . _eduHeadY . " w230 h18"
    Gui, SSOKQuickUrl:Add, Text, %_headOpts%, ±³À°Ã» ½Ã½ºÅÛ ¹Ù·Î°¡±â

    _eduBtnY := _eduHeadY + 21
    _eduNames := "¾÷¹«Æ÷ÅÐ|K¿¡µàÆÄÀÎ|³ªÀÌ½º|¾÷¹«Áö¿ø|EVPN"
    _eduHandlers := "SSOK_QU_EduOpenPortal|SSOK_QU_EduOpenKEdufine|SSOK_QU_EduOpenNeis|SSOK_QU_EduOpenSupport|SSOK_QU_EduOpenEVPN"
    _eduVars := "SSOK_QU_EduRegionPortal|SSOK_QU_EduRegionKEdufine|SSOK_QU_EduRegionNeis|SSOK_QU_EduRegionSupport|SSOK_QU_EduRegionEVPN"
    StringSplit, _hArr, _eduHandlers, |
    StringSplit, _vArr, _eduVars, |
    Loop, Parse, _eduNames, |
    {
        _i := A_Index
        _name := A_LoopField
        _x := _marginX + ((_i - 1) * (_cellW + _cellGap))
        _handler := _hArr%_i%
        _varName := _vArr%_i%
        _opts := "x" . _x . " y" . _eduBtnY . " w" . _cellW . " h25 g" . _handler
        Gui, SSOKQuickUrl:Font, s7.5 bold c222222, Malgun Gothic
        Gui, SSOKQuickUrl:Add, Button, %_opts%, %_name%

        _list := (_i = 4 ? SSOK_QU_EduRegionListSupport : SSOK_QU_EduRegionListAll)
        _ddlY := _eduBtnY + 27
        _opts := "x" . _x . " y" . _ddlY . " w" . _cellW . " r12 v" . _varName
        Gui, SSOKQuickUrl:Font, s7 norm c222222, Malgun Gothic
        Gui, SSOKQuickUrl:Add, DropDownList, %_opts%, %_list%
        if (_i = 1)
            _selected := SSOK_QU_EduRegionPortal
        else if (_i = 2)
            _selected := SSOK_QU_EduRegionKEdufine
        else if (_i = 3)
            _selected := SSOK_QU_EduRegionNeis
        else if (_i = 4)
            _selected := SSOK_QU_EduRegionSupport
        else
            _selected := SSOK_QU_EduRegionEVPN
        GuiControl, SSOKQuickUrl:ChooseString, %_varName%, %_selected%
    }

    ; ---------------------------------------------------------------
    ; ÇÏ´Ü ¼¼Á¾ ±³À°ÇàÁ¤ ¾÷¹«Á¤º¸ ¹Ù·Î°¡±â - ±âÁ¸ 7°³ ±â´É ±×´ë·Î À¯Áö
    ; 7°³ ¹öÆ°À» ¸ðµÎ ÇÑ ÁÙ¿¡ ¹èÄ¡ÇÏ°í ±ä ÀÌ¸§¸¸ Âª°Ô Ç¥½ÃÇÕ´Ï´Ù.
    ; ---------------------------------------------------------------
    _infoHeadY := _eduBtnY + 58
    Gui, SSOKQuickUrl:Font, s8.5 bold c005BAC, Malgun Gothic
    _headOpts := "x15 y" . _infoHeadY . " w280 h18"
    Gui, SSOKQuickUrl:Add, Text, %_headOpts%, ¼¼Á¾ ±³À°ÇàÁ¤ ¾÷¹«Á¤º¸ ¹Ù·Î°¡±â

    _legacyNames := "°ü³»ÇÐ±³|ÇÐ±³¾Ë¸®¹Ì1|ÇÐ±³¾Ë¸®¹Ì2|ÀçÁ¤¾Ë¸®¹Ì|±³À°Ã»¾÷¹«´ã´ç|ÇÐ±³Áö¿øº»ºÎ|°è¾àÁ¤º¸"
    _legacyHandlers := "SSOK_QU_OpenSchoolSearchFromMenu|SSOK_QU_OpenSchoolInfoFromMenu|SSOK_QU_OpenSchoolInfo2FromMenu|SSOK_QU_OpenLocalFinanceInfoFromMenu|SSOK_QU_OpenEduOfficeSearchFromMenu|SSOK_QU_OpenSchoolSupportInfoFromMenu|SSOK_QU_OpenContractInfoFromMenu"
    StringSplit, _lhArr, _legacyHandlers, |
    _infoBtnW := 89
    _infoGap := 4
    _infoY1 := _infoHeadY + 21
    Loop, Parse, _legacyNames, |
    {
        _i := A_Index
        _name := A_LoopField
        _handler := _lhArr%_i%
        _x := 15 + ((_i - 1) * (_infoBtnW + _infoGap))
        _opts := "x" . _x . " y" . _infoY1 . " w" . _infoBtnW . " h27 g" . _handler
        Gui, SSOKQuickUrl:Font, s6.5 norm c222222, Malgun Gothic
        Gui, SSOKQuickUrl:Add, Button, %_opts%, %_name%
    }

    _footerY := _infoY1 + 38
    Gui, SSOKQuickUrl:Font, s7 norm c999999, Malgun Gothic
    _footOpts := "x330 y" . _footerY . " w335 h18 Right"
    Gui, SSOKQuickUrl:Add, Text, %_footOpts%, ÀúÀÛ±Ç: ¼¼Á¾Æ¯º°ÀÚÄ¡½Ã±³À°Ã» ÁÖ¹«°ü ÀÌ¸íÈ£

    _winH := _footerY + 31
    SysGet, _wa, MonitorWorkArea
    _maxH := _waBottom - _waTop - 24
    if (_winH > _maxH)
        _winH := _maxH

    SSOK_GetSidebarAttachedGuiPos(_winW, _winH, SSOK_QU_WinX, SSOK_QU_WinY)
    Gui, SSOKQuickUrl:Show, x%SSOK_QU_WinX% y%SSOK_QU_WinY% w%_winW% h%_winH%, SSOK ÀÚÁÖ°¡´Â »çÀÌÆ®
    Gosub, SSOK_QU_UpdatePinButtons
}

SSOK_QU_GridSearchOpen:
    Gui, SSOKQuickUrl:Submit, NoHide
    _query := Trim(SSOK_QU_GridSearch)
    if (_query = "")
        return
    if (!SSOK_QU_PinMode)
        Gui, SSOKQuickUrl:Destroy
    SSOK_QU_OpenSmartSearch(_query)
return

SSOK_QU_GridOpen:
    if RegExMatch(A_GuiControl, "(\d+)$", _m)
        SSOK_QU_OpenGridSlot(_m1)
return

SSOK_QU_GridEdit:
    if RegExMatch(A_GuiControl, "(\d+)$", _m)
        SSOK_QU_ShowGridEditor(_m1)
return

SSOK_QU_OpenGridSlot(index)
{
    global
    index += 0
    if (index < 1)
    ...
    if (index <= 5)
    {
        _url := SSOK_QU_Url%index%
    }
    else
    {
        _generalIndex := index - 5
        if (_generalIndex > SSOK_QU_LinkCount)
        {
            SSOK_QU_ShowGridEditor(index)
            return
        }
        _url := SSOK_QU_LinkUrl%_generalIndex%
    }

    _url := SSOK_QU_NormalizeUrl(_url)
    if (_url = "")
    {
        SSOK_QU_ShowGridEditor(index)
        return
...
SSOK_QU_ShowGridEditor(index)
{
    ...
    SSOK_QU_GridEditIndex := index
    if (index <= 5)
    {
        _name := SSOK_QU_Name%index%
        _url := SSOK_QU_Url%index%
        _title := "ºü¸¥¸µÅ© " . index . " ¼öÁ¤"
    }
    else
    {
        _generalIndex := index - 5
        ...
        _title := (Trim(_name) = "" && Trim(_url) = "") ? "¹Ù·Î°¡±â Ãß°¡" : "¹Ù·Î°¡±â ¼öÁ¤"
    }

    Gui, SSOKUrlGridEdit:Destroy
    Gui, SSOKUrlGridEdit:+AlwaysOnTop +ToolWindow -MinimizeBox +LabelSSOKUrlGridEditGui
    Gui, SSOKUrlGridEdit:Color, F7FBFF
    Gui, SSOKUrlGridEdit:Font, s10 bold c005BAC, Malgun Gothic
    Gui, SSOKUrlGridEdit:Add, Text, x18 y14 w414 h24 Center, %_title%
    Gui, SSOKUrlGridEdit:Font, s8.5 norm c222222, Malgun Gothic
    Gui, SSOKUrlGridEdit:Add, Text, x18 y51 w70 h22 Right, ¹öÆ° Á¦¸ñ
    Gui, SSOKUrlGridEdit:Add, Edit, x96 y47 w336 h25 vSSOK_QU_GridEditName, %_name%
    Gui, SSOKUrlGridEdit:Add, Text, x18 y85 w70 h22 Right, URL
    Gui, SSOKUrlGridEdit:Add, Edit, x96 y81 w336 h25 vSSOK_QU_GridEditUrl, %_url%
    Gui, SSOKUrlGridEdit:Add, Button, x276 y121 w74 h28 gSSOK_QU_GridEditorSave Default, ÀúÀå
    Gui, SSOKUrlGridEdit:Add, Button, x358 y121 w74 h28 gSSOKUrlGridEditGuiClose, Ãë¼Ò
    Gui, SSOKUrlGridEdit:Show, w450 h164, SSOK ¹Ù·Î°¡±â ¼³Á¤
}

SSOK_QU_GridEditorSave:
    Gui, SSOKUrlGridEdit:Submit, NoHide
    _slot := SSOK_QU_GridEditIndex + 0
    _name := Trim(SSOK_QU_GridEditName)
    _url := Trim(SSOK_QU_GridEditUrl)

    if (_slot <= 5)
    {
        if (_name = "")
            _name := "»çÀÌÆ®" . _slot
        SSOK_QU_Name%_slot% := _name
        SSOK_QU_Url%_slot% := _url
        SSOK_QU_SaveActive := 1
        SSOK_SaveUnifiedIni()
        SSOK_QU_SaveActive := 0
        SSOK_QU_RefreshSidebarQuickLabels()
    }
    else
    {
        _generalIndex := _slot - 5

        ; »ç¿ëÀÚ°¡ ´©¸¥ ºó Ä­ ¹øÈ£ ÀÚÃ¼¿¡ ÀúÀåÇÕ´Ï´Ù.
        ; ¾ÕÂÊ¿¡ ºó Ä­ÀÌ ÀÖ¾îµµ ¸¶Áö¸· ¸µÅ© µÚ·Î ºÙÀÌÁö ¾Ê½À´Ï´Ù.
        if (_generalIndex > SSOK_QU_LinkCount && !(_name = "" && _url = ""))
        {
            _oldCount := SSOK_QU_LinkCount
            SSOK_QU_LinkCount := _generalIndex
            _fillCount := SSOK_QU_LinkCount - _oldCount
            Loop, %_fillCount%
            {
                _fillIndex := _oldCount + A_Index
                if (_fillIndex = _generalIndex)
                    continue
                SSOK_QU_LinkName%_fillIndex% := ""
                SSOK_QU_LinkUrl%_fillIndex% := ""
            }
        }
        ...
        if (_generalIndex <= SSOK_QU_LinkCount)
        {
            if (_name = "" && _url = "")
            {
                ; ºñ¿î Ä­Àº ±×´ë·Î ºó Ä­À¸·Î ³²±â°í µÚÀÇ ¸µÅ©¸¦ ´ç±âÁö ¾Ê½À´Ï´Ù.
                SSOK_QU_LinkName%_generalIndex% := ""
                SSOK_QU_LinkUrl%_generalIndex% := ""
            }
            else
            {
                if (_name = "")
                    _name := "»çÀÌÆ®" . _generalIndex
                SSOK_QU_LinkName%_generalIndex% := _name
                SSOK_QU_LinkUrl%_generalIndex% := _url
            }
            SSOK_QU_SaveGeneralNow()
        }
    }
...
SSOKUrlGridEditGuiEscape:
SSOKUrlGridEditGuiClose:
    Gui, SSOKUrlGridEdit:Destroy
return

SSOK_QU_LinkMatchesSearch(linkIndex, searchText)
{
    global
    _search := Trim(searchText)
    if (_search = "")
        return true
    StringLower, _searchLow, _search
    _hay := SSOK_QU_LinkName%linkIndex% . " " . SSOK_QU_LinkUrl%linkIndex%
    StringLower, _hayLow, _hay
    return InStr(_hayLow, _searchLow) ? true : false
}

SSOK_QU_CountCategoryVisibleLinks(category, searchText)
{
    global
    _count := 0
    Loop, %SSOK_QU_LinkCount%
    {
        _i := A_Index
        if (SSOK_QU_LinkCategory%_i% != category)
            continue
        if (!SSOK_QU_LinkMatchesSearch(_i, searchText))
            continue
        _count++
    }
    return _count
...
SSOK_QU_DashboardDoSearch:
    Gui, SSOKQuickUrl:Submit, NoHide
    SSOK_QU_RenderDashboard(SSOK_QU_DashboardSearch)
return

SSOK_QU_DashboardResetSearch:
    SSOK_QU_DashboardSearch := ""
    SSOK_QU_RenderDashboard("")
return

SSOK_QU_DashboardOpenQuick:
    if RegExMatch(A_GuiControl, "(\d+)$", _m)
        SSOK_QU_OpenQuickDashboard(_m1)
return

SSOK_QU_DashboardOpenLink:
    if RegExMatch(A_GuiControl, "(\d+)$", _m)
        SSOK_QU_OpenGeneralDashboard(_m1)
return

SSOK_QU_OpenQuickDashboard(index)
{
    global
    index += 0
    if (index < 1 || index > 5)
    ...
    if (_url = "")
    {
        MsgBox, 48, SSOK ¾È³», ÁÖ¼Ò°¡ ºñ¾î ÀÖ½À´Ï´Ù.`n`n[¸µÅ© °ü¸®]¿¡¼­ ÁÖ¼Ò¸¦ ¼³Á¤ÇØ ÁÖ¼¼¿ä.
        return
    }
    if (!SSOK_QU_PinMode)
        Gui, SSOKQuickUrl:Destroy
...
SSOK_QU_OpenGeneralDashboard(index)
{
    global
    index += 0
    if (index < 1 || index > SSOK_QU_LinkCount)
        return
    _url := SSOK_QU_LinkUrl%index%
    _url := SSOK_QU_NormalizeUrl(_url)
    if (_url = "")
    {
        MsgBox, 48, SSOK ¾È³», ¸µÅ© ÁÖ¼Ò°¡ ºñ¾î ÀÖ½À´Ï´Ù.`n`n[¸µÅ© °ü¸®]¿¡¼­ ÁÖ¼Ò¸¦ ¼³Á¤ÇØ ÁÖ¼¼¿ä.
        return
    }
    if (!SSOK_QU_PinMode)
        Gui, SSOKQuickUrl:Destroy
...
; -----------------------------------------------------------------
; °ü¸® È­¸é - ¼³Á¤ÇÒ ¶§¸¸ ÇÑ ´Ü°è µé¾î°©´Ï´Ù.
; Æò»ó½Ã Win+F5 »ç¿ë È­¸é¿¡´Â ÆíÁýÄ­/¸ñ·Ï ¼±ÅÃ ´Ü°è¸¦ ³ëÃâÇÏÁö ¾Ê½À´Ï´Ù.
; -----------------------------------------------------------------
SSOK_QU_ShowManage:
    SSOK_QU_ShowManageGui()
return

SSOK_QU_BackDashboard:
    SSOK_QU_RenderDashboard(SSOK_QU_DashboardSearch)
return
...
SSOK_QU_ShowManageGui()
{
    ...
    Gui, SSOKQuickUrl:Font, s8.5 norm c222222, Malgun Gothic
    Gui, SSOKQuickUrl:Add, Button, x20 y13 w115 h27 gSSOK_QU_BackDashboard, ¹Ù·Î°¡±â È­¸é
    Gui, SSOKQuickUrl:Font, s15 bold c005BAC, Malgun Gothic
    Gui, SSOKQuickUrl:Add, Text, x155 y12 w490 h30 Center, ¾÷¹« »çÀÌÆ® ¸µÅ© °ü¸®

    Gui, SSOKQuickUrl:Font, s9 bold c222222, Malgun Gothic
    Gui, SSOKQuickUrl:Add, Radio, x715 y14 w72 h25 vSSOK_QU_PinOnChoice gSSOK_QU_PinOn +0x1000 Group, °íÁ¤
    Gui, SSOKQuickUrl:Add, Radio, x795 y14 w72 h25 vSSOK_QU_PinOffChoice gSSOK_QU_PinOff +0x1000, ÇØÁ¦

    ; ºü¸¥ ¸µÅ© 5°³ ÆíÁý
    Gui, SSOKQuickUrl:Font, s10 bold c005BAC, Malgun Gothic
    Gui, SSOKQuickUrl:Add, Text, x20 y50 w300 h22, ºü¸¥ ¸µÅ© 5°³ (¸ÞÀÎ¸Þ´º¿Í ¿¬°á)
    Gui, SSOKQuickUrl:Font, s8 norm c666666, Malgun Gothic
    Gui, SSOKQuickUrl:Add, Text, x405 y52 w460 h18 Right, ÀÌ 5°³¸¸ ¸ÞÀÎ¸Þ´º ¹Ù·Î°¡±â¿Í 1:1·Î ¿¬°áµË´Ï´Ù.

    Gui, SSOKQuickUrl:Font, s8 bold c555555, Malgun Gothic
    Gui, SSOKQuickUrl:Add, Text, x50 y75 w155 h18 Center, ÀÌ¸§
    Gui, SSOKQuickUrl:Add, Text, x210 y75 w555 h18 Center, ÁÖ¼Ò
    ...
    Loop, 5
    {
        ...
        _y := 96 + ((_i - 1) * 32)
        _ty := _y + 4
        _nameVar := "SSOK_QU_NameEdit" . _i
        _urlVar := "SSOK_QU_Edit" . _i
        _handler := "SSOK_QU_Open" . _i
        _nameVal := SSOK_QU_Name%_i%
        _urlVal := SSOK_QU_Url%_i%
        Gui, SSOKQuickUrl:Add, Text, x20 y%_ty% w24 h22 c005BAC Center, %_i%
        _opts := "x50 y" . _y . " w155 h25 v" . _nameVar
        Gui, SSOKQuickUrl:Add, Edit, %_opts%, %_nameVal%
        _opts := "x210 y" . _y . " w555 h25 v" . _urlVar
        Gui, SSOKQuickUrl:Add, Edit, %_opts%, %_urlVal%
        _opts := "x775 y" . _y . " w100 h25 g" . _handler
        Gui, SSOKQuickUrl:Add, Button, %_opts%, ÀúÀå&&¿­±â
    }
    Gui, SSOKQuickUrl:Font, s8 norm, Malgun Gothic
    Gui, SSOKQuickUrl:Add, Button, x735 y258 w140 h27 gSSOK_QU_SaveAll, ºü¸¥ ¸µÅ© ÀúÀå

    ; ÀÏ¹Ý ¸µÅ© °ü¸®
    Gui, SSOKQuickUrl:Add, Progress, x20 y297 w855 h1 BackgroundD9E4EC cD9E4EC
    Gui, SSOKQuickUrl:Font, s10 bold c005BAC, Malgun Gothic
    Gui, SSOKQuickUrl:Add, Text, x20 y307 w250 h22, ÀÏ¹Ý ¾÷¹« ¸µÅ© °ü¸®
    Gui, SSOKQuickUrl:Font, s8 norm c555555, Malgun Gothic
    Gui, SSOKQuickUrl:Add, Text, x535 y309 w50 h18 Right, °Ë»ö
    Gui, SSOKQuickUrl:Add, Edit, x590 y305 w285 h24 vSSOK_QU_GeneralSearch gSSOK_QU_GeneralSearchChanged

    Gui, SSOKQuickUrl:Font, s9 norm c222222, Malgun Gothic
    Gui, SSOKQuickUrl:Add, ListBox, x20 y337 w180 h205 vSSOK_QU_CategoryChoice gSSOK_QU_CategoryChanged AltSubmit Choose1
    Gui, SSOKQuickUrl:Add, ListView, x210 y337 w665 h205 vSSOK_QU_LinkLV gSSOK_QU_LinkListEvent Grid AltSubmit, ºÐ·ù|ÀÌ¸§|ÁÖ¼Ò

    Gui, SSOKQuickUrl:Font, s8 norm, Malgun Gothic
    Gui, SSOKQuickUrl:Add, Button, x20  y550 w56 h26 gSSOK_QU_CategoryAdd, +ºÐ·ù
    Gui, SSOKQuickUrl:Add, Button, x80  y550 w56 h26 gSSOK_QU_CategoryRename, ÀÌ¸§
    Gui, SSOKQuickUrl:Add, Button, x140 y550 w56 h26 gSSOK_QU_CategoryDelete, »èÁ¦
    Gui, SSOKQuickUrl:Add, Button, x20  y580 w86 h25 gSSOK_QU_CategoryUp, ºÐ·ù À§
    Gui, SSOKQuickUrl:Add, Button, x110 y580 w86 h25 gSSOK_QU_CategoryDown, ºÐ·ù ¾Æ·¡

    Gui, SSOKQuickUrl:Add, Button, x210 y550 w72 h26 gSSOK_QU_LinkAdd, +¸µÅ©
    Gui, SSOKQuickUrl:Add, Button, x286 y550 w72 h26 gSSOK_QU_LinkEdit, ¼öÁ¤
    Gui, SSOKQuickUrl:Add, Button, x362 y550 w72 h26 gSSOK_QU_LinkDelete, »èÁ¦
    Gui, SSOKQuickUrl:Add, Button, x438 y550 w72 h26 gSSOK_QU_LinkUp, À§
    Gui, SSOKQuickUrl:Add, Button, x514 y550 w72 h26 gSSOK_QU_LinkDown, ¾Æ·¡
    Gui, SSOKQuickUrl:Add, Button, x790 y550 w85 h26 gSSOK_QU_LinkOpen, ¿­±â
    Gui, SSOKQuickUrl:Font, s7 norm c777777, Malgun Gothic
    Gui, SSOKQuickUrl:Add, Text, x210 y582 w665 h20, ±âÁ¸ ¸µÅ©´Â »èÁ¦ÇÏÁö ¾Ê°í À¯ÁöÇÏ¸ç, ¿©±â¼­ ºÐ·ù¿Í ¼ø¼­¸¸ ÀÚÀ¯·Ó°Ô °ü¸®ÇÒ ¼ö ÀÖ½À´Ï´Ù.

    SSOK_QU_RefreshCategoryGui("ÀüÃ¼")
    SSOK_GetSidebarAttachedGuiPos(900, 620, SSOK_QU_WinX, SSOK_QU_WinY)
    Gui, SSOKQuickUrl:Show, x%SSOK_QU_WinX% y%SSOK_QU_WinY% w900 h620, SSOK ¸µÅ© °ü¸®
    Gosub, SSOK_QU_UpdatePinButtons
}
...
SSOK_QU_UpdatePinButtons:
    if (SSOK_QU_PinMode)
    {
        GuiControl, SSOKQuickUrl:, SSOK_QU_PinOnChoice, 1
        GuiControl, SSOKQuickUrl:, SSOK_QU_PinOffChoice, 0
        GuiControl, SSOKQuickUrl:, SSOK_QU_PinOnChoice, ¡Ü °íÁ¤
        GuiControl, SSOKQuickUrl:, SSOK_QU_PinOffChoice, ÇØÁ¦
    }
    else
    {
        GuiControl, SSOKQuickUrl:, SSOK_QU_PinOnChoice, 0
        GuiControl, SSOKQuickUrl:, SSOK_QU_PinOffChoice, 1
        GuiControl, SSOKQuickUrl:, SSOK_QU_PinOnChoice, °íÁ¤
        GuiControl, SSOKQuickUrl:, SSOK_QU_PinOffChoice, ¡Ü ÇØÁ¦
    }
return
...
SSOK_QU_CategoryChanged:
    SSOK_QU_RefreshLinkList()
return

SSOK_QU_GeneralSearchChanged:
    SSOK_QU_RefreshLinkList()
return

SSOK_QU_LinkListEvent:
    if (A_GuiEvent = "DoubleClick" && A_EventInfo > 0)
        SSOK_QU_OpenSelectedGeneralLink()
return

SSOK_QU_CategoryAdd:
    InputBox, _newCategory, SSOK ´ëºÐ·ù Ãß°¡, »õ ´ëºÐ·ù ÀÌ¸§À» ÀÔ·ÂÇÏ¼¼¿ä.
    if ErrorLevel
        return
    _newCategory := Trim(_newCategory)
    if (!SSOK_QU_IsValidCategoryName(_newCategory))
    {
        MsgBox, 48, SSOK ¾È³», ºÐ·ù ÀÌ¸§¿¡´Â | ±âÈ£³ª ÁÙ¹Ù²ÞÀ» »ç¿ëÇÒ ¼ö ¾ø½À´Ï´Ù.
        return
    }
    if (SSOK_QU_CategoryExists(_newCategory))
    {
        MsgBox, 48, SSOK ¾È³», °°Àº ÀÌ¸§ÀÇ ºÐ·ù°¡ ÀÌ¹Ì ÀÖ½À´Ï´Ù.
        return
    }
    SSOK_QU_AddCategoryValue(_newCategory)
    SSOK_QU_SaveGeneralNow()
    SSOK_QU_RefreshCategoryGui(_newCategory)
return

SSOK_QU_CategoryRename:
    _catIndex := SSOK_QU_GetSelectedCategoryIndex()
    if (_catIndex < 1)
    {
        MsgBox, 48, SSOK ¾È³», ÀÌ¸§À» ¹Ù²Ü ´ëºÐ·ù¸¦ ¼±ÅÃÇØ ÁÖ¼¼¿ä.
        return
    }
    _oldCategory := SSOK_QU_Category%_catIndex%
    InputBox, _newCategory, SSOK ´ëºÐ·ù ÀÌ¸§ º¯°æ, ÇöÀç ºÐ·ù: %_oldCategory%`n`n»õ ÀÌ¸§À» ÀÔ·ÂÇÏ¼¼¿ä.
    if ErrorLevel
        return
    _newCategory := Trim(_newCategory)
    if (!SSOK_QU_IsValidCategoryName(_newCategory))
    ...
    _sameIndex := SSOK_QU_CategoryExists(_newCategory)
    if (_sameIndex && _sameIndex != _catIndex)
    {
        MsgBox, 48, SSOK ¾È³», °°Àº ÀÌ¸§ÀÇ ºÐ·ù°¡ ÀÌ¹Ì ÀÖ½À´Ï´Ù.
        return
    }
    SSOK_QU_Category%_catIndex% := _newCategory
    Loop, %SSOK_QU_LinkCount%
    {
        _i := A_Index
        if (SSOK_QU_LinkCategory%_i% = _oldCategory)
            SSOK_QU_LinkCategory%_i% := _newCategory
    }
    SSOK_QU_SaveGeneralNow()
    SSOK_QU_RefreshCategoryGui(_newCategory)
return

SSOK_QU_CategoryDelete:
    _catIndex := SSOK_QU_GetSelectedCategoryIndex()
    if (_catIndex < 1)
    {
        MsgBox, 48, SSOK ¾È³», »èÁ¦ÇÒ ´ëºÐ·ù¸¦ ¼±ÅÃÇØ ÁÖ¼¼¿ä.
        return
    }
    if (SSOK_QU_CategoryCount <= 1)
    {
        MsgBox, 48, SSOK ¾È³», ´ëºÐ·ù´Â ÃÖ¼Ò 1°³°¡ ÇÊ¿äÇÕ´Ï´Ù.
        return
    }
    _oldCategory := SSOK_QU_Category%_catIndex%
    if (_oldCategory = "±âÅ¸")
    {
        _fallback := ""
        Loop, %SSOK_QU_CategoryCount%
        {
            _i := A_Index
            if (_i != _catIndex)
            {
                _fallback := SSOK_QU_Category%_i%
                break
            }
        }
    }
    ...
    {
        _etcIndex := SSOK_QU_CategoryExists("±âÅ¸")
        if (!_etcIndex)
            SSOK_QU_AddCategoryValue("±âÅ¸")
        _fallback := "±âÅ¸"
    }
    MsgBox, 36, SSOK ´ëºÐ·ù »èÁ¦, [%_oldCategory%] ºÐ·ù¸¦ »èÁ¦ÇÒ±î¿ä?`n`nÀÌ ºÐ·ù¿¡ ÀÖ´ø ¸µÅ©´Â [%_fallback%] ºÐ·ù·Î ÀÌµ¿µÇ¾î »èÁ¦µÇÁö ¾Ê½À´Ï´Ù.
    IfMsgBox, No
        return
    Loop, %SSOK_QU_LinkCount%
    {
        _i := A_Index
        if (SSOK_QU_LinkCategory%_i% = _oldCategory)
            SSOK_QU_LinkCategory%_i% := _fallback
    }
    SSOK_QU_RemoveCategoryAt(_catIndex)
    SSOK_QU_SaveGeneralNow()
    SSOK_QU_RefreshCategoryGui(_fallback)
return
...
SSOK_QU_CategoryUp:
    ...
    if (_catIndex <= 1)
        return
    _name := SSOK_QU_Category%_catIndex%
    _prevIndex := _catIndex - 1
    _prev := SSOK_QU_Category%_prevIndex%
    SSOK_QU_Category%_prevIndex% := _name
    SSOK_QU_Category%_catIndex% := _prev
    SSOK_QU_SaveGeneralNow()
    SSOK_QU_RefreshCategoryGui(_name)
return

SSOK_QU_CategoryDown:
    _catIndex := SSOK_QU_GetSelectedCategoryIndex()
    if (_catIndex < 1 || _catIndex >= SSOK_QU_CategoryCount)
        return
    _name := SSOK_QU_Category%_catIndex%
    _nextIndex := _catIndex + 1
    _next := SSOK_QU_Category%_nextIndex%
    SSOK_QU_Category%_nextIndex% := _name
    SSOK_QU_Category%_catIndex% := _next
    SSOK_QU_SaveGeneralNow()
    SSOK_QU_RefreshCategoryGui(_name)
return

SSOK_QU_LinkAdd:
    SSOK_QU_ShowLinkEditor("add")
return

SSOK_QU_LinkEdit:
    _linkIndex := SSOK_QU_GetSelectedLinkIndex()
    if (_linkIndex < 1)
    {
        MsgBox, 48, SSOK ¾È³», ¼öÁ¤ÇÒ ¸µÅ©¸¦ ¼±ÅÃÇØ ÁÖ¼¼¿ä.
        return
    }
    SSOK_QU_ShowLinkEditor("edit", _linkIndex)
return
...
SSOK_QU_LinkDelete:
    ...
    if (_linkIndex < 1)
    {
        MsgBox, 48, SSOK ¾È³», »èÁ¦ÇÒ ¸µÅ©¸¦ ¼±ÅÃÇØ ÁÖ¼¼¿ä.
        return
    }
    _linkName := SSOK_QU_LinkName%_linkIndex%
    MsgBox, 36, SSOK ¸µÅ© »èÁ¦, [%_linkName%] ¸µÅ©¸¦ »èÁ¦ÇÒ±î¿ä?
    IfMsgBox, No
        return
    SSOK_QU_RemoveLinkAt(_linkIndex)
    SSOK_QU_SaveGeneralNow()
    SSOK_QU_RefreshLinkList()
return

SSOK_QU_LinkUp:
    SSOK_QU_MoveSelectedLink(-1)
return
...
SSOKUrlEditGuiEscape:
SSOKUrlEditGuiClose:
    Gui, SSOKUrlEdit:Destroy
return

SSOK_QU_LinkEditorSave:
    Gui, SSOKUrlEdit:Submit, NoHide
    _category := Trim(SSOK_QU_LinkEditCategory)
    _name := Trim(SSOK_QU_LinkEditName)
    _url := Trim(SSOK_QU_LinkEditUrl)
    if (_category = "" || !SSOK_QU_CategoryExists(_category))
    {
        MsgBox, 48, SSOK ¾È³», ´ëºÐ·ù¸¦ ¼±ÅÃÇØ ÁÖ¼¼¿ä.
        return
    }
    if (_name = "")
    {
        MsgBox, 48, SSOK ¾È³», ¸µÅ© ÀÌ¸§À» ÀÔ·ÂÇØ ÁÖ¼¼¿ä.
        return
    }
    if (_url = "")
    {
        MsgBox, 48, SSOK ¾È³», ¸µÅ© ÁÖ¼Ò¸¦ ÀÔ·ÂÇØ ÁÖ¼¼¿ä.
        return
    }
    if (SSOK_QU_LinkEditMode = "edit" && SSOK_QU_LinkEditIndex >= 1 && SSOK_QU_LinkEditIndex <= SSOK_QU_LinkCount)
    {
        _idx := SSOK_QU_LinkEditIndex
    }
    else
    {
        SSOK_QU_LinkCount++
        _idx := SSOK_QU_LinkCount
    }
    SSOK_QU_LinkCategory%_idx% := _category
    SSOK_QU_LinkName%_idx% := _name
    SSOK_QU_LinkUrl%_idx% := _url
    SSOK_QU_SaveGeneralNow()
    Gui, SSOKUrlEdit:Destroy
    SSOK_QU_RefreshCategoryGui(_category)
    SSOK_QU_SelectVisibleLinkByIndex(_idx)
return
...
SSOK_QU_LoadBuiltInGridDefaults()
{
    global
    ; ¹èÆ÷ ±âº»°ª: »ç¿ëÀÚ°¡ Á¦°øÇÑ 35Ä­ Win+F5 ¹èÄ¡¸¦ ±×´ë·Î ³»ÀåÇÕ´Ï´Ù.
    ; ´ëºÐ·ù´Â »ç¿ëÇÏÁö ¾ÊÀ¸¸ç °¢ ½½·ÔÀº Name/Url µÎ °ª¸¸ °¡Áý´Ï´Ù.
    SSOK_QU_LinkCount := 35
    Loop, %SSOK_QU_LinkCount%
    {
        _i := A_Index
        SSOK_QU_LinkName%_i% := ""
    ...
    SSOK_QU_LinkName1 := "¼¼Á¾±³À°Ã»"
    SSOK_QU_LinkUrl1 := "https://www.sje.go.kr/"
    SSOK_QU_LinkName2 := "±¹¹Î½Å¹®°í"
    SSOK_QU_LinkUrl2 := "https://www.epeople.go.kr:8055/index.npaid"
    SSOK_QU_LinkName3 := ""
    SSOK_QU_LinkUrl3 := ""
    SSOK_QU_LinkName4 := "ÇÑ±¹Àü·Â ON"
    SSOK_QU_LinkUrl4 := "https://online.kepco.co.kr/"
    SSOK_QU_LinkName5 := "±³À°½Ã¼³ÅëÇÕÁ¤º¸¸Á"
    SSOK_QU_LinkUrl5 := "https://work.keiis.kr/keiis/index.do"
    SSOK_QU_LinkName6 := "¹ý·ÉAI°Ë»ö"
    SSOK_QU_LinkUrl6 := "https://www.law.go.kr/LSW/ais/main.do"
    SSOK_QU_LinkName7 := "GAI ±³Çà AI Çãºê"
    SSOK_QU_LinkUrl7 := "https://gaihub.net/programs"
    SSOK_QU_LinkName8 := "³ªÀÌ½º´ë±¹¹Î¼­ºñ½º"
    SSOK_QU_LinkUrl8 := "https://www.neis.go.kr/"
    SSOK_QU_LinkName9 := "¼¼Á¾½Ã±³À°Ã»¿¬¼ö¿ø"
    SSOK_QU_LinkUrl9 := "https://www.sjti.kr/"
    SSOK_QU_LinkName10 := "ÇÐ±³¾ÈÀü°øÁ¦È¸"
    SSOK_QU_LinkUrl10 := "https://sejong.ssif.or.kr/sejong.do?method=main&target=main"
    SSOK_QU_LinkName11 := "¼¼Á¾½ÃÀÇÈ¸"
    SSOK_QU_LinkUrl11 := "https://council.sejong.go.kr/"
    SSOK_QU_LinkName12 := "ÇÐ±³¾Ë¸®¹Ì"
    SSOK_QU_LinkUrl12 := "https://www.schoolinfo.go.kr/Main.do"
    SSOK_QU_LinkName13 := ""
    SSOK_QU_LinkUrl13 := ""
    SSOK_QU_LinkName14 := ""
    SSOK_QU_LinkUrl14 := ""
    SSOK_QU_LinkName15 := "½ï(SSOK) ºí·Î±×"
    SSOK_QU_LinkUrl15 := "https://blog.naver.com/ssok4edu"
    SSOK_QU_LinkName16 := "±¹°íº¸Á¶±Ý"
    SSOK_QU_LinkUrl16 := "https://www.bojo.go.kr/bojo.do"
    SSOK_QU_LinkName17 := ""
    SSOK_QU_LinkUrl17 := ""
    SSOK_QU_LinkName18 := ""
    SSOK_QU_LinkUrl18 := ""
    SSOK_QU_LinkName19 := ""
    SSOK_QU_LinkUrl19 := ""
    SSOK_QU_LinkName20 := "¹üÁË°æ·ÂÈ¸º¸¼­"
    SSOK_QU_LinkUrl20 := "https://www.bojo.go.kr/bojo.do"
    SSOK_QU_LinkName21 := "Á¤º¸°ø°³Æ÷ÅÐ"
    SSOK_QU_LinkUrl21 := "https://www.open.go.kr/"
    SSOK_QU_LinkName22 := "±¹°¡¹ý·ÉÁ¤º¸"
    SSOK_QU_LinkUrl22 := "https://www.law.go.kr/"
    SSOK_QU_LinkName23 := ""
    SSOK_QU_LinkUrl23 := ""
    SSOK_QU_LinkName24 := "½Ãµµ ±³À°Åë°è"
    SSOK_QU_LinkUrl24 := "https://school.kedi.re.kr/index.do"
    SSOK_QU_LinkName25 := "¹Ù¸¥ÇÑ±Û(¸ÂÃã¹ý°Ë»ç)"
    SSOK_QU_LinkUrl25 := "https://nara-speller.co.kr/speller/"
    SSOK_QU_LinkName26 := "AI SEN ¼­¿ï Æ÷ÅÐ"
    SSOK_QU_LinkUrl26 := "https://aisen-store.vercel.app/"
    SSOK_QU_LinkName27 := ""
    SSOK_QU_LinkUrl27 := ""
    SSOK_QU_LinkName28 := ""
    SSOK_QU_LinkUrl28 := ""
    SSOK_QU_LinkName29 := "Ãæ±³ À§Å° Wiki"
    SSOK_QU_LinkUrl29 := "https://ckwiki.pages.dev/"
    SSOK_QU_LinkName30 := "GAI ±³Çà AI Çãºê"
    SSOK_QU_LinkUrl30 := "https://gaihub.net/programs"
    SSOK_QU_LinkName31 := "¼­¿ï ±³À°ÇàÁ¤Áö¿ø"
    SSOK_QU_LinkUrl31 := "https://baro.sen.go.kr/"
    SSOK_QU_LinkName32 := "ºÎ»ê ÇÐ±³Áö¿ø¼­ºñ½º"
    SSOK_QU_LinkUrl32 := "https://bsss.pen.go.kr/main.do"
    SSOK_QU_LinkName33 := "°æºÏ ÇÐ±³Áö¿ø"
    SSOK_QU_LinkUrl33 := "https://www.gbe.kr/edupia"
    SSOK_QU_LinkName34 := "Ãæ³² ÇÐ±³¾÷¹«ÀÚ·á"
    SSOK_QU_LinkUrl34 := "http://www.cne.go.kr/manual/main.do"
    SSOK_QU_LinkName35 := "¼¼Á¾ ÇÐ±³Áö¿øº»ºÎ"
    SSOK_QU_LinkUrl35 := "https://sssc.sje.go.kr/sssc/main.do"
}

SSOK_QU_EnsureGeneralDataLoaded()
{
    global
    if (SSOK_QU_GeneralLoaded)
        return

    SSOK_QU_GeneralLoaded := 1
    SSOK_QU_LinkCount := 0
    _linkCountRead := "__SSOK_MISSING__"

    if FileExist(SSOK_IniFile)
        IniRead, _linkCountRead, %SSOK_IniFile%, QuickUrlLinks, Count, __SSOK_MISSING__

    if (_linkCountRead != "__SSOK_MISSING__")
    {
        _linkCountRead += 0
        if (_linkCountRead > 0)
        {
            ; Win+F5 ÀÏ¹Ý ¸µÅ©´Â °íÁ¤ ½½·ÔÀÔ´Ï´Ù. ºó Ä­µµ °°Àº ¹øÈ£·Î À¯ÁöÇÕ´Ï´Ù.
            SSOK_QU_LinkCount := _linkCountRead
            Loop, %_linkCountRead%
            {
                _i := A_Index
                IniRead, _name, %SSOK_IniFile%, QuickUrlLinks, Name%_i%,
                IniRead, _url, %SSOK_IniFile%, QuickUrlLinks, Url%_i%,
                SSOK_QU_LinkName%_i% := Trim(_name)
                SSOK_QU_LinkUrl%_i% := Trim(_url)
            }
            return
        }
    }

    ; ÀúÀå°ªÀÌ ¾øÀ¸¸é ÇöÀç ¹èÆ÷ ±âº» 35Ä­À» »ç¿ëÇÕ´Ï´Ù.
    SSOK_QU_LoadBuiltInGridDefaults()
}

SSOK_QU_ImportLegacyLinks()
{
    global
    Loop, 7
    {
        _legacyIndex := A_Index + 5
        _legacyName := SSOK_QU_Name%_legacyIndex%
        _legacyUrl := SSOK_QU_Url%_legacyIndex%
        if (Trim(_legacyName) = "" && Trim(_legacyUrl) = "")
            continue
        if (SSOK_QU_LinkExistsExact(_legacyName, _legacyUrl))
            continue
        _legacyCategory := SSOK_QU_GetLegacyCategory(_legacyIndex)
        if (!SSOK_QU_CategoryExists(_legacyCategory))
            SSOK_QU_AddCategoryValue(_legacyCategory)
        SSOK_QU_LinkCount++
        _dst := SSOK_QU_LinkCount
        SSOK_QU_LinkCategory%_dst% := _legacyCategory
        SSOK_QU_LinkName%_dst% := _legacyName
        SSOK_QU_LinkUrl%_dst% := _legacyUrl
    }
}

SSOK_QU_LinkExistsExact(name, url)
{
    global
    Loop, %SSOK_QU_LinkCount%
    {
        _i := A_Index
        if (SSOK_QU_LinkName%_i% = name && SSOK_QU_LinkUrl%_i% = url)
            return true
    }
    return false
}

SSOK_QU_AddDefaultCategories()
{
    global
    _defaults := "¾÷¹«½Ã½ºÅÛ|È¸°è¡¤ÀçÁ¤|°è¾à¡¤±¸¸Å|ÀÎ»ç¡¤º¹¹«|½Ã¼³¡¤¾ÈÀü|¹ý¹«¡¤±ÔÁ¤|Á¤º¸¡¤Àü»ê|ÇÐ»ç¡¤ÇÐ»ý|¿¬¼ö¡¤ÀÚ·á|±âÅ¸"
    Loop, Parse, _defaults, |
        SSOK_QU_AddCategoryValue(A_LoopField)
}

SSOK_QU_AddCategoryValue(name)
{
    global
    name := Trim(name)
    if (name = "")
        return 0
    _exists := SSOK_QU_CategoryExists(name)
    if (_exists)
        return _exists
    SSOK_QU_CategoryCount++
    _idx := SSOK_QU_CategoryCount
    SSOK_QU_Category%_idx% := name
    return _idx
}

SSOK_QU_CategoryExists(name)
{
    global
    _target := Trim(name)
    Loop, %SSOK_QU_CategoryCount%
    {
        _i := A_Index
        if (SSOK_QU_Category%_i% = _target)
            return _i
    }
    return 0
}

SSOK_QU_GetLegacyCategory(index)
{
    if (index = 6)
        return "¹ý¹«¡¤±ÔÁ¤"
    if (index = 7)
...
SSOK_QU_BuildGeneralIniText()
{
    global
    SSOK_QU_EnsureGeneralDataLoaded()
    _text := "`r`n[QuickUrlLinks]`r`n"
    _text .= "Count=" . SSOK_QU_LinkCount . "`r`n"
    Loop, %SSOK_QU_LinkCount%
    {
        _i := A_Index
        _name := SSOK_QU_CleanIniValue(SSOK_QU_LinkName%_i%)
        _url := SSOK_QU_CleanIniValue(SSOK_QU_LinkUrl%_i%)
        _text .= "Name" . _i . "=" . _name . "`r`n"
        _text .= "Url" . _i . "=" . _url . "`r`n"
    }
    return _text
}
...
SSOK_QU_IsValidCategoryName(name)
{
    name := Trim(name)
    if (name = "")
        return false
    if InStr(name, "|")
        return false
    if InStr(name, "`r") || InStr(name, "`n")
        return false
    return true
}

SSOK_QU_BuildCategoryPipe(includeAll := true)
{
    global
    _list := includeAll ? "ÀüÃ¼" : ""
    Loop, %SSOK_QU_CategoryCount%
    {
        _i := A_Index
        _name := SSOK_QU_Category%_i%
        if (_list = "")
            _list := _name
        else
            _list .= "|" . _name
...
SSOK_QU_GetSelectedCategoryIndex()
{
    global
    GuiControlGet, _choice, SSOKQuickUrl:, SSOK_QU_CategoryChoice
    _choice += 0
    if (_choice <= 1)
        return 0
    _idx := _choice - 1
    if (_idx > SSOK_QU_CategoryCount)
        return 0
    return _idx
}

SSOK_QU_GetSelectedCategoryName()
{
    global
    _idx := SSOK_QU_GetSelectedCategoryIndex()
    if (_idx < 1)
        return ""
    return SSOK_QU_Category%_idx%
}

SSOK_QU_RefreshCategoryGui(selectName := "")
{
    global
    SSOK_QU_EnsureGeneralDataLoaded()
    if (selectName = "")
    {
        _current := SSOK_QU_GetSelectedCategoryName()
        selectName := (_current = "" ? "ÀüÃ¼" : _current)
    }
    _list := SSOK_QU_BuildCategoryPipe(true)
    GuiControl, SSOKQuickUrl:, SSOK_QU_CategoryChoice, |%_list%
    _choice := 1
    if (selectName != "" && selectName != "ÀüÃ¼")
    {
        _idx := SSOK_QU_CategoryExists(selectName)
        if (_idx)
            _choice := _idx + 1
    }
    GuiControl, SSOKQuickUrl:Choose, SSOK_QU_CategoryChoice, %_choice%
    SSOK_QU_RefreshLinkList()
}
...
{
    global
    _filterCategory := SSOK_QU_GetSelectedCategoryName()
    GuiControlGet, _search, SSOKQuickUrl:, SSOK_QU_GeneralSearch
    _search := Trim(_search)
    StringLower, _searchLow, _search

    Gui, SSOKQuickUrl:Default
    Gui, ListView, SSOK_QU_LinkLV
    LV_Delete()
    SSOK_QU_VisibleCount := 0
    Loop, %SSOK_QU_LinkCount%
    {
        _i := A_Index
        _cat := SSOK_QU_LinkCategory%_i%
        _name := SSOK_QU_LinkName%_i%
        _url := SSOK_QU_LinkUrl%_i%
        if (_filterCategory != "" && _cat != _filterCategory)
            continue
        if (_searchLow != "")
        {
            _hay := _cat . " " . _name . " " . _url
            StringLower, _hayLow, _hay
            if !InStr(_hayLow, _searchLow)
                continue
        }
        _row := LV_Add("", _cat, _name, _url)
        SSOK_QU_VisibleLink%_row% := _i
        SSOK_QU_VisibleCount := _row
    }
    LV_ModifyCol(1, 105)
    LV_ModifyCol(2, 175)
    LV_ModifyCol(3, 365)
...
SSOK_QU_GetSelectedLinkIndex()
{
    ...
    if (_row < 1)
        return 0
    _idx := SSOK_QU_VisibleLink%_row%
    return _idx + 0
}

SSOK_QU_SelectVisibleLinkByIndex(linkIndex)
{
    global
    Gui, SSOKQuickUrl:Default
    Gui, ListView, SSOK_QU_LinkLV
    Loop, %SSOK_QU_VisibleCount%
    {
        _row := A_Index
        if (SSOK_QU_VisibleLink%_row% = linkIndex)
        {
            LV_Modify(_row, "Select Focus Vis")
            return
        }
    }
}

SSOK_QU_ShowLinkEditor(mode, linkIndex := 0)
{
    global
    SSOK_QU_EnsureGeneralDataLoaded()
    _defaultCategory := SSOK_QU_GetSelectedCategoryName()
    if (_defaultCategory = "")
        _defaultCategory := SSOK_QU_Category1
    _name := ""
    _url := ""
    if (mode = "edit" && linkIndex >= 1 && linkIndex <= SSOK_QU_LinkCount)
    {
        _defaultCategory := SSOK_QU_LinkCategory%linkIndex%
        _name := SSOK_QU_LinkName%linkIndex%
        _url := SSOK_QU_LinkUrl%linkIndex%
    }
    SSOK_QU_LinkEditMode := mode
    SSOK_QU_LinkEditIndex := linkIndex
    _categoryList := SSOK_QU_BuildCategoryPipe(false)

    Gui, SSOKUrlEdit:Destroy
    Gui, SSOKUrlEdit:+OwnerSSOKQuickUrl +AlwaysOnTop +ToolWindow -MinimizeBox
    Gui, SSOKUrlEdit:Color, F7FBFF
    Gui, SSOKUrlEdit:Font, s10 bold c005BAC, Malgun Gothic
    Gui, SSOKUrlEdit:Add, Text, x18 y14 w420 h24 Center, ¾÷¹« ¸µÅ© ¼³Á¤
    Gui, SSOKUrlEdit:Font, s9 norm c222222, Malgun Gothic
    Gui, SSOKUrlEdit:Add, Text, x18 y52 w70 h22 Right, ´ëºÐ·ù
    Gui, SSOKUrlEdit:Add, DropDownList, x96 y48 w330 r10 vSSOK_QU_LinkEditCategory, %_categoryList%
    GuiControl, SSOKUrlEdit:ChooseString, SSOK_QU_LinkEditCategory, %_defaultCategory%
    Gui, SSOKUrlEdit:Add, Text, x18 y86 w70 h22 Right, ÀÌ¸§
    Gui, SSOKUrlEdit:Add, Edit, x96 y82 w330 h25 vSSOK_QU_LinkEditName, %_name%
    Gui, SSOKUrlEdit:Add, Text, x18 y120 w70 h22 Right, ÁÖ¼Ò
    Gui, SSOKUrlEdit:Add, Edit, x96 y116 w330 h25 vSSOK_QU_LinkEditUrl, %_url%
    Gui, SSOKUrlEdit:Add, Button, x276 y158 w72 h27 gSSOK_QU_LinkEditorSave Default, ÀúÀå
    Gui, SSOKUrlEdit:Add, Button, x354 y158 w72 h27 gSSOKUrlEditGuiClose, Ãë¼Ò
    Gui, SSOKUrlEdit:Show, w445 h200, SSOK ¸µÅ© ¼³Á¤
}

SSOK_QU_RemoveCategoryAt(index)
{
    global
    if (index < 1 || index > SSOK_QU_CategoryCount)
        return
    _moveCount := SSOK_QU_CategoryCount - index
    Loop, %_moveCount%
    {
        _dst := index + A_Index - 1
        _src := _dst + 1
        SSOK_QU_Category%_dst% := SSOK_QU_Category%_src%
    }
    SSOK_QU_Category%SSOK_QU_CategoryCount% := ""
    SSOK_QU_CategoryCount--
}

SSOK_QU_RemoveLinkAt(index)
{
    global
    if (index < 1 || index > SSOK_QU_LinkCount)
        return
    _moveCount := SSOK_QU_LinkCount - index
    Loop, %_moveCount%
    {
        _dst := index + A_Index - 1
        _src := _dst + 1
        SSOK_QU_LinkCategory%_dst% := SSOK_QU_LinkCategory%_src%
        SSOK_QU_LinkName%_dst% := SSOK_QU_LinkName%_src%
        SSOK_QU_LinkUrl%_dst% := SSOK_QU_LinkUrl%_src%
    }
    SSOK_QU_LinkCategory%SSOK_QU_LinkCount% := ""
    SSOK_QU_LinkName%SSOK_QU_LinkCount% := ""
    SSOK_QU_LinkUrl%SSOK_QU_LinkCount% := ""
    SSOK_QU_LinkCount--
}

SSOK_QU_MoveSelectedLink(direction)
{
    global
    _idx := SSOK_QU_GetSelectedLinkIndex()
    if (_idx < 1)
        return
    _cat := SSOK_QU_LinkCategory%_idx%
    _target := 0
    if (direction < 0)
    {
        _i := _idx - 1
        while (_i >= 1)
        {
            if (SSOK_QU_LinkCategory%_i% = _cat)
            {
                _target := _i
                break
            }
    ...
    _catTmp := SSOK_QU_LinkCategory%_idx%
    _nameTmp := SSOK_QU_LinkName%_idx%
    _urlTmp := SSOK_QU_LinkUrl%_idx%
    SSOK_QU_LinkCategory%_idx% := SSOK_QU_LinkCategory%_target%
    SSOK_QU_LinkName%_idx% := SSOK_QU_LinkName%_target%
    SSOK_QU_LinkUrl%_idx% := SSOK_QU_LinkUrl%_target%
    SSOK_QU_LinkCategory%_target% := _catTmp
    SSOK_QU_LinkName%_target% := _nameTmp
    SSOK_QU_LinkUrl%_target% := _urlTmp

    SSOK_QU_SaveGeneralNow()
    SSOK_QU_RefreshLinkList()
    SSOK_QU_SelectVisibleLinkByIndex(_target)
}
...
SSOK_QU_OpenSelectedGeneralLink()
{
    ...
    if (_idx < 1)
    {
        MsgBox, 48, SSOK ¾È³», ¿­ ¸µÅ©¸¦ ¼±ÅÃÇØ ÁÖ¼¼¿ä.
        return
    }
    _url := SSOK_QU_LinkUrl%_idx%
    _url := SSOK_QU_NormalizeUrl(_url)
    if (_url = "")
    {
        MsgBox, 48, SSOK ¾È³», ¸µÅ© ÁÖ¼Ò°¡ ºñ¾î ÀÖ½À´Ï´Ù.
        return
    }
    if (!SSOK_QU_PinMode)
        Gui, SSOKQuickUrl:Destroy
...
{
    global
    if (SSOK_SidebarHwnd = "" || !WinExist("ahk_id " . SSOK_SidebarHwnd))
        return
    WinGetPos,,, _w,, ahk_id %SSOK_SidebarHwnd%
    _chars := 1
    if (_w >= 170)
        _chars := 2
    if (_w >= 250)
    ...
    {
        _i := A_Index
        _name := SSOK_QU_Name%_i%
        _label := SSOK_Sidebar_FirstCharLabel(_name, _i, _chars)
        _ctrl := "SSOK_SidebarQU" . _i
        GuiControl, SSOKSide:, %_ctrl%, %_label%
    }
}
...
SSOK_QU_SaveAll:
    SSOK_QU_SaveAllFromGui()
    MsgBox, 64, SSOK ¾È³», ºü¸¥ ¸µÅ© 5°³¸¦ ÀúÀåÇß½À´Ï´Ù.`n`n¸ÞÀÎ¸Þ´ºÀÇ 5°³ URL ¹öÆ°¿¡µµ °°Àº °ªÀÌ Àû¿ëµË´Ï´Ù.
return
