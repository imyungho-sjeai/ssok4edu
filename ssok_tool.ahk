...
QI_ShowGui:
    ...
    Gui, QIQuick:Add, Button, x595 y493 w70 h28 gQI_Input11, ÀúÀå&&½ÇÇà
...
SSOK_ShowWorkToolsGui:
    ...
    SSOK_AdminCalMenuText := SSOK_AdminCalEnabled ? "¾÷¹«´Þ·Â [ON]" : "¾÷¹«´Þ·Â [OFF]"
    ...
    Gui, SSOKWorkTools:Add, Button, x8 y39  w%SSOK_WorkToolsButtonW% h25 gSSOK_WorkTools_Travel, ¿©ºñÁ¤»ê¼­
    Gui, SSOKWorkTools:Add, Button, x8 y70  w%SSOK_WorkToolsButtonW% h25 gSSOK_WorkTools_Calc, °è»ê±â
    Gui, SSOKWorkTools:Add, Button, x8 y101 w%SSOK_WorkToolsButtonW% h25 gSSOK_WorkTools_AutoClick, ¸¶¿ì½º ¸ÅÅ©·Î
    Gui, SSOKWorkTools:Add, Button, x8 y132 w%SSOK_WorkToolsButtonW% h25 gSSOK_WorkTools_Timer, ¾÷¹«¿ë Å¸ÀÌ¸Ó
    Gui, SSOKWorkTools:Add, Button, x8 y163 w%SSOK_WorkToolsButtonW% h25 gSSOK_WorkTools_CompanyInfo, ±¹¼¼Ã»¡¤Á¶´ÞÃ» ¾÷Ã¼Á¶È¸
    Gui, SSOKWorkTools:Add, Button, x8 y194 w%SSOK_WorkToolsButtonW% h25 gSSOK_WorkTools_Privacy, °³ÀÎÁ¤º¸ ¼û±â±â
    Gui, SSOKWorkTools:Add, Button, x8 y225 w%SSOK_WorkToolsNumW% h25 gSSOK_WorkTools_CommaToggle, ¼ýÀÚ-Ãµ¿ø
    Gui, SSOKWorkTools:Add, Button, x%SSOK_WorkToolsNumX2% y225 w%SSOK_WorkToolsNumW% h25 gSSOK_WorkTools_MoneyToggle, ¼ýÀÚ-±Ý¾×
    Gui, SSOKWorkTools:Add, Button, x8 y256 w%SSOK_WorkToolsButtonW% h25 gSSOK_WorkTools_CardCompare, ¹ýÀÎÄ«µå³»¿ªºñ±³
    Gui, SSOKWorkTools:Add, Button, x8 y287 w%SSOK_WorkToolsButtonW% h25 gSSOK_WorkTools_WorkPublic, ¾÷¹«ÃßÁøºñ°ø°³
    Gui, SSOKWorkTools:Add, Button, x8 y318 w%SSOK_WorkToolsButtonW% h25 gSSOK_WorkTools_SchoolSearch, ÇÐ±³°Ë»ö (Àü±¹)
    Gui, SSOKWorkTools:Add, Button, x8 y349 w%SSOK_WorkToolsButtonW% h25 gSSOK_WorkTools_SchoolSearchNational, ÇÐ±³°Ë»ö (¼¼Á¾)
    Gui, SSOKWorkTools:Add, Button, x8 y380 w%SSOK_WorkToolsButtonW% h25 gSSOK_WorkTools_LocalFinanceInfo, Áö¹æ±³À°ÀçÁ¤Á¤º¸
    Gui, SSOKWorkTools:Add, Button, x8 y411 w%SSOK_WorkToolsButtonW% h25 gSSOK_WorkTools_ContractG2B, °è¾àÇöÈ² (³ª¶óÀåÅÍ)
    Gui, SSOKWorkTools:Add, Button, x8 y442 w%SSOK_WorkToolsButtonW% h25 gSSOK_WorkTools_ContractLofin365, °è¾àÇöÈ² (Áö¹æÀçÁ¤365)
    Gui, SSOKWorkTools:Add, Button, x8 y473 w%SSOK_WorkToolsButtonW% h25 gSSOK_WorkTools_ContractSejong, °è¾àÇöÈ² (¼¼Á¾)
    Gui, SSOKWorkTools:Add, Button, x8 y504 w%SSOK_WorkToolsButtonW% h25 vSSOK_WorkTools_Exit gSSOK_Tool_SidebarDeleteProxy, Á¾·á
    Gui, SSOKWorkTools:Add, Button, x8 y535 w%SSOK_WorkToolsButtonW% h25 vSSOK_WorkTools_BetaToggle gSSOK_WorkTools_BetaToggle, Å×½ºÆ®¹öÀü Beta
    Gui, SSOKWorkTools:Add, Button, x8 y597 w%SSOK_WorkToolsButtonW% h25 vSSOK_WorkTools_Win1 gSSOK_Advanced_Win1 Hidden, °£Æí ¿øÀÎÇàÀ§(win+2)
    Gui, SSOKWorkTools:Add, Button, x8 y628 w%SSOK_WorkToolsButtonW% h25 vSSOK_WorkTools_Win2 gSSOK_Advanced_Win2 Hidden, °£Æí ¿øÀÎÇàÀ§(win+4)
    Gui, SSOKWorkTools:Add, Button, x8 y659 w%SSOK_WorkToolsButtonW% h25 vSSOK_WorkTools_ExpenseDraft gSSOK_Advanced_ExpenseDraft Hidden, °£Æí ÁöÃâÇ°ÀÇ(win+1)
...
; ============================================================================
; ÇàÁ¤¾÷¹«´Þ·Â - ÀüºÏÆ¯º°ÀÚÄ¡µµ±³À°Ã» + ¼­¿ïÆ¯º°½Ã±³À°Ã»(ÃÊ) ÅëÇÕ ¾Ë¸²
; ¼³Á¤ ÀúÀå: ssok.ini [AdminCalendar]
; ±âº»°ª: Áö¿ª ¼­¿ï+ÀüºÏ / ON / 09:00, 11:00, 14:00, 16:30
; ============================================================================
...
Gui, SSOKAdminCalSettings:Font, s9 Norm, Malgun Gothic
    ...
    Gui, SSOKAdminCalSettings:Add, Checkbox, x112 yp w80 h24 vSSOK_AdminCalCfgJeonbuk, ÀüºÏ
    ...
    Gui, SSOKAdminCalSettings:Add, Text, x16 y+12 w390 h42 c666666, ¼±ÅÃÇÑ Áö¿ªÀÇ ¿À´Ã ÀÏÁ¤À» ÇÑ È­¸é¿¡ ÇÕÃÄ Ç¥½ÃÇÕ´Ï´Ù. ÀÏÁ¤ ¾Õ¿¡´Â ¼­¿ï/ÀüºÏ Áö¿ª¸íÀ» ºÙÀÌÁö ¾Ê½À´Ï´Ù. ¾Ë¸² È½¼ö¿Í ½Ã°£Àº ±âÁ¸ ¼³Á¤À» ±×´ë·Î »ç¿ëÇÕ´Ï´Ù.
...
SSOK_AdminCalendar_UpdateMenuText()
{
    ...
    SSOK_AdminCalendar_LoadSettings()
    text := SSOK_AdminCalEnabled ? "¾÷¹«´Þ·Â [ON]" : "¾÷¹«´Þ·Â [OFF]"
...
SSOK_AdminCalendar_ShowTodayPopup(manual := false)
{
    ...
    FormatTime, dateTitle,, M¿ù dÀÏ
    title := dateTitle . " ¿À´ÃÀÇ ÇàÁ¤¾÷¹« ´Þ·Â"

    ; ¼­¿ï/ÀüºÏ ÀÏÁ¤°ú º°°³·Î, ¼³Á¤µÈ ±â°üÀÇ ÇÐ±³ ÀÏÁ¤/±Þ½ÄÀº Ç×»ó Á¶È¸ÇÕ´Ï´Ù.
    ...
    ; ¼³Á¤µÈ ÇÐ±³¿¡ ±ÝÁÖ ÇÐ»çÀÏÁ¤ÀÌ ÀÖÀ¸¸é ¼­¿ï/ÀüºÏ ÀÏÁ¤Àº Ç¥½ÃÇÏÁö ¾Ê½À´Ï´Ù.
    ; ÇÐ±³ ÇÐ»çÀÏÁ¤ + ¿À´Ã ±Þ½Ä¸¸ °£´ÜÈ÷ Ç¥½ÃÇÕ´Ï´Ù.
    ...
    {
        ; Áö¿ª ÇàÁ¤´Þ·Â Á¶È¸¿¡ ½ÇÆÐÇØµµ ÇÐ±³ ÀÏÁ¤/±Þ½ÄÀÌ ÀÖÀ¸¸é ±× Á¤º¸´Â Ç¥½ÃÇÕ´Ï´Ù.
        SSOK_AdminCalendar_ShowPopupColumns(title, [], [], schoolLine, mealLine)
    }
    else
    {
        errText := "ÇàÁ¤´Þ·ÂÀ» ºÒ·¯¿ÀÁö ¸øÇß½À´Ï´Ù."
...
SSOK_AdminCalendar_ShowPopupColumns(title, seoulEvents, jeonbukEvents, schoolLine := "", mealLine := "")
{
    ...
    ; ¼­¿ï/ÀüºÏ ÀÏÁ¤ÀÌ ¾ø¾îµµ ÇÐ±³ ÇÐ»çÀÏÁ¤ ¶Ç´Â ±Þ½ÄÀÌ ÀÖÀ¸¸é ÆË¾÷À» Ç¥½ÃÇÕ´Ï´Ù.
    ...
    ; ÀÏÁ¤ÀÌ ¾ø´Â Áö¿ªÀº "ÀÏÁ¤ ¾øÀ½" ¹®±¸¸¦ Ç¥½ÃÇÏÁö ¾Ê°í ºóÄ­À¸·Î µÐ´Ù.
    ...
    ; ¼­¿ï/ÀüºÏ ÀÏÁ¤ÀÌ ¸¹¾ÆÁ®µµ ÁÙ¹Ù²ÞÀ» ÁÙÀÌµµ·Ï ÆøÀº À¯ÁöÇÏ°í, ÁÖ°£ÀÏÁ¤/±Þ½Ä º»¹®Àº 8pt·Î ÅëÀÏ
    Gui, SSOKAdminCalPopup:Font, s8 Bold, Malgun Gothic
    Gui, SSOKAdminCalPopup:Add, Text, x20 y13 w920 h22 Center, %title%

    Gui, SSOKAdminCalPopup:Font, s6 Normal c909090, Malgun Gothic
    Gui, SSOKAdminCalPopup:Add, Text, x24 y40 w435 h13 Left, ¼­¿ï
    Gui, SSOKAdminCalPopup:Add, Text, x505 y40 w435 h13 Left, ÀüºÏ
    ...
    ; ¼¼·Î ±¸ºÐ¼±Àº ½ÇÁ¦ ¼­¿ï/ÀüºÏ ÀÏÁ¤ ³ôÀÌ¿¡ ¸ÂÃç ÀÚµ¿ ¿¬Àå
    sepY := 39
    ...
    ; ¸ÞÀÎ »ó´Ü ±â°ü¸í¿¡ ¿¬°áµÈ ÇÐ±³ÀÇ ÀÌ¹ø ÁÖ(¿ù~ÀÏ) ÇÐ»çÀÏÁ¤
    if (schoolDisplay != "")
    {
        Gui, SSOKAdminCalPopup:Font, s8 Bold c30445A, Malgun Gothic
    ...
    ; ¿À´Ã ±Þ½Ä: Á¶½Ä/Áß½Ä/¼®½Ä ¸ðµÎ °°Àº µé¿©¾²±â ±ÔÄ¢ Àû¿ë
    if (mealDisplay != "")
...
SSOK_AdminCalendar_GetMySchoolWeekSchedule()
{
    ...
    if (!hasEvents)
        line := "ÁÖ°£ÀÏÁ¤: ±ÝÁÖ ÇÐ»çÀÏÁ¤ ¾øÀ½"
    else
        line := "ÁÖ°£ÀÏÁ¤: " . SSOK_AdminCalendar_FormatSchoolWeekEvents(events)
...
SSOK_AdminCalendar_DownloadSeoulHtml(ByRef error)
{
    ...
    for _, url in urls
    {
        try
        {
            ...
            if (req.Status >= 200 && req.Status < 400)
            {
                ...
                if (InStr(htmlUtf8, "ÇÐ±³±Þº° ÀÏÁ¤") || InStr(htmlUtf8, "ÃÊµîÇÐ±³"))
                    return htmlUtf8

                htmlText := req.ResponseText . ""
                if (InStr(htmlText, "ÇÐ±³±Þº° ÀÏÁ¤") || InStr(htmlText, "ÃÊµîÇÐ±³"))
                    return htmlText
            }
        }
        catch e
    ...
    if (error = "")
        error := "¼­¿ï±³À°Ã» ÇÐ±³±Þº° ÀÏÁ¤ ÆäÀÌÁö¿¡ ¿¬°áµÇÁö ¾Ê¾Ò½À´Ï´Ù."
    return ""
}

SSOK_AdminCalendar_ParseTodaySeoul(html)
{
    ; ¼­¿ï±³À°Ã» ¸ÞÀÎ È­¸é¿¡¼­ ±âº»À¸·Î ³»·Á¿À´Â 'À¯' ÀÏÁ¤ »ç¿ë
    ...
    for i, line in lines
    {
        ...
        {
            prev := Trim(lines[j])
            if (prev = "À¯" || InStr(prev, "À¯Ä¡¿ø"))
            {
                isKindergarten := true
                break
            }
            if (prev = "ÃÊ" || prev = "Áß" || prev = "°í")
                break
            j--
        }
    ...
    if (!target)
        return {ok:false, text:"", count:0, error:"¼­¿ï±³À°Ã» ¿À´Ã ÀÏÁ¤(À¯) ¿µ¿ªÀ» Ã£Áö ¸øÇß½À´Ï´Ù.", events:[]}
    ...
    Loop, 80
    {
        ...
        if (InStr(item, "ÇÐ±³±Þº° ÀÏÁ¤") || InStr(item, "°³ÀÎÁ¤º¸Ã³¸®¹æÄ§"))
            break
        if (item = "À¯Ä¡¿ø" || item = "¾÷¹«ÀÏÁ¤Ç¥" || item = "³ªÀÇ ÀÏÁ¤")
            continue
...
SSOK_AdminCalendar_DownloadHtml(ByRef error)
{
    ...
    for _, url in urls
    {
        try
        {
            ...
            if (req.Status >= 200 && req.Status < 400)
            {
                ; »çÀÌÆ®°¡ UTF-8ÀÌ¹Ç·Î ResponseBody¸¦ ¸ÕÀú ¸í½ÃÀûÀ¸·Î UTF-8·Î ÇØ¼®
                htmlUtf8 := SSOK_AdminCalendar_ResponseBodyToUtf8(req.ResponseBody)

                if (InStr(htmlUtf8, "ÇàÁ¤´Þ·Â") || InStr(htmlUtf8, "¿À´Ã³¯Â¥"))
                    return htmlUtf8

                ; È¯°æ¿¡ µû¶ó ResponseText°¡ Á¤»óÀÏ ¼ö ÀÖÀ¸¹Ç·Î µÎ ¹øÂ°·Î »ç¿ë
                htmlText := req.ResponseText . ""

                if (InStr(htmlText, "ÇàÁ¤´Þ·Â") || InStr(htmlText, "¿À´Ã³¯Â¥"))
                    return htmlText

                ; ¸¶Áö¸·À¸·Î º¸µå ½Äº°ÀÚ°¡ ÀÖÀ¸¸é HTML ÀÚÃ¼´Â ¹Þ¾Æ¿Â °ÍÀ¸·Î ÆÇ´Ü
    ...
    if (error = "")
        error := "ÀüºÏ±³À°Ã» ÇàÁ¤´Þ·Â ÆäÀÌÁö¿¡ ¿¬°áµÇÁö ¾Ê¾Ò½À´Ï´Ù."
...
SSOK_AdminCalendar_ParseToday(html)
{
    ...
    ; 1¼øÀ§: ¼­¹ö°¡ ºÙ¿© ÁÖ´Â "¿À´Ã³¯Â¥" Ç¥±â°¡ µé¾î°£ td¸¦ Á÷Á¢ Ã£À½
    patToday := "is)<td\b[^>]*>(?:(?!</td>)[\s\S])*?¿À´Ã³¯Â¥(?:(?!</td>)[\s\S])*?</td>"

    if RegExMatch(html, patToday, mToday)
        cellHtml := mToday

    ; 2¼øÀ§: ¸ðµç td¸¦ ¼øÈ¸ÇÏ¸ç ¼¿ÀÇ Ã¹ ¼ýÀÚ°¡ ¿À´Ã ³¯Â¥ÀÎÁö È®ÀÎ
    if (cellHtml = "")
    {
        pos := 1
    ...
    if (cellHtml = "")
        return {ok:false, text:"", count:0, error:"¿À´Ã ³¯Â¥ÀÇ ´Þ·Â Ä­À» Ã£Áö ¸øÇß½À´Ï´Ù.", events:[]}

    ; ¿À´Ã Ä­À» Ã£¾ÒÀ¸¸é, ÀÏÁ¤ÀÌ ÇÑ °Çµµ ¾ø¾îµµ ÀÌ°ÍÀº Á¤»óÀÔ´Ï´Ù.
    events := []
    pos := 1
