...
; ============================================================================
; ¿¹»ê DashBoard
; - ±Ù°ÅÀÚ·á: ÇÐ±³È¸°è-»ç¾÷°ü¸®-»ç¾÷°ü¸®Ä«µå-»ç¾÷°ü¸®Ä«µåÇö¾×
; - ¼¿º´ÇÕÇØÁ¦ ÈÄ ´Ù¿î·ÎµåÇÑ Excel ÆÄÀÏÀ» Å½»ö±â¿¡¼­ ¹Ù·Î ²ø¾î ³õ¾Æ ºÐ¼®
; - ¼¼ºÎ»ç¾÷ / ¼¼ºÎÇ×¸ñ / ¿ø°¡Åë°èºñ¸ñ / »êÃâ³»¿ª / ¼¼ºÎÇ×¸ñ´ã´çÀÚº° Áý°è¡¤Á¤·Ä
; - ¿øÀÎÇàÀ§¾×¡¤ÁýÇà·ü¡¤¿¹»êÀÜ¾× / ÁöÃâÇ°ÀÇ¾×¡¤ÁýÇà·ü¡¤ÁöÃâÀÜ¾× Ç¥½Ã
; - »êÃâ³»¿ªÀÇ (¼ö)/(¸ñ)Àº ¸ñÀûÀç¿ø, ³ª¸ÓÁö´Â ÀÏ¹ÝÀç¿ø, (ÅëÇÕ**)Àº ÅëÇÕ±³ºÎ»ç¾÷À¸·Î º°µµ °ü¸®
; - ¼Ò°è/ÇÕ°è/ÃÑ°è ÇàÀº Áßº¹ Áý°è¿¡¼­ Á¦¿ÜÇÏ°í ½ÇÁ¦ »êÃâ³»¿ª Çà¸¸ ÇÕ»ê
; - ¿øº» ÃÑ°è °ËÁõ, ÁýÇàÀÜ¾×, ÃÊ°úÁýÇà ÇÊÅÍ, 0¿ø Á¦¿Ü, °Ë»ö, Á¤·Ä, Ç¥ º¹»ç Áö¿ø
; ============================================================================
...
SSOKBudgetDashboardGuiEscape:
    SetTimer, SSOK_Expense_BudgetDashboard_RefreshDeferred, Off
...
SSOK_Expense_BudgetDashboard_Clear:
    ...
    GuiControl, SSOKBudgetDashboard:, SSOK_BudgetDashboardFileDrop, »ç¾÷°ü¸®Ä«µå(Çö¾×) ¶Ç´Â ÁýÇà½ÇÀû xls / xlsx ÆÄÀÏÀ» ¿©±â¿¡ ºÙ¿© ³õ¾ÆÁÖ¼¼¿ä
    GuiControl, SSOKBudgetDashboard:, SSOK_BudgetDashboardStatus, Å½»ö±â¿¡¼­ ¼¿º´ÇÕÇØÁ¦ÇÑ Excel ÆÄÀÏÀ» À§ ¿µ¿ªÀ¸·Î ²ø¾î ³õ¾ÆÁÖ¼¼¿ä.
return

SSOK_Expense_BudgetDashboard_ExportExe:
    SSOK_Expense_BudgetDashboard_CreateExe()
return

SSOK_Expense_BudgetDashboard_DownloadsFolder()
{
    ; Resolve the Windows Downloads known folder, including a relocated folder.
    VarSetCapacity(folderId, 16, 0)
    DllCall("ole32\CLSIDFromString", "WStr", "{374DE290-123F-4565-9164-39C4925E467B}", "Ptr", &folderId)
    pathPtr := 0
    result := DllCall("shell32\SHGetKnownFolderPath", "Ptr", &folderId, "UInt", 0, "Ptr", 0, "PtrP", pathPtr)
    if (result = 0 && pathPtr)
    {
        folder := StrGet(pathPtr, "UTF-16")
        DllCall("ole32\CoTaskMemFree", "Ptr", pathPtr)
        return folder
    }
    EnvGet, profileDir, USERPROFILE
    return profileDir . "\Downloads"
}

SSOK_Expense_BudgetDashboard_CreateExe()
{
    global SSOK_BudgetDashboardParsed, SSOK_BudgetDashboardFilePath
    if (!IsObject(SSOK_BudgetDashboardParsed) || !SSOK_BudgetDashboardParsed.rows.Length())
    {
        MsgBox, 48, ¿¹»ê Dashboard, ¸ÕÀú ¿¹»ê µ¥ÀÌÅÍ¸¦ ºÒ·¯¿Í ÁÖ¼¼¿ä.
        return false
    }
    FormatTime, stamp,, yyyyMMdd_HHmmss
    FileSelectFile, outputPath, S16, % SSOK_Expense_BudgetDashboard_DownloadsFolder() . "\¿¹»êDashboard_" . stamp . ".exe", ¹èÆ÷¿ë EXE ¸¸µé±â, ½ÇÇà ÆÄÀÏ (*.exe)
    if (outputPath = "")
        return false
    if !RegExMatch(outputPath, "i)\.exe$")
        outputPath .= ".exe"
    try
    {
        SSOK_Expense_BudgetDashboard_BuildExe(outputPath)
        MsgBox, 64, ¿¹»ê Dashboard, % "¹èÆ÷¿ë EXE¸¦ ¸¸µé¾ú½À´Ï´Ù.`n`n" . outputPath . "`n`nÀÌ ÆÄÀÏ ÇÏ³ª¸¦ Àü´ÞÇÏ¸é µË´Ï´Ù. »ý¼º ½ÃÁ¡ÀÇ ÀüÃ¼ ¿¹»ê µ¥ÀÌÅÍ°¡ Æ÷ÇÔµÇ¾î ÀÖ½À´Ï´Ù."
        return true
    }
    catch e
    {
        MsgBox, 48, ¿¹»ê Dashboard, % "EXE »ý¼º ½ÇÆÐ:`n" . e.Message
        return false
    }
}

SSOK_Expense_BudgetDashboard_FileDate(filePath)
{
    SplitPath, filePath, name
    if !RegExMatch(name, "(?<!\d)(20\d{2})(\d{2})(\d{2})", m)
        if !RegExMatch(name, "(?<!\d)(20\d{2})[.\-_](\d{1,2})[.\-_](\d{1,2})(?!\d)", m)
            return ""
    year := m1 + 0, month := m2 + 0, day := m3 + 0
    days := [31, (Mod(year, 400) = 0 || (Mod(year, 4) = 0 && Mod(year, 100) != 0)) ? 29 : 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]
    if (month < 1 || month > 12 || day < 1 || day > days[month])
        return ""
    return year . "." . month . "." . day . "."
}

SSOK_Expense_BudgetDashboard_Title()
{
    global SSOK_BudgetDashboardParsed, SSOK_BudgetDashboardStandalone, SSOK_BudgetDashboardFilePath
    p := SSOK_BudgetDashboardParsed
    if (!SSOK_BudgetDashboardStandalone || p.institution = "")
    {
        institution := ""
        if IsFunc("SSOK_GetOrgName")
            institution := Trim(Func("SSOK_GetOrgName").Call())
        if (institution = "" && !SSOK_BudgetDashboardStandalone)
            IniRead, institution, % A_ScriptDir . "\ssok.ini", MajorTodos, OrgName, %A_Space%
        if (Trim(institution) != "")
            p.institution := Trim(institution)
    }
    fileDate := SSOK_Expense_BudgetDashboard_FileDate(SSOK_BudgetDashboardFilePath)
    if (fileDate != "")
    {
        p.referenceDate := fileDate
        p.fiscalYear := SubStr(fileDate, 1, 4)
    }
    if !RegExMatch(p.fiscalYear, "^20\d{2}$")
        p.fiscalYear := A_YYYY
    title := (p.institution != "" ? p.institution . " " : "") . p.fiscalYear . "³â ÇÐ±³È¸°è ¿¹»ê Dashboard"
    return title . (p.referenceDate != "" ? "  (±âÁØÀÏÀÚ: " . p.referenceDate . ")" : "")
}

SSOK_Expense_BudgetDashboard_SnapshotXml()
{
    global SSOK_BudgetDashboardParsed, SSOK_BudgetDashboardFilePath
    SSOK_Expense_BudgetDashboard_Title()
    SplitPath, SSOK_BudgetDashboardFilePath, sourceName
    FormatTime, created,, yyyy-MM-dd HH:mm:ss
    xml := "<budget version=""1""><source>" . SSOK_Expense_BudgetDashboard_XmlText(sourceName) . "</source><created>" . created . "</created><summary>"
    for key, value in SSOK_BudgetDashboardParsed
        if (key != "rows" && !IsObject(value))
            xml .= "<field name=""" . key . """>" . SSOK_Expense_BudgetDashboard_XmlText(value) . "</field>"
    xml .= "</summary><rows>"
    for _, row in SSOK_BudgetDashboardParsed.rows
    {
        xml .= "<row>"
    ...
    return xml . "</rows></budget>"
}

SSOK_Expense_BudgetDashboard_LoadSnapshot(xml)
{
    global SSOK_BudgetDashboardParsed, SSOK_BudgetDashboardFilePath
    ...
    if !doc.loadXML(xml)
        throw Exception("³»Àå ¿¹»ê µ¥ÀÌÅÍ¸¦ ÀÐÀ» ¼ö ¾ø½À´Ï´Ù.")
    if (doc.documentElement.nodeName != "budget" || doc.documentElement.getAttribute("version") != "1")
        throw Exception("Áö¿øÇÏÁö ¾Ê´Â ¿¹»ê µ¥ÀÌÅÍ Çü½ÄÀÔ´Ï´Ù.")
    parsed := {rows:[]}
    for node in doc.selectNodes("/budget/summary/field")
        parsed[node.getAttribute("name")] := node.text
    for node in doc.selectNodes("/budget/rows/row")
    {
        row := {}
        for field in node.selectNodes("field")
            row[field.getAttribute("name")] := field.text
        if (row.resourceType = "ÀÏ¹ÝÀç¿ø")
        {
            row.generalCategory := SSOK_Expense_BudgetDashboard_GetGeneralCategory(row.calc)
            row.generalGroupName := SSOK_Expense_BudgetDashboard_GetGeneralGroupName(row.calc, row.generalCategory)
        }
        parsed.rows.Push(row)
    }
    if (!parsed.rows.Length() || parsed.rows.Length() != parsed.rowCount)
        throw Exception("³»Àå ¿¹»ê µ¥ÀÌÅÍÀÇ Çà ¼ö°¡ ÀÏÄ¡ÇÏÁö ¾Ê½À´Ï´Ù.")
    SSOK_BudgetDashboardParsed := parsed
    ; Keep exports independent of the sender's original folder.
    SSOK_BudgetDashboardFilePath := A_Desktop . "\" . doc.selectSingleNode("/budget/source").text
    return doc.selectSingleNode("/budget/created").text
}

SSOK_Expense_BudgetDashboard_ReadEmbeddedSnapshot()
{
    module := DllCall("GetModuleHandle", "Ptr", 0, "Ptr")
    resource := DllCall("FindResourceW", "Ptr", module, "WStr", "SSOK_BUDGET_DATA", "Ptr", 10, "Ptr")
    if (!resource)
        throw Exception("¿¹»ê µ¥ÀÌÅÍ°¡ ¾ø´Â ½ÇÇà ÆÄÀÏÀÔ´Ï´Ù. Dashboard¿¡¼­ ¹èÆ÷¿ë EXE¸¦ ¸¸µé¾î ÁÖ¼¼¿ä.")
    size := DllCall("SizeofResource", "Ptr", module, "Ptr", resource, "UInt")
    loaded := DllCall("LoadResource", "Ptr", module, "Ptr", resource, "Ptr")
    data := DllCall("LockResource", "Ptr", loaded, "Ptr")
    if (!data || !size)
        throw Exception("³»Àå ¿¹»ê µ¥ÀÌÅÍ¸¦ ÀÐÀ» ¼ö ¾ø½À´Ï´Ù.")
...
SSOK_Expense_BudgetDashboard_BuildExe(outputPath)
{
    global SSOK_BudgetDashboardStandalone
    if (outputPath = A_ScriptFullPath)
        throw Exception("½ÇÇà ÁßÀÎ ÇÁ·Î±×·¥°ú ´Ù¸¥ ÆÄÀÏ ÀÌ¸§À» ¼±ÅÃÇØ ÁÖ¼¼¿ä.")
    SplitPath, outputPath,, outputDir
    if (!InStr(FileExist(outputDir), "D"))
        throw Exception("ÀúÀå Æú´õ¸¦ Ã£À» ¼ö ¾ø½À´Ï´Ù.")
    ; Stage beside destination, publish only after the resource update succeeds.
    staged := outputPath . "." . DllCall("GetCurrentProcessId") . "." . A_TickCount . ".tmp"
    update := 0
    try
    {
        if (SSOK_BudgetDashboardStandalone)
        {
            FileCopy, %A_ScriptFullPath%, %staged%, 0
            if ErrorLevel
                throw Exception("½ÇÇà ÆÄÀÏÀ» º¹»çÇÒ ¼ö ¾ø½À´Ï´Ù.")
        }
        else
        {
            FileInstall, ssok_budget.exe, %staged%, 0
            if ErrorLevel
                throw Exception("Dashboard Àü¿ë ½ÇÇà ÆÄÀÏÀ» Ã£°Å³ª º¹»çÇÒ ¼ö ¾ø½À´Ï´Ù.")
        }
        xml := SSOK_Expense_BudgetDashboard_SnapshotXml()
        size := StrPut(xml, "UTF-8") - 1
        VarSetCapacity(buffer, size + 1, 0)
        StrPut(xml, &buffer, size + 1, "UTF-8")
        update := DllCall("BeginUpdateResourceW", "WStr", staged, "Int", false, "Ptr")
        if (!update)
            throw Exception("EXE µ¥ÀÌÅÍ ¿µ¿ªÀ» ¿­ ¼ö ¾ø½À´Ï´Ù. (" . A_LastError . ")")
        if !DllCall("UpdateResourceW", "Ptr", update, "Ptr", 10, "WStr", "SSOK_BUDGET_DATA", "UShort", 0, "Ptr", &buffer, "UInt", size)
            throw Exception("EXE¿¡ µ¥ÀÌÅÍ¸¦ ³ÖÀ» ¼ö ¾ø½À´Ï´Ù. (" . A_LastError . ")")
        saved := DllCall("EndUpdateResourceW", "Ptr", update, "Int", false)
        update := 0
        if (!saved)
            throw Exception("EXE µ¥ÀÌÅÍ ÀúÀå¿¡ ½ÇÆÐÇß½À´Ï´Ù.")
        if !DllCall("MoveFileExW", "WStr", staged, "WStr", outputPath, "UInt", 0x9)
            throw Exception("EXE¸¦ ÀúÀåÇÒ ¼ö ¾ø½À´Ï´Ù. ÆÄÀÏÀÌ ½ÇÇà ÁßÀÎÁö È®ÀÎÇØ ÁÖ¼¼¿ä. (" . A_LastError . ")")
        return true
    }
    finally
    {
        if (update)
            DllCall("EndUpdateResourceW", "Ptr", update, "Int", true)
        ; Only remove our exact staging file, never the destination.
        if FileExist(staged)
            FileDelete, %staged%
    }
}
...
SSOK_Expense_BudgetDashboard_Refresh:
    SetTimer, SSOK_Expense_BudgetDashboard_RefreshDeferred, -180
return

SSOK_Expense_BudgetDashboard_RefreshDeferred:
    SSOK_Expense_BudgetDashboard_RefreshAll()
return

SSOK_Expense_BudgetDashboard_ResourceViewRefresh:
    SSOK_Expense_BudgetDashboard_RefreshResourceControl(A_GuiControl)
return
...
SSOK_Expense_BudgetDashboard_ExportOverall:
    SSOK_Expense_BudgetDashboard_ExportListView("SSOK_BudgetDashboardMainLV", "ÀüÃ¼ÇöÈ²"
        , "¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|°è¾à±Ý¾× (¿øÀÎÇàÀ§)|Áö±Þ¾×|¹ÌÁöÃâ¾× (Áö±Þ´ë±â)|¿¹»êÀÜ¾× (ºÒ¿ë¿¹»ê¾×)|ÁýÇà·ü (¿øÀÎÇàÀ§)|¸ñÀûÀç¿ø|ÀÏ¹ÝÀç¿ø|´ã´çÀÚ"
        , "SSOK_BudgetDashboardListStatus")
return

SSOK_Expense_BudgetDashboard_ExportPurposeUser:
    SSOK_Expense_BudgetDashboard_ExportListView("SSOK_BudgetDashboardPurposeUserLV", "¸ñÀûÀç¿ø_¼öÀÍÀÚ"
        , "¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|°è¾à±Ý¾× (¿øÀÎÇàÀ§)|Áö±Þ¾×|¹ÌÁöÃâ¾× (Áö±Þ´ë±â)|¿¹»êÀÜ¾× (ºÒ¿ë¿¹»ê¾×)|ÁýÇà·ü (¿øÀÎÇàÀ§)|´ã´çÀÚ"
        , "SSOK_BudgetDashboardPurposeUserStatus")
return

SSOK_Expense_BudgetDashboard_ExportPurposeAll:
    SSOK_Expense_BudgetDashboard_ExportListView("SSOK_BudgetDashboardPurposeAllLV", "¸ñÀûÀç¿ø_ÀüÃ¼"
        , "¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|°è¾à±Ý¾× (¿øÀÎÇàÀ§)|Áö±Þ¾×|¹ÌÁöÃâ¾× (Áö±Þ´ë±â)|¿¹»êÀÜ¾× (ºÒ¿ë¿¹»ê¾×)|ÁýÇà·ü (¿øÀÎÇàÀ§)|´ã´çÀÚ"
        , "SSOK_BudgetDashboardPurposeAllStatus")
return

SSOK_Expense_BudgetDashboard_ExportPurposeProject:
    SSOK_Expense_BudgetDashboard_ExportListView("SSOK_BudgetDashboardPurposeProjectLV", "¸ñÀûÀç¿ø_¸ñÀû»ç¾÷"
        , "¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|°è¾à±Ý¾× (¿øÀÎÇàÀ§)|Áö±Þ¾×|¹ÌÁöÃâ¾× (Áö±Þ´ë±â)|¿¹»êÀÜ¾× (ºÒ¿ë¿¹»ê¾×)|ÁýÇà·ü (¿øÀÎÇàÀ§)|´ã´çÀÚ"
        , "SSOK_BudgetDashboardPurposeProjectStatus")
return

SSOK_Expense_BudgetDashboard_ExportPurposeNational:
    SSOK_Expense_BudgetDashboard_ExportListView("SSOK_BudgetDashboardPurposeNationalLV", "¸ñÀûÀç¿ø_±¹°íº¸Á¶±Ý"
        , "¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|°è¾à±Ý¾× (¿øÀÎÇàÀ§)|Áö±Þ¾×|¹ÌÁöÃâ¾× (Áö±Þ´ë±â)|¿¹»êÀÜ¾× (ºÒ¿ë¿¹»ê¾×)|ÁýÇà·ü (¿øÀÎÇàÀ§)|´ã´çÀÚ"
        , "SSOK_BudgetDashboardPurposeNationalStatus")
return

SSOK_Expense_BudgetDashboard_ExportGeneralAll:
    SSOK_Expense_BudgetDashboard_ExportListView("SSOK_BudgetDashboardGeneralAllLV", "ÀÏ¹ÝÀç¿ø_ÀüÃ¼"
        , "¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|°è¾à±Ý¾× (¿øÀÎÇàÀ§)|Áö±Þ¾×|¹ÌÁöÃâ¾× (Áö±Þ´ë±â)|¿¹»êÀÜ¾× (ºÒ¿ë¿¹»ê¾×)|ÁýÇà·ü (¿øÀÎÇàÀ§)|´ã´çÀÚ"
        , "SSOK_BudgetDashboardGeneralAllStatus")
return

SSOK_Expense_BudgetDashboard_ExportGeneralFuture:
    SSOK_Expense_BudgetDashboard_ExportListView("SSOK_BudgetDashboardFutureLV", "ÀÏ¹ÝÀç¿ø_¹Ì·¡"
        , "¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|°è¾à±Ý¾× (¿øÀÎÇàÀ§)|Áö±Þ¾×|¹ÌÁöÃâ¾× (Áö±Þ´ë±â)|¿¹»êÀÜ¾× (ºÒ¿ë¿¹»ê¾×)|ÁýÇà·ü (¿øÀÎÇàÀ§)|´ã´çÀÚ"
        , "SSOK_BudgetDashboardFutureStatus")
return

SSOK_Expense_BudgetDashboard_ExportGeneralSpecial:
    SSOK_Expense_BudgetDashboard_ExportListView("SSOK_BudgetDashboardSpecialLV", "ÀÏ¹ÝÀç¿ø_Æ¯¼º"
        , "¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|°è¾à±Ý¾× (¿øÀÎÇàÀ§)|Áö±Þ¾×|¹ÌÁöÃâ¾× (Áö±Þ´ë±â)|¿¹»êÀÜ¾× (ºÒ¿ë¿¹»ê¾×)|ÁýÇà·ü (¿øÀÎÇàÀ§)|´ã´çÀÚ"
        , "SSOK_BudgetDashboardSpecialStatus")
return

SSOK_Expense_BudgetDashboard_ExportGeneralIntegrated:
    SSOK_Expense_BudgetDashboard_ExportListView("SSOK_BudgetDashboardIntegratedLV", "ÀÏ¹ÝÀç¿ø_ÅëÇÕ"
        , "¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|°è¾à±Ý¾× (¿øÀÎÇàÀ§)|Áö±Þ¾×|¹ÌÁöÃâ¾× (Áö±Þ´ë±â)|¿¹»êÀÜ¾× (ºÒ¿ë¿¹»ê¾×)|ÁýÇà·ü (¿øÀÎÇàÀ§)|´ã´çÀÚ"
        , "SSOK_BudgetDashboardIntegratedStatus")
return

SSOK_Expense_BudgetDashboard_ExportGeneralPlain:
    SSOK_Expense_BudgetDashboard_ExportListView("SSOK_BudgetDashboardGeneralLV", "ÀÏ¹ÝÀç¿ø_ÀÏ¹Ý"
        , "¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|°è¾à±Ý¾× (¿øÀÎÇàÀ§)|Áö±Þ¾×|¹ÌÁöÃâ¾× (Áö±Þ´ë±â)|¿¹»êÀÜ¾× (ºÒ¿ë¿¹»ê¾×)|ÁýÇà·ü (¿øÀÎÇàÀ§)|´ã´çÀÚ"
        , "SSOK_BudgetDashboardGeneralStatus")
return

SSOK_Expense_BudgetDashboard_ExportWorkAll:
    SSOK_Expense_BudgetDashboard_ExportListView("SSOK_BudgetDashboardWorkAllLV", "¾÷¹«ÃßÁøºñ_ÀüÃ¼"
        , "¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|°è¾à±Ý¾× (¿øÀÎÇàÀ§)|Áö±Þ¾×|¹ÌÁöÃâ¾× (Áö±Þ´ë±â)|¿¹»êÀÜ¾× (ºÒ¿ë¿¹»ê¾×)|ÁýÇà·ü (¿øÀÎÇàÀ§)|´ã´çÀÚ"
        , "SSOK_BudgetDashboardWorkAllStatus")
return

SSOK_Expense_BudgetDashboard_ExportWorkGeneral:
    SSOK_Expense_BudgetDashboard_ExportListView("SSOK_BudgetDashboardWorkGeneralLV", "¾÷¹«ÃßÁøºñ_ÀÏ¹ÝÀç¿ø"
        , "¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|°è¾à±Ý¾× (¿øÀÎÇàÀ§)|Áö±Þ¾×|¹ÌÁöÃâ¾× (Áö±Þ´ë±â)|¿¹»êÀÜ¾× (ºÒ¿ë¿¹»ê¾×)|ÁýÇà·ü (¿øÀÎÇàÀ§)|´ã´çÀÚ"
        , "SSOK_BudgetDashboardWorkGeneralStatus")
return

SSOK_Expense_BudgetDashboard_ExportWorkPurpose:
    SSOK_Expense_BudgetDashboard_ExportListView("SSOK_BudgetDashboardWorkPurposeLV", "¾÷¹«ÃßÁøºñ_¸ñÀûÀç¿ø"
        , "¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|°è¾à±Ý¾× (¿øÀÎÇàÀ§)|Áö±Þ¾×|¹ÌÁöÃâ¾× (Áö±Þ´ë±â)|¿¹»êÀÜ¾× (ºÒ¿ë¿¹»ê¾×)|ÁýÇà·ü (¿øÀÎÇàÀ§)|´ã´çÀÚ"
        , "SSOK_BudgetDashboardWorkPurposeStatus")
return

SSOK_Expense_BudgetDashboard_ExportOver:
    SSOK_Expense_BudgetDashboard_ExportListView("SSOK_BudgetDashboardOverLV", "ÃÊ°úÁýÇà_°ü¸®"
        , "»óÅÂ|Àç¿ø|¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|°è¾à±Ý¾× (¿øÀÎÇàÀ§)|Áö±Þ¾×|¹ÌÁöÃâ¾× (Áö±Þ´ë±â)|¿¹»êÀÜ¾× (ºÒ¿ë¿¹»ê¾×)|ÁýÇà·ü (¿øÀÎÇàÀ§)", "SSOK_BudgetDashboardOverStatus")
return

SSOK_Expense_BudgetDashboard_BuildOverEntries()
{
    global SSOK_BudgetDashboardParsed
    affected := {}, groups := {}, entries := []
    for _, row in SSOK_Expense_BudgetDashboard_FilterRows()
    {
        key := row.business . Chr(30) . row.subItem . Chr(30) . row.cost
        if (row.commit > row.current)
            affected[key] := true
    }
    for _, row in SSOK_Expense_BudgetDashboard_FilterRows()
    {
        key := row.business . Chr(30) . row.subItem . Chr(30) . row.cost
        if !affected.HasKey(key)
            continue
...
SSOK_Expense_BudgetDashboard_FillOver()
{
    GuiControlGet, view, SSOKBudgetDashboard:, SSOK_BudgetDashboardOverView
    details := SSOK_Expense_BudgetDashboard_BuildOverEntries()
    entries := view = "»êÃâ³»¿ªº°" ? details : SSOK_Expense_BudgetDashboard_BuildEntries(view, "ÀüÃ¼", false, details)
    Gui, SSOKBudgetDashboard:Default
    Gui, SSOKBudgetDashboard:ListView, SSOK_BudgetDashboardOverLV
    GuiControl, SSOKBudgetDashboard:-Redraw, SSOK_BudgetDashboardOverLV
    LV_Delete()
    current := 0, commit := 0, paid := 0, overCount := 0, excess := 0
    for _, row in entries
    {
        isOver := row.commit > row.current
        LV_Add("", isOver ? "ÃÊ°ú" : "°ü·Ã", row.resourceType
            , view = "¿ø°¡Åë°èºñ¸ñº°" ? "" : row.business
            , view = "¼¼ºÎ»ç¾÷º°" || view = "¿ø°¡Åë°èºñ¸ñº°" ? "" : row.subItem
            , view = "»êÃâ³»¿ªº°" ? row.calc : ""
            , view = "¿ø°¡Åë°èºñ¸ñº°" || view = "»êÃâ³»¿ªº°" ? row.cost : ""
            , SSOK_Expense_FormatNumber(row.current), SSOK_Expense_FormatNumber(row.commit), SSOK_Expense_FormatNumber(row.paid), SSOK_Expense_FormatNumber(row.commit - row.paid), SSOK_Expense_FormatNumber(row.current - row.commit), SSOK_Expense_BudgetDashboard_FormatRate(row.current, row.commit))
        current += row.current, commit += row.commit, paid += row.paid
        if (isOver)
            overCount++, excess += row.commit - row.current
    }
    widths := [65,75,120,140,210,110,100,110,100,110,120,105]
    for index, width in widths
        LV_ModifyCol(index, (index >= 7 ? "Right " : "") . width)
    SSOK_Expense_BudgetDashboard_SetTopSummary("SSOK_BudgetDashboardOverLV", 7, current, commit, paid)
    status := "ÃÊ°ú " . overCount . "°Ç / ÃÊ°ú±Ý¾× " . SSOK_Expense_FormatNumber(excess) . "¿ø / °ü·Ã Æ÷ÇÔ " . entries.Length() . "°Ç. »ó´Ü ÇÕ°è´Â °ü·Ã ³»¿ª±îÁö Æ÷ÇÔÇÕ´Ï´Ù."
    GuiControl, SSOKBudgetDashboard:, SSOK_BudgetDashboardOverStatus, %status%
    GuiControl, SSOKBudgetDashboard:+Redraw, SSOK_BudgetDashboardOverLV
    SSOK_Expense_BudgetDashboard_UpdateColors("SSOK_BudgetDashboardOverLV")
}

SSOK_Expense_BudgetDashboard_CostRefresh:
    SSOK_Expense_BudgetDashboard_FillCost()
return

SSOK_Expense_BudgetDashboard_ExportCost:
    SSOK_Expense_BudgetDashboard_ExportListView("SSOK_BudgetDashboardCostLV", "¿ø°¡Åë°èºñ¸ñº°_ÇÕ°è"
        , "¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|°è¾à±Ý¾× (¿øÀÎÇàÀ§)|Áö±Þ¾×|¹ÌÁöÃâ¾× (Áö±Þ´ë±â)|¿¹»êÀÜ¾× (ºÒ¿ë¿¹»ê¾×)|ÁýÇà·ü (¿øÀÎÇàÀ§)|¸ñÀûÀç¿ø|ÀÏ¹ÝÀç¿ø", "SSOK_BudgetDashboardCostStatus")
return

SSOK_Expense_BudgetDashboard_FillCost()
{
    global SSOK_BudgetDashboardCostResource, SSOK_BudgetDashboardCostSort
    Gui, SSOKBudgetDashboard:Submit, NoHide
    entries := SSOK_Expense_BudgetDashboard_BuildEntries("¿ø°¡Åë°èºñ¸ñº°", SSOK_BudgetDashboardCostResource, true)
    SSOK_Expense_BudgetDashboard_SortEntries(entries, SSOK_BudgetDashboardCostSort)
    Gui, SSOKBudgetDashboard:Default
    Gui, SSOKBudgetDashboard:ListView, SSOK_BudgetDashboardCostLV
    GuiControl, SSOKBudgetDashboard:-Redraw, SSOK_BudgetDashboardCostLV
    LV_Delete()
    current := 0, commit := 0, paid := 0
    purpose := 0, general := 0
    for _, entry in entries
    {
        LV_Add("", entry.label, SSOK_Expense_FormatNumber(entry.current), SSOK_Expense_FormatNumber(entry.commit), SSOK_Expense_FormatNumber(entry.paid), SSOK_Expense_FormatNumber(entry.commit - entry.paid), SSOK_Expense_FormatNumber(entry.current - entry.commit), SSOK_Expense_BudgetDashboard_FormatRate(entry.current, entry.commit)
            , SSOK_Expense_FormatNumber(entry.settle), SSOK_Expense_FormatNumber(entry.general))
        purpose += entry.settle, general += entry.general
        current += entry.current, commit += entry.commit, paid += entry.paid
    }
    SSOK_Expense_BudgetDashboard_SetTopSummary("SSOK_BudgetDashboardCostLV", 2, current, commit, paid, purpose, general)
    LV_ModifyCol(1, 300)
    Loop, 9
        LV_ModifyCol(A_Index + 1, "Right 120")
    GuiControl, SSOKBudgetDashboard:+Redraw, SSOK_BudgetDashboardCostLV
    summary := SSOK_Expense_BudgetDashboard_MakeDetailedSummary(SSOK_BudgetDashboardCostResource . " ÇÕ°è", entries.Length(), current, commit, paid)
    GuiControl, SSOKBudgetDashboard:, SSOK_BudgetDashboardCostStatus, %summary%
    SSOK_Expense_BudgetDashboard_UpdateColors("SSOK_BudgetDashboardCostLV")
}
...
SSOK_Expense_BudgetDashboard_Show()
{
    ...
    Gui, SSOKBudgetDashboard:Destroy
    Gui, SSOKBudgetDashboard:New, -ToolWindow +MinimizeBox +HwndSSOK_BudgetDashboardHwnd, ¾÷¹«¿ëµµ±¸ - ¿¹»ê DashBoard
    SSOK_Expense_BudgetDashboard_SetIcon(SSOK_BudgetDashboardHwnd)
    ...
    Gui, SSOKBudgetDashboard:Add, Text, x20 y16 w820 h30 c005BAC, ¾÷¹«¿ëµµ±¸ - ¿¹»ê DashBoard

    Gui, SSOKBudgetDashboard:Font, s10 Bold, Malgun Gothic
    fileDisplay := FileExist(SSOK_BudgetDashboardFilePath) ? SSOK_BudgetDashboardFilePath : "»ç¾÷°ü¸®Ä«µå(Çö¾×) ¶Ç´Â ÁýÇà½ÇÀû xls / xlsx ÆÄÀÏÀ» ¿©±â¿¡ ºÙ¿© ³õ¾ÆÁÖ¼¼¿ä"
    ...
    Gui, SSOKBudgetDashboard:Add, Text, x20 y194 w820 h24 c555555, ÀÚ·áÃâÃ³: ÇÐ±³È¸°è-»ç¾÷°ü¸®-»ç¾÷°ü¸®Ä«µå-»ç¾÷°ü¸®Ä«µåÇö¾×
    Gui, SSOKBudgetDashboard:Add, Text, x20 y220 w820 h24 c555555, * ¼¿º´ÇÕÇØÁ¦ÇÑ ¿¢¼¿ ÆÄÀÏÀ» ´Ù¿î·Îµå ÇÕ´Ï´Ù.
    Gui, SSOKBudgetDashboard:Add, Text, x20 y246 w820 h1 Hidden vSSOK_BudgetDashboardStatus,

    Gui, SSOKBudgetDashboard:Add, Button, x20 y258 w150 h38 gSSOK_Expense_BudgetDashboard_Clear, ÃÊ±âÈ­

    Gui, SSOKBudgetDashboard:Show, w860 h318 Center, ¾÷¹«¿ëµµ±¸ - ¿¹»ê DashBoard
}
...
SSOK_Expense_BudgetDashboard_HandleDrop(dropText, targetControl := "")
{
    ...
    {
        MsgBox, 48, ¿¹»ê DashBoard, Excel ÆÄÀÏ(.xlsx/.xlsm/.xls)¸¸ ²ø¾î ³õÀ» ¼ö ÀÖ½À´Ï´Ù.
        return
    }

    GuiControl, SSOKBudgetDashboard:, SSOK_BudgetDashboardFileDrop, % SSOK_BudgetDashboardFilePath
    GuiControl, SSOKBudgetDashboard:, SSOK_BudgetDashboardFileDrop, ÆÄÀÏÀ» ÀÐ°í ºÐ¼®ÇÏ°í ÀÖ½À´Ï´Ù...

    SSOK_Expense_BudgetDashboard_LoadFile(files[1])
}
...
SSOK_Expense_BudgetDashboard_LoadFile(filePath)
{
    ...
    {
        MsgBox, 48, ¿¹»ê DashBoard, »ç¾÷°ü¸®Ä«µå(Çö¾×) Excel ÆÄÀÏÀ» Ã£À» ¼ö ¾ø½À´Ï´Ù.
        return false
    }

    if !RegExMatch(filePath, "i)\.(xlsx|xlsm|xls)$")
    {
        MsgBox, 48, ¿¹»ê DashBoard, Excel ÆÄÀÏ(.xlsx/.xlsm/.xls)¸¸ »ç¿ëÇÒ ¼ö ÀÖ½À´Ï´Ù.
        return false
    }
    ...
    {
        GuiControl, SSOKBudgetDashboard:, SSOK_BudgetDashboardFileDrop, ºÐ¼® ½ÇÆÐ - ´Ù¸¥ ¿¹»ê Excel ÆÄÀÏÀ» ¿©±â¿¡ ³õ¾ÆÁÖ¼¼¿ä
        if (errMsg = "")
            errMsg := "»ç¾÷°ü¸®Ä«µå(Çö¾×) ÆÄÀÏÀ» ºÐ¼®ÇÏÁö ¸øÇß½À´Ï´Ù."
        MsgBox, 48, ¿¹»ê DashBoard, %errMsg%
        return false
    }
...
SSOK_Expense_BudgetDashboard_ReadExcel(filePath, ByRef parsed, ByRef errMsg)
{
    ...
    try
    {
        ...
        Loop, %sheetCount%
        {
            ...
            if SSOK_Expense_BudgetDashboard_FindHeader(ws
                ...
                , colCurrent, colDraft, colCommit, colPaid
        ...
        {
            errMsg := "»ç¾÷°ü¸®Ä«µå(Çö¾×) Ç¥ ¸Ó¸®±ÛÀ» Ã£Áö ¸øÇß½À´Ï´Ù.`n`n"
                . "ÇÊ¼ö ¿­: ¼¼ºÎ»ç¾÷, ¼¼ºÎÇ×¸ñ, ¿ø°¡Åë°èºñ¸ñ, »êÃâ³»¿ª, ¿¹»êÇö¾×(A), ÁöÃâÇ°ÀÇ¾×, ¿øÀÎÇàÀ§¾×(B), Áö±Þ¾×(C)`n"
                . "* ¼¿º´ÇÕÇØÁ¦ ÈÄ ´Ù¿î·ÎµåÇÑ ÆÄÀÏÀÎÁö È®ÀÎÇØ ÁÖ¼¼¿ä."
        }
        else
        {
            stage := "¿¹»ê ÀÚ·á Áý°è"
    ...
    {
        errMsg := "¿¹»ê Dashboard ºÐ¼® Áß ¿À·ù°¡ ¹ß»ýÇß½À´Ï´Ù.`n`n´Ü°è: " . stage . "`n" . e.Message
    }
...
SSOK_Expense_BudgetDashboard_FindHeader(ws, ByRef headerRow, ByRef lastRow, ByRef colBusiness, ByRef colSubItem, ByRef colCost, ByRef colCalc, ByRef colCurrent, ByRef colDraft, ByRef colCommit, ByRef colPaid, ByRef colSettle, ByRef colGeneral, ByRef colManager)
{
    headerRow := 0
    lastRow := 0
    colBusiness := 0
    ...
    Loop, %rowTotal%
    {
        ...
        Loop, %colTotal%
        {
            ...
            if (key = "¼¼ºÎ»ç¾÷")
                ...
                tmpSubItem := c
            else if ((key = "¿ø°¡Åë°èºñ¸ñ" || key = "¿ø°¡Åë°èºñ¸ñ¸í"))
                tmpCost := c
            else if (key = "»êÃâ³»¿ª")
                tmpCalc := c
            else if InStr(key, "¿¹»êÇö¾×")
                tmpCurrent := c
            else if InStr(key, "ÁöÃâÇ°ÀÇ¾×")
                tmpDraft := c
            else if (InStr(key, "¿øÀÎÇàÀ§¾×") || InStr(key, "¿øÀÎÇàÀ§±Ý¾×"))
                tmpCommit := c
            else if (InStr(key, "Áö±Þ¾×") || InStr(key, "ÁöÃâ±Ý¾×"))
                tmpPaid := c
            else if (key = "Á¤»êÀç¿ø")
                tmpSettle := c
            else if (key = "ÀÏ¹ÝÀç¿ø")
                tmpGeneral := c
            else if InStr(key, "¼¼ºÎÇ×¸ñ´ã´çÀÚ")
                tmpManager := c
        }

        if (tmpBusiness && tmpCost && tmpCalc && tmpCurrent && tmpCommit && tmpPaid)
        {
            headerRow := r
...
SSOK_Expense_BudgetDashboard_ReadSheet(ws, headerRow, lastRow, colBusiness, colSubItem, colCost, colCalc, colCurrent, colDraft, colCommit, colPaid, colSettle, colGeneral, colManager, ByRef parsed, ByRef errMsg)
{
    errMsg := ""
    rows := []
    totalCurrent := 0
    ...
    ; ¸ñÀûÀç¿ø ¼¼ºÎ ±¸ºÐ
    purposeUserCurrent := 0
    purposeUserCommit := 0
    purposeUserPaid := 0
    purposeProjectCurrent := 0
    purposeProjectCommit := 0
    purposeProjectPaid := 0
    purposeNationalCurrent := 0
    purposeNationalCommit := 0
    purposeNationalPaid := 0

    ; ÀÏ¹ÝÀç¿ø ¼¼ºÎ ±¸ºÐ
    generalIntegratedCurrent := 0
    generalIntegratedCommit := 0
    generalIntegratedPaid := 0
    generalFutureCurrent := 0
    generalFutureCommit := 0
    generalFuturePaid := 0
    generalSpecialCurrent := 0
    generalSpecialCommit := 0
    generalSpecialPaid := 0
    generalPlainCurrent := 0
    generalPlainCommit := 0
    generalPlainPaid := 0
    ...
    if (dataCount < 1)
    {
        errMsg := "»ç¾÷°ü¸®Ä«µå¿¡ Áý°èÇÒ ÀÚ·á°¡ ¾ø½À´Ï´Ù."
        return false
    }
    ...
    Loop, %dataCount%
    {
        ...
        if InStr(marker, "ÃÑ°è")
        {
            ...
            sourcePaid := SSOK_Expense_BudgetDashboard_ParseNumber(SSOK_Expense_BudgetDashboard_GetCellText(ws, r, colPaid))
            sourcePurpose := colSettle ? SSOK_Expense_BudgetDashboard_ParseNumber(SSOK_Expense_BudgetDashboard_GetCellText(ws, r, colSettle)) : 0
            sourceGeneral := colGeneral ? SSOK_Expense_BudgetDashboard_ParseNumber(SSOK_Expense_BudgetDashboard_GetCellText(ws, r, colGeneral)) : 0
        }

        ; ¼Ò°è/ÇÕ°è/ÃÑ°è´Â ½ÇÁ¦ »êÃâ³»¿ªÀ» ´Ù½Ã ÇÕÄ£ ÇàÀÌ¹Ç·Î Áßº¹ Áý°è¿¡¼­ Á¦¿ÜÇÕ´Ï´Ù.
        if (InStr(marker, "¼Ò°è") || InStr(marker, "ÇÕ°è") || InStr(marker, "ÃÑ°è"))
            continue
        ...
        ; ½ÇÁ¦ »êÃâ³»¿ª Çà¸¸ ´ë½Ãº¸µå ¿øÀÚ·á·Î »ç¿ëÇÕ´Ï´Ù.
        if (calc = "" || cost = "")
            continue

        if (SSOK_Expense_BudgetDashboard_HeaderKey(business) = "¼¼ºÎ»ç¾÷"
            || SSOK_Expense_BudgetDashboard_HeaderKey(calc) = "»êÃâ³»¿ª")
            continue
        ...
        paid := SSOK_Expense_BudgetDashboard_ParseNumber(SSOK_Expense_BudgetDashboard_GetCellText(ws, r, colPaid))
        rate := (current != 0) ? (commit / current * 100) : ((commit > 0) ? 1000000000 : 0)
        paidRate := (current != 0) ? (paid / current * 100) : ((paid > 0) ? 1000000000 : 0)

        ; Àç¿ø ±¸ºÐÀº »êÃâ³»¿ª Ç¥±â¸¦ ±âÁØÀ¸·Î ÇÕ´Ï´Ù.
        ; ¸ñÀûÀç¿ø: (¼ö), (¸ñ), (¸ñ**) / ±¹°í»ç¾÷Àº ¸ñÀûÀç¿ø Áß ¼¼ºÎ»ç¾÷ ¶Ç´Â ¼¼ºÎÇ×¸ñ¿¡ [±¹°í]°¡ ÀÖ´Â Ç×¸ñ
        ; ÀÏ¹ÝÀç¿ø: ¸ñÀûÀç¿ø ¿Ü ÀüÃ¼ / (ÅëÇÕ**), (¹Ì·¡**), (¹Ì), (Æ¯¼º*)´Â °¢°¢ º°µµ ±¸ºÐ
        purposeCategory := SSOK_Expense_BudgetDashboard_GetPurposeCategory(business, subItem, calc)
        if (purposeCategory != "")
        {
            resourceType := "¸ñÀûÀç¿ø"
            generalCategory := ""
            generalGroupName := ""
        }
        else
        {
            resourceType := "ÀÏ¹ÝÀç¿ø"
            generalCategory := SSOK_Expense_BudgetDashboard_GetGeneralCategory(calc)
            generalGroupName := SSOK_Expense_BudgetDashboard_GetGeneralGroupName(calc, generalCategory)
        }

        purposeCurrent := (resourceType = "¸ñÀûÀç¿ø") ? current : 0
        generalCurrent := (resourceType = "ÀÏ¹ÝÀç¿ø") ? current : 0
        purposeCommit := (resourceType = "¸ñÀûÀç¿ø") ? commit : 0
        generalCommit := (resourceType = "ÀÏ¹ÝÀç¿ø") ? commit : 0
        purposePaid := (resourceType = "¸ñÀûÀç¿ø") ? paid : 0
        generalPaid := (resourceType = "ÀÏ¹ÝÀç¿ø") ? paid : 0
        ...
        rows.Push({business:business
            ...
            , settle:purposeCurrent
            , general:generalCurrent
            , settleCommit:purposeCommit
            , generalCommit:generalCommit
            , settlePaid:purposePaid
            , generalPaid:generalPaid})

        totalCurrent += current
        totalDraft += draft
        totalCommit += commit
        totalPaid += paid
        totalPurpose += purposeCurrent
        totalGeneral += generalCurrent
        totalPurposeCommit += purposeCommit
        totalGeneralCommit += generalCommit
        totalPurposePaid += purposePaid
        totalGeneralPaid += generalPaid

        if (purposeCategory = "¼öÀÍÀÚ")
        {
            purposeUserCurrent += current
            purposeUserCommit += commit
            purposeUserPaid += paid
        }
        else if (purposeCategory = "±¹°í»ç¾÷")
        {
            purposeNationalCurrent += current
            purposeNationalCommit += commit
            purposeNationalPaid += paid
        }
        else if (purposeCategory = "¸ñÀû»ç¾÷")
        {
            purposeProjectCurrent += current
            purposeProjectCommit += commit
            purposeProjectPaid += paid
        }
        else if (generalCategory = "ÅëÇÕ")
        {
            generalIntegratedCurrent += current
            generalIntegratedCommit += commit
            generalIntegratedPaid += paid
        }
        else if (generalCategory = "¹Ì·¡")
        {
            generalFutureCurrent += current
            generalFutureCommit += commit
            generalFuturePaid += paid
        }
        else if (generalCategory = "Æ¯¼º")
        {
            generalSpecialCurrent += current
            generalSpecialCommit += commit
            generalSpecialPaid += paid
        }
        else if (generalCategory = "ÀÏ¹Ý")
        {
            generalPlainCurrent += current
            generalPlainCommit += commit
            generalPlainPaid += paid
        }
    ...
    if (!rows.Length())
    {
        errMsg := "½ÇÁ¦ »êÃâ³»¿ª ÇàÀ» Ã£Áö ¸øÇß½À´Ï´Ù.`n`n¼¿º´ÇÕÇØÁ¦ ÈÄ ´Ù¿î·ÎµåÇÑ »ç¾÷°ü¸®Ä«µå(Çö¾×) ÆÄÀÏÀÎÁö È®ÀÎÇØ ÁÖ¼¼¿ä."
        return false
    }

    rate := (totalCurrent != 0) ? (totalCommit / totalCurrent * 100) : 0
    paidRate := (totalCurrent != 0) ? (totalPaid / totalCurrent * 100) : 0
    balance := totalCurrent - totalCommit
    paidBalance := totalCurrent - totalPaid
    draftRate := (totalCurrent != 0) ? (totalDraft / totalCurrent * 100) : 0
    draftBalance := totalCurrent - totalDraft
    resourceTotal := totalPurpose + totalGeneral
    ...
    if (sourceTotalFound)
    {
        if (Abs(totalCurrent - sourceCurrent) < 0.5
            ...
            && Abs(totalPaid - sourcePaid) < 0.5)
            verifyText := "¿øº» ÃÑ°è ÀÏÄ¡"
        else
            verifyText := "¿øº» ÃÑ°è ºÒÀÏÄ¡"
    }
    else
    {
        verifyText := "¿øº» ÃÑ°è Çà ¾øÀ½"
    }

    if (sourceTotalFound && colSettle && colGeneral)
    {
        if (Abs(totalPurpose - sourcePurpose) < 0.5 && Abs(totalGeneral - sourceGeneral) < 0.5)
            resourceVerifyText := "Àç¿ø ÃÑ°è ÀÏÄ¡"
        else
            resourceVerifyText := "Àç¿ø ÃÑ°è È®ÀÎ ÇÊ¿ä"
    }
    else if (Abs(resourceTotal - totalCurrent) < 0.5)
        resourceVerifyText := "»êÃâ³»¿ª Àç¿ø±¸ºÐ ÀÏÄ¡"
    else
        resourceVerifyText := "»êÃâ³»¿ª Àç¿ø±¸ºÐ È®ÀÎ ÇÊ¿ä"
    ...
    parsed := {rows:rows
        ...
        , settleCommit:totalPurposeCommit
        , generalCommit:totalGeneralCommit
        , settlePaid:totalPurposePaid
        , generalPaid:totalGeneralPaid
        , purposeUserCurrent:purposeUserCurrent
        , purposeUserCommit:purposeUserCommit
        , purposeUserPaid:purposeUserPaid
        , purposeProjectCurrent:purposeProjectCurrent
        , purposeProjectCommit:purposeProjectCommit
        , purposeProjectPaid:purposeProjectPaid
        , purposeNationalCurrent:purposeNationalCurrent
        , purposeNationalCommit:purposeNationalCommit
        , purposeNationalPaid:purposeNationalPaid
        , generalIntegratedCurrent:generalIntegratedCurrent
        , generalIntegratedCommit:generalIntegratedCommit
        , generalIntegratedPaid:generalIntegratedPaid
        , generalFutureCurrent:generalFutureCurrent
        , generalFutureCommit:generalFutureCommit
        , generalFuturePaid:generalFuturePaid
        , generalSpecialCurrent:generalSpecialCurrent
        , generalSpecialCommit:generalSpecialCommit
        , generalSpecialPaid:generalSpecialPaid
        , generalPlainCurrent:generalPlainCurrent
        , generalPlainCommit:generalPlainCommit
        , generalPlainPaid:generalPlainPaid
...
SSOK_Expense_BudgetDashboard_GetResourceType(calc)
{
    ; ±âÁ¸ È£Ãâ È£È¯¿ë: »êÃâ³»¿ª¸¸À¸·Î ¸ñÀû/ÀÏ¹ÝÀ» ±¸ºÐÇÕ´Ï´Ù.
    text := calc . ""
    if RegExMatch(text, "[\(£¨]\s*¼ö\s*[\)£©]")
        return "¸ñÀûÀç¿ø"
    if RegExMatch(text, "[\(£¨]\s*¸ñ[^\)£©]*[\)£©]")
        return "¸ñÀûÀç¿ø"
    return "ÀÏ¹ÝÀç¿ø"
}

SSOK_Expense_BudgetDashboard_GetPurposeCategory(business, subItem, calc)
{
    text := calc . ""
    if RegExMatch(text, "[\(£¨]\s*¼ö\s*[\)£©]")
        return "¼öÀÍÀÚ"
    ...
    if RegExMatch(text, "[\(£¨]\s*¸ñ[^\)£©]*[\)£©]")
    {
        if (InStr(business . "", "[±¹°í]") || InStr(subItem . "", "[±¹°í]"))
            return "±¹°í»ç¾÷"
        return "¸ñÀû»ç¾÷"
    }
    return ""
...
SSOK_Expense_BudgetDashboard_GetGeneralCategory(calc)
{
    text := calc . ""
    if RegExMatch(text, "[\(£¨]\s*ÅëÇÕ[^\)£©]*(?:[\)£©]|$)")
        return "ÅëÇÕ"
    if RegExMatch(text, "[\(£¨]\s*(¹Ì·¡[^\)£©]*|¹Ì\s*\d*)\s*(?:[\)£©]|$)")
        return "¹Ì·¡"
    if RegExMatch(text, "[\(£¨]\s*Æ¯¼º[^\)£©]*[\)£©]")
        return "Æ¯¼º"
    return "ÀÏ¹Ý"
}

SSOK_Expense_BudgetDashboard_GetGeneralGroupName(calc, category)
{
    text := calc . ""
    if (category = "ÅëÇÕ")
    {
        if RegExMatch(text, "[\(£¨]\s*(ÅëÇÕ[^\)£©]*)(?:[\)£©]|$)", m)
            return "(" . Trim(m1) . ")"
    }
    else if (category = "¹Ì·¡")
    {
        if RegExMatch(text, "[\(£¨]\s*(¹Ì·¡[^\)£©]*|¹Ì\s*\d*)\s*(?:[\)£©]|$)", m)
            return "(" . Trim(m1) . ")"
    }
    else if (category = "Æ¯¼º")
    {
        if RegExMatch(text, "[\(£¨]\s*(Æ¯¼º[^\)£©]*)[\)£©]", m)
            return "(" . Trim(m1) . ")"
    }
    return "ÀÏ¹Ý"
}

SSOK_Expense_BudgetDashboard_GetIntegratedName(calc)
{
    text := calc . ""
    if RegExMatch(text, "[\(£¨]\s*(ÅëÇÕ[^\)£©]*)(?:[\)£©]|$)", m)
    {
...
SSOK_Expense_BudgetDashboard_ShowDashboard()
{
    global SSOK_BudgetDashboardPurposeAllLabel, SSOK_BudgetDashboardPurposeAllView, SSOK_BudgetDashboardPurposeAllSort, SSOK_BudgetDashboardPurposeAllLV, SSOK_BudgetDashboardPurposeAllStatus, SSOK_BudgetDashboardPurposeAllExport, SSOK_BudgetDashboardGeneralAllLabel, SSOK_BudgetDashboardGeneralAllView, SSOK_BudgetDashboardGeneralAllSort, SSOK_BudgetDashboardGeneralAllLV, SSOK_BudgetDashboardGeneralAllStatus, SSOK_BudgetDashboardGeneralAllExport, SSOK_BudgetDashboardWorkAllLV, SSOK_BudgetDashboardWorkAllStatus, SSOK_BudgetDashboardWorkAllExport, SSOK_BudgetDashboardPurposeTotal, SSOK_BudgetDashboardGeneralTotal, SSOK_BudgetDashboardWorkTotal
    global SSOK_BudgetDashboardPurposeProjectExport, SSOK_BudgetDashboardPurposeProjectLabel, SSOK_BudgetDashboardPurposeNationalExport, SSOK_BudgetDashboardPurposeNationalLabel, SSOK_BudgetDashboardPurposeUserExport, SSOK_BudgetDashboardPurposeUserLabel, SSOK_BudgetDashboardFutureExport, SSOK_BudgetDashboardFutureLabel, SSOK_BudgetDashboardSpecialExport, SSOK_BudgetDashboardSpecialLabel, SSOK_BudgetDashboardIntegratedExport, SSOK_BudgetDashboardIntegratedLabel, SSOK_BudgetDashboardGeneralExport, SSOK_BudgetDashboardGeneralLabel, SSOK_BudgetDashboardWorkGeneralExport, SSOK_BudgetDashboardWorkPurposeExport
    global SSOK_BudgetDashboardFileDrop, SSOK_BudgetDashboardPendingLV, SSOK_BudgetDashboardUnusedLV
    global SSOK_BudgetDashboardPendingView, SSOK_BudgetDashboardOverView, SSOK_BudgetDashboardUnusedView
    global SSOK_BudgetDashboardOverLV, SSOK_BudgetDashboardOverStatus
    global SSOK_BudgetDashboardSubtotal, SSOK_BudgetDashboardCostLV, SSOK_BudgetDashboardCostStatus
    global SSOK_BudgetDashboardCostResource, SSOK_BudgetDashboardCostSort
    global SSOK_BudgetDashboardPurposeProjectSort, SSOK_BudgetDashboardPurposeNationalSort, SSOK_BudgetDashboardPurposeUserSort, SSOK_BudgetDashboardFutureSort, SSOK_BudgetDashboardSpecialSort, SSOK_BudgetDashboardIntegratedSort, SSOK_BudgetDashboardGeneralSort
    global SSOK_BudgetDashboardParsed, SSOK_BudgetDashboardHwnd, SSOK_BudgetDashboardFilePath
    global SSOK_BudgetDashboardViewChoice, SSOK_BudgetDashboardSearch, SSOK_BudgetDashboardManagerSearch
    ...
    global SSOK_BudgetDashboardTab, SSOK_BudgetDashboardMainLV
    global SSOK_BudgetDashboardPurposeSubTab
    global SSOK_BudgetDashboardPurposeUserLV, SSOK_BudgetDashboardPurposeUserStatus, SSOK_BudgetDashboardPurposeUserView
    global SSOK_BudgetDashboardPurposeProjectLV, SSOK_BudgetDashboardPurposeProjectStatus, SSOK_BudgetDashboardPurposeProjectView
    global SSOK_BudgetDashboardPurposeNationalLV, SSOK_BudgetDashboardPurposeNationalStatus, SSOK_BudgetDashboardPurposeNationalView
    global SSOK_BudgetDashboardGeneralSubTab
    global SSOK_BudgetDashboardFutureLV, SSOK_BudgetDashboardFutureStatus, SSOK_BudgetDashboardFutureView
    global SSOK_BudgetDashboardSpecialLV, SSOK_BudgetDashboardSpecialStatus, SSOK_BudgetDashboardSpecialView
    global SSOK_BudgetDashboardIntegratedLV, SSOK_BudgetDashboardIntegratedStatus, SSOK_BudgetDashboardIntegratedView
    global SSOK_BudgetDashboardGeneralLV, SSOK_BudgetDashboardGeneralStatus, SSOK_BudgetDashboardGeneralView
    global SSOK_BudgetDashboardWorkSubTab
    global SSOK_BudgetDashboardWorkGeneralLV, SSOK_BudgetDashboardWorkGeneralStatus
    global SSOK_BudgetDashboardWorkPurposeLV, SSOK_BudgetDashboardWorkPurposeStatus
    ...
    Gui, SSOKBudgetDashboard:Destroy
    global SSOK_BudgetDashboardSummaryControls
    SSOK_BudgetDashboardSummaryControls := {}
    global SSOK_BudgetDashboardHeaders
    SSOK_BudgetDashboardHeaders := {}
    global SSOK_BudgetDashboardRowColors
    SSOK_BudgetDashboardRowColors := {}
    global SSOK_BudgetDashboardFonts, SSOK_BudgetDashboardButtons, SSOK_BudgetDashboardButtonGroups, SSOK_BudgetDashboardHeaderSort
    for _, font in SSOK_BudgetDashboardFonts
        DllCall("DeleteObject", "Ptr", font)
    SSOK_BudgetDashboardFonts := {}
    SSOK_BudgetDashboardButtons := {}
    SSOK_BudgetDashboardButtonGroups := {}
    SSOK_BudgetDashboardHeaderSort := {}
    OnMessage(0x4E, Func("SSOK_Expense_BudgetDashboard_CustomDraw"))
    p := SSOK_BudgetDashboardParsed
    dashboardTitle := SSOK_Expense_BudgetDashboard_Title()
    SSOK_BudgetDashboardViewChoice := "¼¼ºÎÇ×¸ñº°"
    SSOK_BudgetDashboardSearch := ""
    SSOK_BudgetDashboardManagerSearch := ""
    global SSOK_BudgetDashboardKpiControls, SSOK_BudgetDashboardScopeStatus
    SSOK_BudgetDashboardKpiControls := {}
    SSOK_BudgetDashboardSortChoice := "Çö¾× Å«¼ø"
    SSOK_BudgetDashboardResourceChoice := "ÀüÃ¼"
    SSOK_BudgetDashboardSubtotal := 0
    SSOK_BudgetDashboardHideZero := 0
    SSOK_BudgetDashboardOverOnly := 0
    global SSOK_BudgetDashboardLastSearch, SSOK_BudgetDashboardLastOver
    SSOK_BudgetDashboardLastSearch := Chr(30)
    SSOK_BudgetDashboardLastOver := 0
    SSOK_BudgetDashboardPurposeProjectView := "¼¼ºÎ³»¿ªº°"
    SSOK_BudgetDashboardPurposeNationalView := "¼¼ºÎ³»¿ªº°"
    SSOK_BudgetDashboardPurposeUserView := "¼¼ºÎ³»¿ªº°"
    SSOK_BudgetDashboardFutureView := "¼¼ºÎ³»¿ªº°"
    SSOK_BudgetDashboardSpecialView := "¼¼ºÎ³»¿ªº°"
    SSOK_BudgetDashboardIntegratedView := "¼¼ºÎ³»¿ªº°"
    SSOK_BudgetDashboardGeneralView := "¼¼ºÎ³»¿ªº°"

    SplitPath, SSOK_BudgetDashboardFilePath, sourceName

    Gui, SSOKBudgetDashboard:Destroy
    Gui, SSOKBudgetDashboard:New, -ToolWindow +MinimizeBox +HwndSSOK_BudgetDashboardHwnd, ¿¹»ê DashBoard
    SSOK_Expense_BudgetDashboard_SetIcon(SSOK_BudgetDashboardHwnd)
    Gui, SSOKBudgetDashboard:Margin, 18, 12
    Gui, SSOKBudgetDashboard:Color, F7FBFF

    Gui, SSOKBudgetDashboard:Font, s13 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, Text, x18 y10 w710 h28 c005BAC, %dashboardTitle%
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, Text, x18 y32 w900 h18 c555555 vSSOK_BudgetDashboardScopeStatus, % "»êÃâ³»¿ª " . p.rowCount . "Çà  |  ÃÊ°úÁýÇà " . p.overCount . "°Ç  |  " . p.verifyText . "  |  " . p.resourceVerifyText
    Gui, SSOKBudgetDashboard:Add, Text, x940 y32 w540 h18 Right c777777, %sourceName%

    balanceColor := (p.balance < 0) ? "C62828" : "555555"
    paidBalanceColor := (p.paidBalance < 0) ? "C62828" : "555555"

    ; °¢ ¹Ú½º: ÀüÃ¼°ª + ÀÏ¹ÝÀç¿ø/¸ñÀûÀç¿ø °ªÀ» °°Àº Æ² ¾È¿¡ Ç¥½Ã
    SSOK_Expense_BudgetDashboard_AddSplitKpi(18, 50, 237, "¿¹»êÇö¾×"
        , SSOK_Expense_FormatNumber(p.current) . "¿ø"
        , "ÀÏ¹Ý " . SSOK_Expense_FormatNumber(p.general) . "¿ø"
        , "¸ñÀû " . SSOK_Expense_FormatNumber(p.settle) . "¿ø", "005BAC")
    SSOK_Expense_BudgetDashboard_AddSplitKpi(263, 50, 237, "°è¾à±Ý¾× (¿øÀÎÇàÀ§)"
        , SSOK_Expense_FormatNumber(p.commit) . "¿ø"
        , "ÀÏ¹Ý " . SSOK_Expense_FormatNumber(p.generalCommit) . "¿ø"
        , "¸ñÀû " . SSOK_Expense_FormatNumber(p.settleCommit) . "¿ø", "6A5ACD")
    SSOK_Expense_BudgetDashboard_AddSplitKpi(508, 50, 237, "Áö±Þ¾×"
        , SSOK_Expense_FormatNumber(p.paid) . "¿ø"
        , "ÀÏ¹Ý " . SSOK_Expense_FormatNumber(p.generalPaid) . "¿ø"
        , "¸ñÀû " . SSOK_Expense_FormatNumber(p.settlePaid) . "¿ø", "3A6EA5")
    SSOK_Expense_BudgetDashboard_AddSplitKpi(753, 50, 237, "¹ÌÁöÃâ¾× (Áö±Þ´ë±â)"
        , SSOK_Expense_FormatNumber(p.commit - p.paid) . "¿ø"
        , "ÀÏ¹Ý " . SSOK_Expense_FormatNumber(p.generalCommit - p.generalPaid) . "¿ø"
        , "¸ñÀû " . SSOK_Expense_FormatNumber(p.settleCommit - p.settlePaid) . "¿ø", "3A6EA5")
    SSOK_Expense_BudgetDashboard_AddSplitKpi(998, 50, 237, "¿¹»êÀÜ¾× (ºÒ¿ë¿¹»ê¾×)"
        , SSOK_Expense_FormatNumber(p.current - p.commit) . "¿ø"
        , "ÀÏ¹Ý " . SSOK_Expense_FormatNumber(p.general - p.generalCommit) . "¿ø"
        , "¸ñÀû " . SSOK_Expense_FormatNumber(p.settle - p.settleCommit) . "¿ø", "555555")
    SSOK_Expense_BudgetDashboard_AddSplitKpi(1243, 50, 237, "ÁýÇà·ü (¿øÀÎÇàÀ§)"
        , SSOK_Expense_BudgetDashboard_FormatRate(p.current, p.commit)
        , "ÀÏ¹Ý " . SSOK_Expense_BudgetDashboard_FormatRate(p.general, p.generalCommit)
        , "¸ñÀû " . SSOK_Expense_BudgetDashboard_FormatRate(p.settle, p.settleCommit), "00856A")

    Gui, SSOKBudgetDashboard:Font, s9 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Font, s8 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Font, s9 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, Text, x18 y928 w1462 h44 +Border Center 0x200 vSSOK_BudgetDashboardFileDrop gSSOK_Expense_BudgetDashboard_PickFile c005BAC, »ç¾÷°ü¸®Ä«µå(Çö¾×) ¶Ç´Â ÁýÇà½ÇÀû xls / xlsx ÆÄÀÏÀ» ¿©±â¿¡ ºÙ¿© ³õ¾ÆÁÖ¼¼¿ä (Å¬¸¯ÇÏ¿© ¼±ÅÃ)
    Gui, SSOKBudgetDashboard:Add, Tab3, x18 y174 w1462 h748 vSSOK_BudgetDashboardTab gSSOK_Expense_BudgetDashboard_SelectSection, ÀüÃ¼ ÇöÈ²|¸ñÀûÀç¿ø °ü¸®|ÀÏ¹ÝÀç¿ø °ü¸®|¾÷¹«ÃßÁøºñ °ü¸®|¹ÌÁöÃâ¾× °ü¸®|ÃÊ°úÁýÇà °ü¸®|ºÒ¿ë¾× °ü¸®|¿ø°¡Åë°èºñ¸ñº° ÇÕ°è

    ; ÀüÃ¼ ÇöÈ²
    Gui, SSOKBudgetDashboard:Tab, 1, 1
    Gui, SSOKBudgetDashboard:Font, s8 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x68 y175 w180 vSSOK_BudgetDashboardViewChoice gSSOK_Expense_BudgetDashboard_Refresh, ¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°||»êÃâ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°|¼¼ºÎÇ×¸ñ´ã´çÀÚº°|Àç¿øº°|Àç¿ø±¸ºÐº°
    SSOK_Expense_BudgetDashboard_AddChoiceButtons("SSOK_BudgetDashboardViewChoice", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|»êÃâ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", 32, 212, 116, "view")
    Gui, SSOKBudgetDashboard:Font, s8 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Font, s8 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x594 y175 w190 vSSOK_BudgetDashboardSortChoice gSSOK_Expense_BudgetDashboard_Refresh, Çö¾× Å«¼ø||Ç°ÀÇ¾× Å«¼ø|¿øÀÎÇàÀ§ Å«¼ø|ÁýÇà·ü ³ôÀº¼ø|ÁýÇà·ü ³·Àº¼ø|¸ñÀûÀç¿ø Å«¼ø|¸ñÀû ÁýÇà·ü ³ôÀº¼ø|ÀÏ¹ÝÀç¿ø Å«¼ø|ÀÏ¹Ý ÁýÇà·ü ³ôÀº¼ø|ÀÌ¸§¼ø
    Gui, SSOKBudgetDashboard:Font, s8 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x832 y175 w105 vSSOK_BudgetDashboardResourceChoice gSSOK_Expense_BudgetDashboard_Refresh, ÀüÃ¼||¸ñÀûÀç¿ø|ÀÏ¹ÝÀç¿ø
    SSOK_Expense_BudgetDashboard_AddChoiceButtons("SSOK_BudgetDashboardResourceChoice", "ÀüÃ¼|ÀÏ¹ÝÀç¿ø|¸ñÀûÀç¿ø", "ÀüÃ¼|ÀÏ¹ÝÀç¿ø|¸ñÀûÀç¿ø", 530, 212, 50, "resource")
    Gui, SSOKBudgetDashboard:Add, CheckBox, x310 y246 w100 h24 vSSOK_BudgetDashboardHideZero gSSOK_Expense_BudgetDashboard_Refresh, 0¿ø Çà Á¦¿Ü
    Gui, SSOKBudgetDashboard:Add, CheckBox, x430 y246 w105 h24 vSSOK_BudgetDashboardOverOnly gSSOK_Expense_BudgetDashboard_Refresh, ÃÊ°úÁýÇà¸¸
    Gui, SSOKBudgetDashboard:Add, Button, x1248 y208 w86 h30 gSSOK_Expense_BudgetDashboard_ExportOverall, Excel ÃßÃâ
    Gui, SSOKBudgetDashboard:Add, Button, x1340 y208 w126 h30 gSSOK_Expense_BudgetDashboard_ExportExe, ¹èÆ÷¿ë EXE ¸¸µé±â
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, CheckBox, x32 y246 w260 h24 vSSOK_BudgetDashboardSubtotal gSSOK_Expense_BudgetDashboard_Refresh, Àç¿ø±¸ºÐº° ÇÕ°èº¸±â
    Gui, SSOKBudgetDashboard:Add, ListView, x32 y304 w1434 h528 Grid -Multi NoSort gSSOK_Expense_BudgetDashboard_HeaderClick vSSOK_BudgetDashboardMainLV, ¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|°è¾à±Ý¾× (¿øÀÎÇàÀ§)|Áö±Þ¾×|¹ÌÁöÃâ¾× (Áö±Þ´ë±â)|¿¹»êÀÜ¾× (ºÒ¿ë¿¹»ê¾×)|ÁýÇà·ü (¿øÀÎÇàÀ§)|¸ñÀûÀç¿ø|ÀÏ¹ÝÀç¿ø|´ã´çÀÚ
    SSOK_Expense_BudgetDashboard_AddTopSummary("SSOK_BudgetDashboardMainLV", [120,120,165,100,100,100,100,105,110,85,85,85,140], 32, 276, 1434, 556)
    Gui, SSOKBudgetDashboard:Add, Text, x32 y838 w1434 h34 +0x200 vSSOK_BudgetDashboardListStatus Hidden c555555,

    ; ¸ñÀûÀç¿ø: ¸ñÀû»ç¾÷ / ±¹°íº¸Á¶±Ý / ¼öÀÍÀÚ
    Gui, SSOKBudgetDashboard:Tab, 2, 1
    Gui, SSOKBudgetDashboard:Font, s8 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x46 y214 w260 vSSOK_BudgetDashboardPurposeSubTab gSSOK_Expense_BudgetDashboard_SelectSection, ÀüÃ¼||¸ñÀû»ç¾÷|±¹°íº¸Á¶±Ý|¼öÀÍÀÚ
    SSOK_Expense_BudgetDashboard_AddChoiceButtons("SSOK_BudgetDashboardPurposeSubTab", "ÀüÃ¼|¸ñÀû»ç¾÷ºñ|±¹°íº¸Á¶±Ý|¼öÀÍÀÚºÎ´ã±Ý", "ÀüÃ¼|¸ñÀû»ç¾÷|±¹°íº¸Á¶±Ý|¼öÀÍÀÚ", 46, 212, 100, "section")
    Gui, SSOKBudgetDashboard:Add, Text, x560 y214 w890 h28 +0x200 c005BAC vSSOK_BudgetDashboardPurposeTotal,

    ; ¸ñÀûÀç¿ø > ¸ñÀû»ç¾÷
    Gui, SSOKBudgetDashboard:Tab, 2, 1
    Gui, SSOKBudgetDashboard:Font, s8 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, Text, x46 y252 w36 h24 +0x200 vSSOK_BudgetDashboardPurposeAllLabel, º¸±â
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x84 y250 w140 vSSOK_BudgetDashboardPurposeAllView gSSOK_Expense_BudgetDashboard_ResourceViewRefresh, ¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°||¿ø°¡Åë°èºñ¸ñº°
    SSOK_Expense_BudgetDashboard_AddChoiceButtons("SSOK_BudgetDashboardPurposeAllView", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", 84, 248, 116, "view")
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x560 y250 w190 vSSOK_BudgetDashboardPurposeAllSort gSSOK_Expense_BudgetDashboard_ResourceViewRefresh, ÀÌ¸§¼ø||Çö¾× Å«¼ø|¿øÀÎÇàÀ§ Å«¼ø|Áö±Þ¾× Å«¼ø|ÀÜ¾× Å«¼ø|ÁýÇà·ü ³ôÀº¼ø|ÁýÇà·ü ³·Àº¼ø
    Gui, SSOKBudgetDashboard:Add, ListView, x46 y312 w1406 h489 Grid -Multi NoSort gSSOK_Expense_BudgetDashboard_HeaderClick vSSOK_BudgetDashboardPurposeAllLV, ¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|°è¾à±Ý¾× (¿øÀÎÇàÀ§)|Áö±Þ¾×|¹ÌÁöÃâ¾× (Áö±Þ´ë±â)|¿¹»êÀÜ¾× (ºÒ¿ë¿¹»ê¾×)|ÁýÇà·ü (¿øÀÎÇàÀ§)|´ã´çÀÚ
    SSOK_Expense_BudgetDashboard_AddTopSummary("SSOK_BudgetDashboardPurposeAllLV", [130,130,200,115,100,105,100,105,110,100,170], 46, 284, 1406, 517)
    Gui, SSOKBudgetDashboard:Add, Text, x46 y810 w1190 h58 +0x200 vSSOK_BudgetDashboardPurposeAllStatus Hidden c555555,
    Gui, SSOKBudgetDashboard:Add, Button, x1270 y823 w168 h34 vSSOK_BudgetDashboardPurposeAllExport gSSOK_Expense_BudgetDashboard_ExportPurposeAll, ÀÌ ÅÇ Excel ÃßÃâ

    ; ¸ñÀûÀç¿ø > ±¹°íº¸Á¶±Ý
    Gui, SSOKBudgetDashboard:Tab, 2, 1
    Gui, SSOKBudgetDashboard:Font, s8 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, Text, x46 y252 w36 h24 +0x200 vSSOK_BudgetDashboardPurposeProjectLabel, º¸±â
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x84 y250 w140 vSSOK_BudgetDashboardPurposeProjectView gSSOK_Expense_BudgetDashboard_ResourceViewRefresh, ¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°||¿ø°¡Åë°èºñ¸ñº°
    SSOK_Expense_BudgetDashboard_AddChoiceButtons("SSOK_BudgetDashboardPurposeProjectView", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", 84, 248, 116, "view")
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x560 y250 w190 vSSOK_BudgetDashboardPurposeProjectSort gSSOK_Expense_BudgetDashboard_ResourceViewRefresh, ÀÌ¸§¼ø||Çö¾× Å«¼ø|¿øÀÎÇàÀ§ Å«¼ø|Áö±Þ¾× Å«¼ø|ÀÜ¾× Å«¼ø|ÁýÇà·ü ³ôÀº¼ø|ÁýÇà·ü ³·Àº¼ø
    Gui, SSOKBudgetDashboard:Add, ListView, x46 y312 w1406 h489 Grid -Multi NoSort gSSOK_Expense_BudgetDashboard_HeaderClick vSSOK_BudgetDashboardPurposeProjectLV, ¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|°è¾à±Ý¾× (¿øÀÎÇàÀ§)|Áö±Þ¾×|¹ÌÁöÃâ¾× (Áö±Þ´ë±â)|¿¹»êÀÜ¾× (ºÒ¿ë¿¹»ê¾×)|ÁýÇà·ü (¿øÀÎÇàÀ§)|´ã´çÀÚ
    SSOK_Expense_BudgetDashboard_AddTopSummary("SSOK_BudgetDashboardPurposeProjectLV", [130,130,200,115,100,105,100,105,110,100,170], 46, 284, 1406, 517)
    Gui, SSOKBudgetDashboard:Add, Text, x46 y810 w1190 h58 +0x200 vSSOK_BudgetDashboardPurposeProjectStatus Hidden c555555,
    Gui, SSOKBudgetDashboard:Add, Button, x1270 y823 w168 h34 vSSOK_BudgetDashboardPurposeProjectExport gSSOK_Expense_BudgetDashboard_ExportPurposeProject, ÀÌ ÅÇ Excel ÃßÃâ

    ; ¸ñÀûÀç¿ø > ±¹°íº¸Á¶±Ý
    Gui, SSOKBudgetDashboard:Tab, 2, 1
    Gui, SSOKBudgetDashboard:Font, s8 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, Text, x46 y252 w36 h24 +0x200 vSSOK_BudgetDashboardPurposeNationalLabel, º¸±â
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x84 y250 w140 vSSOK_BudgetDashboardPurposeNationalView gSSOK_Expense_BudgetDashboard_ResourceViewRefresh, ¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°||¿ø°¡Åë°èºñ¸ñº°
    SSOK_Expense_BudgetDashboard_AddChoiceButtons("SSOK_BudgetDashboardPurposeNationalView", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", 84, 248, 116, "view")
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x560 y250 w190 vSSOK_BudgetDashboardPurposeNationalSort gSSOK_Expense_BudgetDashboard_ResourceViewRefresh, ÀÌ¸§¼ø||Çö¾× Å«¼ø|¿øÀÎÇàÀ§ Å«¼ø|Áö±Þ¾× Å«¼ø|ÀÜ¾× Å«¼ø|ÁýÇà·ü ³ôÀº¼ø|ÁýÇà·ü ³·Àº¼ø
    Gui, SSOKBudgetDashboard:Add, ListView, x46 y312 w1406 h489 Grid -Multi NoSort gSSOK_Expense_BudgetDashboard_HeaderClick vSSOK_BudgetDashboardPurposeNationalLV, ¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|°è¾à±Ý¾× (¿øÀÎÇàÀ§)|Áö±Þ¾×|¹ÌÁöÃâ¾× (Áö±Þ´ë±â)|¿¹»êÀÜ¾× (ºÒ¿ë¿¹»ê¾×)|ÁýÇà·ü (¿øÀÎÇàÀ§)|´ã´çÀÚ
    SSOK_Expense_BudgetDashboard_AddTopSummary("SSOK_BudgetDashboardPurposeNationalLV", [130,130,200,115,100,105,100,105,110,100,170], 46, 284, 1406, 517)
    Gui, SSOKBudgetDashboard:Add, Text, x46 y810 w1190 h58 +0x200 vSSOK_BudgetDashboardPurposeNationalStatus Hidden c555555,
    Gui, SSOKBudgetDashboard:Add, Button, x1270 y823 w168 h34 vSSOK_BudgetDashboardPurposeNationalExport gSSOK_Expense_BudgetDashboard_ExportPurposeNational, ÀÌ ÅÇ Excel ÃßÃâ

    ; ¸ñÀûÀç¿ø > ¼öÀÍÀÚ
    Gui, SSOKBudgetDashboard:Tab, 2, 1
    Gui, SSOKBudgetDashboard:Font, s8 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, Text, x46 y252 w36 h24 +0x200 vSSOK_BudgetDashboardPurposeUserLabel, º¸±â
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x84 y250 w140 vSSOK_BudgetDashboardPurposeUserView gSSOK_Expense_BudgetDashboard_ResourceViewRefresh, ¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°||¿ø°¡Åë°èºñ¸ñº°
    SSOK_Expense_BudgetDashboard_AddChoiceButtons("SSOK_BudgetDashboardPurposeUserView", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", 84, 248, 116, "view")
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x560 y250 w190 vSSOK_BudgetDashboardPurposeUserSort gSSOK_Expense_BudgetDashboard_ResourceViewRefresh, ÀÌ¸§¼ø||Çö¾× Å«¼ø|¿øÀÎÇàÀ§ Å«¼ø|Áö±Þ¾× Å«¼ø|ÀÜ¾× Å«¼ø|ÁýÇà·ü ³ôÀº¼ø|ÁýÇà·ü ³·Àº¼ø
    Gui, SSOKBudgetDashboard:Add, ListView, x46 y312 w1406 h489 Grid -Multi NoSort gSSOK_Expense_BudgetDashboard_HeaderClick vSSOK_BudgetDashboardPurposeUserLV, ¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|°è¾à±Ý¾× (¿øÀÎÇàÀ§)|Áö±Þ¾×|¹ÌÁöÃâ¾× (Áö±Þ´ë±â)|¿¹»êÀÜ¾× (ºÒ¿ë¿¹»ê¾×)|ÁýÇà·ü (¿øÀÎÇàÀ§)|´ã´çÀÚ
    SSOK_Expense_BudgetDashboard_AddTopSummary("SSOK_BudgetDashboardPurposeUserLV", [130,130,200,115,100,105,100,105,110,100,170], 46, 284, 1406, 517)
    Gui, SSOKBudgetDashboard:Add, Text, x46 y810 w1190 h58 +0x200 vSSOK_BudgetDashboardPurposeUserStatus Hidden c555555,
    Gui, SSOKBudgetDashboard:Add, Button, x1270 y823 w168 h34 vSSOK_BudgetDashboardPurposeUserExport gSSOK_Expense_BudgetDashboard_ExportPurposeUser, ÀÌ ÅÇ Excel ÃßÃâ

    ; ÀÏ¹ÝÀç¿ø: ¹Ì·¡ / Æ¯¼º / ÅëÇÕ / ÀÏ¹Ý
    Gui, SSOKBudgetDashboard:Tab, 3, 1
    Gui, SSOKBudgetDashboard:Font, s8 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x46 y214 w260 vSSOK_BudgetDashboardGeneralSubTab gSSOK_Expense_BudgetDashboard_SelectSection, ÀüÃ¼||¹Ì·¡|Æ¯¼º|ÅëÇÕ|ÀÏ¹Ý
    SSOK_Expense_BudgetDashboard_AddChoiceButtons("SSOK_BudgetDashboardGeneralSubTab", "ÀüÃ¼|¹Ì·¡|Æ¯¼º|ÅëÇÕ|ÀÏ¹Ý", "ÀüÃ¼|¹Ì·¡|Æ¯¼º|ÅëÇÕ|ÀÏ¹Ý", 46, 212, 100, "section")
    Gui, SSOKBudgetDashboard:Add, Text, x560 y214 w890 h28 +0x200 c005BAC vSSOK_BudgetDashboardGeneralTotal,

    ; ÀÏ¹ÝÀç¿ø > ¹Ì·¡
    Gui, SSOKBudgetDashboard:Tab, 3, 1
    Gui, SSOKBudgetDashboard:Font, s8 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, Text, x46 y252 w36 h24 +0x200 vSSOK_BudgetDashboardGeneralAllLabel, º¸±â
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x84 y250 w140 vSSOK_BudgetDashboardGeneralAllView gSSOK_Expense_BudgetDashboard_ResourceViewRefresh, ¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°||¿ø°¡Åë°èºñ¸ñº°
    SSOK_Expense_BudgetDashboard_AddChoiceButtons("SSOK_BudgetDashboardGeneralAllView", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", 84, 248, 116, "view")
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x560 y250 w190 vSSOK_BudgetDashboardGeneralAllSort gSSOK_Expense_BudgetDashboard_ResourceViewRefresh, ÀÌ¸§¼ø||Çö¾× Å«¼ø|¿øÀÎÇàÀ§ Å«¼ø|Áö±Þ¾× Å«¼ø|ÀÜ¾× Å«¼ø|ÁýÇà·ü ³ôÀº¼ø|ÁýÇà·ü ³·Àº¼ø
    Gui, SSOKBudgetDashboard:Add, ListView, x46 y312 w1406 h489 Grid -Multi NoSort gSSOK_Expense_BudgetDashboard_HeaderClick vSSOK_BudgetDashboardGeneralAllLV, ¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|°è¾à±Ý¾× (¿øÀÎÇàÀ§)|Áö±Þ¾×|¹ÌÁöÃâ¾× (Áö±Þ´ë±â)|¿¹»êÀÜ¾× (ºÒ¿ë¿¹»ê¾×)|ÁýÇà·ü (¿øÀÎÇàÀ§)|´ã´çÀÚ
    SSOK_Expense_BudgetDashboard_AddTopSummary("SSOK_BudgetDashboardGeneralAllLV", [130,130,200,115,100,105,100,105,110,100,170], 46, 284, 1406, 517)
    Gui, SSOKBudgetDashboard:Add, Text, x46 y810 w1190 h58 +0x200 vSSOK_BudgetDashboardGeneralAllStatus Hidden c555555,
    Gui, SSOKBudgetDashboard:Add, Button, x1270 y823 w168 h34 vSSOK_BudgetDashboardGeneralAllExport gSSOK_Expense_BudgetDashboard_ExportGeneralAll, ÀÌ ÅÇ Excel ÃßÃâ

    ; ÀÏ¹ÝÀç¿ø > Æ¯¼º
    Gui, SSOKBudgetDashboard:Tab, 3, 1
    Gui, SSOKBudgetDashboard:Font, s8 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, Text, x46 y252 w36 h24 +0x200 vSSOK_BudgetDashboardFutureLabel, º¸±â
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x84 y250 w140 vSSOK_BudgetDashboardFutureView gSSOK_Expense_BudgetDashboard_ResourceViewRefresh, ¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°||¿ø°¡Åë°èºñ¸ñº°
    SSOK_Expense_BudgetDashboard_AddChoiceButtons("SSOK_BudgetDashboardFutureView", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", 84, 248, 116, "view")
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x560 y250 w190 vSSOK_BudgetDashboardFutureSort gSSOK_Expense_BudgetDashboard_ResourceViewRefresh, ÀÌ¸§¼ø||Çö¾× Å«¼ø|¿øÀÎÇàÀ§ Å«¼ø|Áö±Þ¾× Å«¼ø|ÀÜ¾× Å«¼ø|ÁýÇà·ü ³ôÀº¼ø|ÁýÇà·ü ³·Àº¼ø
    Gui, SSOKBudgetDashboard:Add, ListView, x46 y312 w1406 h489 Grid -Multi NoSort gSSOK_Expense_BudgetDashboard_HeaderClick vSSOK_BudgetDashboardFutureLV, ¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|°è¾à±Ý¾× (¿øÀÎÇàÀ§)|Áö±Þ¾×|¹ÌÁöÃâ¾× (Áö±Þ´ë±â)|¿¹»êÀÜ¾× (ºÒ¿ë¿¹»ê¾×)|ÁýÇà·ü (¿øÀÎÇàÀ§)|´ã´çÀÚ
    SSOK_Expense_BudgetDashboard_AddTopSummary("SSOK_BudgetDashboardFutureLV", [130,130,200,115,100,105,100,105,110,100,170], 46, 284, 1406, 517)
    Gui, SSOKBudgetDashboard:Add, Text, x46 y810 w1190 h58 +0x200 vSSOK_BudgetDashboardFutureStatus Hidden c555555,
    Gui, SSOKBudgetDashboard:Add, Button, x1270 y823 w168 h34 vSSOK_BudgetDashboardFutureExport gSSOK_Expense_BudgetDashboard_ExportGeneralFuture, ÀÌ ÅÇ Excel ÃßÃâ

    ; ÀÏ¹ÝÀç¿ø > Æ¯¼º
    Gui, SSOKBudgetDashboard:Tab, 3, 1
    Gui, SSOKBudgetDashboard:Font, s8 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, Text, x46 y252 w36 h24 +0x200 vSSOK_BudgetDashboardSpecialLabel, º¸±â
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x84 y250 w140 vSSOK_BudgetDashboardSpecialView gSSOK_Expense_BudgetDashboard_ResourceViewRefresh, ¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°||¿ø°¡Åë°èºñ¸ñº°
    SSOK_Expense_BudgetDashboard_AddChoiceButtons("SSOK_BudgetDashboardSpecialView", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", 84, 248, 116, "view")
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x560 y250 w190 vSSOK_BudgetDashboardSpecialSort gSSOK_Expense_BudgetDashboard_ResourceViewRefresh, ÀÌ¸§¼ø||Çö¾× Å«¼ø|¿øÀÎÇàÀ§ Å«¼ø|Áö±Þ¾× Å«¼ø|ÀÜ¾× Å«¼ø|ÁýÇà·ü ³ôÀº¼ø|ÁýÇà·ü ³·Àº¼ø
    Gui, SSOKBudgetDashboard:Add, ListView, x46 y312 w1406 h489 Grid -Multi NoSort gSSOK_Expense_BudgetDashboard_HeaderClick vSSOK_BudgetDashboardSpecialLV, ¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|°è¾à±Ý¾× (¿øÀÎÇàÀ§)|Áö±Þ¾×|¹ÌÁöÃâ¾× (Áö±Þ´ë±â)|¿¹»êÀÜ¾× (ºÒ¿ë¿¹»ê¾×)|ÁýÇà·ü (¿øÀÎÇàÀ§)|´ã´çÀÚ
    SSOK_Expense_BudgetDashboard_AddTopSummary("SSOK_BudgetDashboardSpecialLV", [130,130,200,115,100,105,100,105,110,100,170], 46, 284, 1406, 517)
    Gui, SSOKBudgetDashboard:Add, Text, x46 y810 w1190 h58 +0x200 vSSOK_BudgetDashboardSpecialStatus Hidden c555555,
    Gui, SSOKBudgetDashboard:Add, Button, x1270 y823 w168 h34 vSSOK_BudgetDashboardSpecialExport gSSOK_Expense_BudgetDashboard_ExportGeneralSpecial, ÀÌ ÅÇ Excel ÃßÃâ

    ; ÀÏ¹ÝÀç¿ø > ÅëÇÕ
    Gui, SSOKBudgetDashboard:Tab, 3, 1
    Gui, SSOKBudgetDashboard:Font, s8 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, Text, x46 y252 w36 h24 +0x200 vSSOK_BudgetDashboardIntegratedLabel, º¸±â
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x84 y250 w140 vSSOK_BudgetDashboardIntegratedView gSSOK_Expense_BudgetDashboard_ResourceViewRefresh, ¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°||¿ø°¡Åë°èºñ¸ñº°
    SSOK_Expense_BudgetDashboard_AddChoiceButtons("SSOK_BudgetDashboardIntegratedView", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", 84, 248, 116, "view")
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x560 y250 w190 vSSOK_BudgetDashboardIntegratedSort gSSOK_Expense_BudgetDashboard_ResourceViewRefresh, ÀÌ¸§¼ø||Çö¾× Å«¼ø|¿øÀÎÇàÀ§ Å«¼ø|Áö±Þ¾× Å«¼ø|ÀÜ¾× Å«¼ø|ÁýÇà·ü ³ôÀº¼ø|ÁýÇà·ü ³·Àº¼ø
    Gui, SSOKBudgetDashboard:Add, ListView, x46 y312 w1406 h489 Grid -Multi NoSort gSSOK_Expense_BudgetDashboard_HeaderClick vSSOK_BudgetDashboardIntegratedLV, ¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|°è¾à±Ý¾× (¿øÀÎÇàÀ§)|Áö±Þ¾×|¹ÌÁöÃâ¾× (Áö±Þ´ë±â)|¿¹»êÀÜ¾× (ºÒ¿ë¿¹»ê¾×)|ÁýÇà·ü (¿øÀÎÇàÀ§)|´ã´çÀÚ
    SSOK_Expense_BudgetDashboard_AddTopSummary("SSOK_BudgetDashboardIntegratedLV", [130,130,200,115,100,105,100,105,110,100,170], 46, 284, 1406, 517)
    Gui, SSOKBudgetDashboard:Add, Text, x46 y810 w1190 h58 +0x200 vSSOK_BudgetDashboardIntegratedStatus Hidden c555555,
    Gui, SSOKBudgetDashboard:Add, Button, x1270 y823 w168 h34 vSSOK_BudgetDashboardIntegratedExport gSSOK_Expense_BudgetDashboard_ExportGeneralIntegrated, ÀÌ ÅÇ Excel ÃßÃâ

    ; ÀÏ¹ÝÀç¿ø > ÀÏ¹Ý
    Gui, SSOKBudgetDashboard:Tab, 3, 1
    Gui, SSOKBudgetDashboard:Font, s8 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, Text, x46 y252 w36 h24 +0x200 vSSOK_BudgetDashboardGeneralLabel, º¸±â
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x84 y250 w140 vSSOK_BudgetDashboardGeneralView gSSOK_Expense_BudgetDashboard_ResourceViewRefresh, ¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°||¿ø°¡Åë°èºñ¸ñº°
    SSOK_Expense_BudgetDashboard_AddChoiceButtons("SSOK_BudgetDashboardGeneralView", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|¼¼ºÎ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", 84, 248, 116, "view")
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x560 y250 w190 vSSOK_BudgetDashboardGeneralSort gSSOK_Expense_BudgetDashboard_ResourceViewRefresh, ÀÌ¸§¼ø||Çö¾× Å«¼ø|¿øÀÎÇàÀ§ Å«¼ø|Áö±Þ¾× Å«¼ø|ÀÜ¾× Å«¼ø|ÁýÇà·ü ³ôÀº¼ø|ÁýÇà·ü ³·Àº¼ø
    Gui, SSOKBudgetDashboard:Add, ListView, x46 y312 w1406 h489 Grid -Multi NoSort gSSOK_Expense_BudgetDashboard_HeaderClick vSSOK_BudgetDashboardGeneralLV, ¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|°è¾à±Ý¾× (¿øÀÎÇàÀ§)|Áö±Þ¾×|¹ÌÁöÃâ¾× (Áö±Þ´ë±â)|¿¹»êÀÜ¾× (ºÒ¿ë¿¹»ê¾×)|ÁýÇà·ü (¿øÀÎÇàÀ§)|´ã´çÀÚ
    SSOK_Expense_BudgetDashboard_AddTopSummary("SSOK_BudgetDashboardGeneralLV", [130,130,200,115,100,105,100,105,110,100,170], 46, 284, 1406, 517)
    Gui, SSOKBudgetDashboard:Add, Text, x46 y810 w1190 h58 +0x200 vSSOK_BudgetDashboardGeneralStatus Hidden c555555,
    Gui, SSOKBudgetDashboard:Add, Button, x1270 y823 w168 h34 vSSOK_BudgetDashboardGeneralExport gSSOK_Expense_BudgetDashboard_ExportGeneralPlain, ÀÌ ÅÇ Excel ÃßÃâ

    ; ¾÷¹«ÃßÁøºñ: ÀÏ¹Ý / ¸ñÀû
    Gui, SSOKBudgetDashboard:Tab, 4, 1
    Gui, SSOKBudgetDashboard:Font, s8 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x46 y214 w260 vSSOK_BudgetDashboardWorkSubTab gSSOK_Expense_BudgetDashboard_SelectSection, ÀüÃ¼||ÀÏ¹Ý|¸ñÀû
    SSOK_Expense_BudgetDashboard_AddChoiceButtons("SSOK_BudgetDashboardWorkSubTab", "ÀüÃ¼|ÀÏ¹Ý|¸ñÀû", "ÀüÃ¼|ÀÏ¹Ý|¸ñÀû", 46, 212, 100, "section")
    Gui, SSOKBudgetDashboard:Add, Text, x560 y214 w890 h28 +0x200 c005BAC vSSOK_BudgetDashboardWorkTotal,

    Gui, SSOKBudgetDashboard:Tab, 4, 1
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, ListView, x46 y282 w1406 h529 Grid -Multi NoSort gSSOK_Expense_BudgetDashboard_HeaderClick vSSOK_BudgetDashboardWorkAllLV, ¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|°è¾à±Ý¾× (¿øÀÎÇàÀ§)|Áö±Þ¾×|¹ÌÁöÃâ¾× (Áö±Þ´ë±â)|¿¹»êÀÜ¾× (ºÒ¿ë¿¹»ê¾×)|ÁýÇà·ü (¿øÀÎÇàÀ§)|´ã´çÀÚ
    SSOK_Expense_BudgetDashboard_AddTopSummary("SSOK_BudgetDashboardWorkAllLV", [160,160,235,130,95,95,105,95,95,95,95,170], 46, 254, 1406, 557)
    Gui, SSOKBudgetDashboard:Add, Text, x46 y818 w1190 h54 +0x200 vSSOK_BudgetDashboardWorkAllStatus Hidden c555555,
    Gui, SSOKBudgetDashboard:Add, Button, x1270 y829 w168 h34 vSSOK_BudgetDashboardWorkAllExport gSSOK_Expense_BudgetDashboard_ExportWorkAll, ÀÌ ÅÇ Excel ÃßÃâ

    Gui, SSOKBudgetDashboard:Tab, 4, 1
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, ListView, x46 y282 w1406 h529 Grid -Multi NoSort gSSOK_Expense_BudgetDashboard_HeaderClick vSSOK_BudgetDashboardWorkGeneralLV, ¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|°è¾à±Ý¾× (¿øÀÎÇàÀ§)|Áö±Þ¾×|¹ÌÁöÃâ¾× (Áö±Þ´ë±â)|¿¹»êÀÜ¾× (ºÒ¿ë¿¹»ê¾×)|ÁýÇà·ü (¿øÀÎÇàÀ§)|´ã´çÀÚ
    SSOK_Expense_BudgetDashboard_AddTopSummary("SSOK_BudgetDashboardWorkGeneralLV", [160,160,235,130,95,95,105,95,95,95,95,170], 46, 254, 1406, 557)
    Gui, SSOKBudgetDashboard:Add, Text, x46 y818 w1190 h54 +0x200 vSSOK_BudgetDashboardWorkGeneralStatus Hidden c555555,
    Gui, SSOKBudgetDashboard:Add, Button, x1270 y829 w168 h34 vSSOK_BudgetDashboardWorkGeneralExport gSSOK_Expense_BudgetDashboard_ExportWorkGeneral, ÀÌ ÅÇ Excel ÃßÃâ

    Gui, SSOKBudgetDashboard:Tab, 4, 1
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, ListView, x46 y282 w1406 h529 Grid -Multi NoSort gSSOK_Expense_BudgetDashboard_HeaderClick vSSOK_BudgetDashboardWorkPurposeLV, ¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|°è¾à±Ý¾× (¿øÀÎÇàÀ§)|Áö±Þ¾×|¹ÌÁöÃâ¾× (Áö±Þ´ë±â)|¿¹»êÀÜ¾× (ºÒ¿ë¿¹»ê¾×)|ÁýÇà·ü (¿øÀÎÇàÀ§)|´ã´çÀÚ
    SSOK_Expense_BudgetDashboard_AddTopSummary("SSOK_BudgetDashboardWorkPurposeLV", [160,160,235,130,95,95,105,95,95,95,95,170], 46, 254, 1406, 557)
    Gui, SSOKBudgetDashboard:Add, Text, x46 y818 w1190 h54 +0x200 vSSOK_BudgetDashboardWorkPurposeStatus Hidden c555555,
    Gui, SSOKBudgetDashboard:Add, Button, x1270 y829 w168 h34 vSSOK_BudgetDashboardWorkPurposeExport gSSOK_Expense_BudgetDashboard_ExportWorkPurpose, ÀÌ ÅÇ Excel ÃßÃâ

    Gui, SSOKBudgetDashboard:Tab, 6, 1
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x32 y212 w180 vSSOK_BudgetDashboardOverView gSSOK_Expense_BudgetDashboard_OverViewRefresh, ¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|»êÃâ³»¿ªº°||¿ø°¡Åë°èºñ¸ñº°
    SSOK_Expense_BudgetDashboard_AddChoiceButtons("SSOK_BudgetDashboardOverView", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|»êÃâ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|»êÃâ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", 32, 212, 116, "balanceView")
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, Text, x540 y214 w700 h26, ÃÊ°úÁýÇà »êÃâ³»¿ª°ú °°Àº ¼¼ºÎ»ç¾÷¡¤¼¼ºÎÇ×¸ñ¡¤¿ø°¡Åë°èºñ¸ñÀÇ ÀüÃ¼ ³»¿ªÀ» ÇÔ²² Ç¥½ÃÇÕ´Ï´Ù.
    Gui, SSOKBudgetDashboard:Add, Button, x1270 y210 w168 h30 gSSOK_Expense_BudgetDashboard_ExportOver, ÀÌ ÅÇ Excel ÃßÃâ
    Gui, SSOKBudgetDashboard:Add, ListView, x32 y278 w1434 h554 Grid -Multi NoSort gSSOK_Expense_BudgetDashboard_HeaderClick vSSOK_BudgetDashboardOverLV, »óÅÂ|Àç¿ø|¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|°è¾à±Ý¾× (¿øÀÎÇàÀ§)|Áö±Þ¾×|¹ÌÁöÃâ¾× (Áö±Þ´ë±â)|¿¹»êÀÜ¾× (ºÒ¿ë¿¹»ê¾×)|ÁýÇà·ü (¿øÀÎÇàÀ§)
    SSOK_Expense_BudgetDashboard_AddTopSummary("SSOK_BudgetDashboardOverLV", [65,75,120,140,210,110,100,110,100,110,120,105], 32, 250, 1434, 582)
    Gui, SSOKBudgetDashboard:Add, Text, x32 y838 w1434 h54 vSSOK_BudgetDashboardOverStatus Hidden,

    Gui, SSOKBudgetDashboard:Tab, 5, 1
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x32 y212 w180 vSSOK_BudgetDashboardPendingView gSSOK_Expense_BudgetDashboard_PendingViewRefresh, ¼¼ºÎ»ç¾÷º°||¼¼ºÎÇ×¸ñº°|»êÃâ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°
    SSOK_Expense_BudgetDashboard_AddChoiceButtons("SSOK_BudgetDashboardPendingView", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|»êÃâ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|»êÃâ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", 32, 212, 116, "balanceView")
    Gui, SSOKBudgetDashboard:Add, Text, x540 y214 w700 h26, °è¾à±Ý¾× - Áö±Þ¾×ÀÌ 0º¸´Ù Å« Áö±Þ´ë±â ³»¿ªÀÔ´Ï´Ù.
    Gui, SSOKBudgetDashboard:Add, Button, x1270 y210 w168 h30 gSSOK_Expense_BudgetDashboard_ExportPending, ÀÌ ÅÇ Excel ÃßÃâ
    Gui, SSOKBudgetDashboard:Add, ListView, x32 y278 w1434 h554 Grid -Multi NoSort gSSOK_Expense_BudgetDashboard_HeaderClick vSSOK_BudgetDashboardPendingLV, ¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|°è¾à±Ý¾× (¿øÀÎÇàÀ§)|Áö±Þ¾×|¹ÌÁöÃâ¾× (Áö±Þ´ë±â)|¿¹»êÀÜ¾× (ºÒ¿ë¿¹»ê¾×)|ÁýÇà·ü (¿øÀÎÇàÀ§)|´ã´çÀÚ
    SSOK_Expense_BudgetDashboard_AddTopSummary("SSOK_BudgetDashboardPendingLV", [130,130,200,115,100,105,100,105,110,100,170], 32, 250, 1434, 582)

    Gui, SSOKBudgetDashboard:Tab, 7, 1
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x32 y212 w180 vSSOK_BudgetDashboardUnusedView gSSOK_Expense_BudgetDashboard_UnusedViewRefresh, ¼¼ºÎ»ç¾÷º°||¼¼ºÎÇ×¸ñº°|»êÃâ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°
    SSOK_Expense_BudgetDashboard_AddChoiceButtons("SSOK_BudgetDashboardUnusedView", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|»êÃâ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", "¼¼ºÎ»ç¾÷º°|¼¼ºÎÇ×¸ñº°|»êÃâ³»¿ªº°|¿ø°¡Åë°èºñ¸ñº°", 32, 212, 116, "balanceView")
    Gui, SSOKBudgetDashboard:Add, Text, x540 y214 w700 h26, ¿¹»êÇö¾× - °è¾à±Ý¾×ÀÌ 0º¸´Ù Å« ³»¿ªÀÔ´Ï´Ù. ÇöÀç ¹Ì°è¾à ÀÜ¾×ÀÌ¸ç È®Á¤ ºÒ¿ë¾×Àº ¾Æ´Õ´Ï´Ù.
    Gui, SSOKBudgetDashboard:Add, Button, x1270 y210 w168 h30 gSSOK_Expense_BudgetDashboard_ExportUnused, ÀÌ ÅÇ Excel ÃßÃâ
    Gui, SSOKBudgetDashboard:Add, ListView, x32 y278 w1434 h554 Grid -Multi NoSort gSSOK_Expense_BudgetDashboard_HeaderClick vSSOK_BudgetDashboardUnusedLV, ¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|°è¾à±Ý¾× (¿øÀÎÇàÀ§)|Áö±Þ¾×|¹ÌÁöÃâ¾× (Áö±Þ´ë±â)|¿¹»êÀÜ¾× (ºÒ¿ë¿¹»ê¾×)|ÁýÇà·ü (¿øÀÎÇàÀ§)|´ã´çÀÚ
    SSOK_Expense_BudgetDashboard_AddTopSummary("SSOK_BudgetDashboardUnusedLV", [130,130,200,115,100,105,100,105,110,100,170], 32, 250, 1434, 582)

    Gui, SSOKBudgetDashboard:Tab, 8, 1
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x32 y213 w140 vSSOK_BudgetDashboardCostResource gSSOK_Expense_BudgetDashboard_CostRefresh, ÀüÃ¼||ÀÏ¹ÝÀç¿ø|¸ñÀûÀç¿ø
    SSOK_Expense_BudgetDashboard_AddChoiceButtons("SSOK_BudgetDashboardCostResource", "ÀüÃ¼|ÀÏ¹ÝÀç¿ø|¸ñÀûÀç¿ø", "ÀüÃ¼|ÀÏ¹ÝÀç¿ø|¸ñÀûÀç¿ø", 32, 211, 85, "resource")
    Gui, SSOKBudgetDashboard:Add, DropDownList, Hidden x305 y213 w180 vSSOK_BudgetDashboardCostSort gSSOK_Expense_BudgetDashboard_CostRefresh, Çö¾× Å«¼ø||ÀÌ¸§¼ø|¿øÀÎÇàÀ§ Å«¼ø|Áö±Þ¾× Å«¼ø|ÀÜ¾× Å«¼ø|ÁýÇà·ü ³ôÀº¼ø|ÁýÇà·ü ³·Àº¼ø
    Gui, SSOKBudgetDashboard:Add, Text, x500 y215 w620 h24, ÀÎ°Çºñ ¹× ¹ýÁ¤ºÎ´ã±ÝÀº ÀÎ°Çºñ·Î ÅëÇÕÇÕ´Ï´Ù.
    Gui, SSOKBudgetDashboard:Add, Button, x1270 y210 w168 h30 gSSOK_Expense_BudgetDashboard_ExportCost, ÀÌ ÅÇ Excel ÃßÃâ
    Gui, SSOKBudgetDashboard:Add, ListView, x32 y278 w1434 h554 Grid -Multi NoSort gSSOK_Expense_BudgetDashboard_HeaderClick vSSOK_BudgetDashboardCostLV, ¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|°è¾à±Ý¾× (¿øÀÎÇàÀ§)|Áö±Þ¾×|¹ÌÁöÃâ¾× (Áö±Þ´ë±â)|¿¹»êÀÜ¾× (ºÒ¿ë¿¹»ê¾×)|ÁýÇà·ü (¿øÀÎÇàÀ§)|¸ñÀûÀç¿ø|ÀÏ¹ÝÀç¿ø
    SSOK_Expense_BudgetDashboard_AddTopSummary("SSOK_BudgetDashboardCostLV", [300,120,120,120,120,130,120,120,120], 32, 250, 1434, 582)
    Gui, SSOKBudgetDashboard:Add, Text, x32 y838 w1434 h54 vSSOK_BudgetDashboardCostStatus Hidden,

    Gui, SSOKBudgetDashboard:Tab
    Gui, SSOKBudgetDashboard:Font, s8 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, Text, x740 y176 w80 h24 +0x200, »ç¾÷¸í °Ë»ö:
    Gui, SSOKBudgetDashboard:Add, Edit, x824 y174 w220 h24 vSSOK_BudgetDashboardSearch gSSOK_Expense_BudgetDashboard_Refresh
    Gui, SSOKBudgetDashboard:Add, Text, x1060 y176 w124 h24 +0x200, »ç¾÷´ã´çÀÚ¸í °Ë»ö:
    Gui, SSOKBudgetDashboard:Add, Edit, x1186 y174 w280 h24 vSSOK_BudgetDashboardManagerSearch gSSOK_Expense_BudgetDashboard_Refresh
    Gui, SSOKBudgetDashboard:Show, w1498 h990 Center, %dashboardTitle%
    SSOK_Expense_BudgetDashboard_RefreshAll()
}


SSOK_Expense_BudgetDashboard_FilterRows()
{
    global SSOK_BudgetDashboardParsed, SSOK_BudgetDashboardSearch, SSOK_BudgetDashboardManagerSearch
    businessQuery := Trim(SSOK_BudgetDashboardSearch)
    managerQuery := Trim(SSOK_BudgetDashboardManagerSearch)
    rows := []
    for _, row in SSOK_BudgetDashboardParsed.rows
    {
        ; The business field also accepts a manager's name; the manager field is specific.
        pool := row.business . " " . row.subItem . " " . row.calc . " " . row.manager
        if (businessQuery != "" && !InStr(pool, businessQuery))
            continue
        if (managerQuery != "" && !InStr(row.manager, managerQuery))
            continue
        rows.Push(row)
    }
    return rows
...
SSOK_Expense_BudgetDashboard_FilterTotals()
{
    totals := {current:0, commit:0, paid:0, general:0, settle:0, generalCommit:0, settleCommit:0, generalPaid:0, settlePaid:0, rowCount:0, overCount:0}
    for _, row in SSOK_Expense_BudgetDashboard_FilterRows()
    {
        for _, field in ["current", "commit", "paid", "general", "settle", "generalCommit", "settleCommit", "generalPaid", "settlePaid"]
...
SSOK_Expense_BudgetDashboard_RefreshAll()
{
    ...
    if !IsObject(SSOK_BudgetDashboardParsed)
        return
    Gui, SSOKBudgetDashboard:Submit, NoHide
    global SSOK_BudgetDashboardLastSearch, SSOK_BudgetDashboardLastOver, SSOK_BudgetDashboardOverOnly, SSOK_BudgetDashboardSubtotal
    searchKey := Trim(SSOK_BudgetDashboardSearch) . Chr(30) . Trim(SSOK_BudgetDashboardManagerSearch)
    if (searchKey != SSOK_BudgetDashboardLastSearch || SSOK_BudgetDashboardOverOnly != SSOK_BudgetDashboardLastOver)
    {
        detail := searchKey != Chr(30) || SSOK_BudgetDashboardOverOnly
        GuiControl, SSOKBudgetDashboard:ChooseString, SSOK_BudgetDashboardViewChoice, % detail ? "»êÃâ³»¿ªº°" : "¼¼ºÎÇ×¸ñº°"
        if (detail)
        {
            SSOK_BudgetDashboardSubtotal := 0
            GuiControl, SSOKBudgetDashboard:, SSOK_BudgetDashboardSubtotal, 0
        }
        SSOK_Expense_BudgetDashboard_MarkChoice("SSOK_BudgetDashboardViewChoice")
        SSOK_BudgetDashboardLastSearch := searchKey
        SSOK_BudgetDashboardLastOver := SSOK_BudgetDashboardOverOnly
    }
    SSOK_Expense_BudgetDashboard_RefreshList()
    SSOK_Expense_BudgetDashboard_RefreshPurposeList()
    SSOK_Expense_BudgetDashboard_RefreshGeneralLists()
    SSOK_Expense_BudgetDashboard_RefreshWorkExpenseList()
    SSOK_Expense_BudgetDashboard_FillCost()
    SSOK_Expense_BudgetDashboard_FillOver()
    SSOK_Expense_BudgetDashboard_FillBalance("Pending")
    SSOK_Expense_BudgetDashboard_FillBalance("Unused")
    for _, resource in ["¸ñÀûÀç¿ø", "ÀÏ¹ÝÀç¿ø", "¾÷¹«ÃßÁøºñ"]
    {
        rows := resource = "¾÷¹«ÃßÁøºñ" ? SSOK_Expense_BudgetDashboard_BuildWorkExpenseEntries() : SSOK_Expense_BudgetDashboard_BuildEntries("Àç¿øº°", resource)
        current := 0, commit := 0, paid := 0
        for _, row in rows
            current += row.current, commit += row.commit, paid += row.paid
        summary := resource . " Á¶È¸ ÇÕ°è  |  Çö¾× " . SSOK_Expense_FormatNumber(current)
            . "¿ø  |  ¿øÀÎÇàÀ§ " . SSOK_Expense_FormatNumber(commit)
            . "¿ø  |  ¿¹»êÀÜ¾× " . SSOK_Expense_FormatNumber(current - commit)
            . "¿ø  |  Áö±Þ " . SSOK_Expense_FormatNumber(paid) . "¿ø"
        control := "SSOK_BudgetDashboard" . (resource = "¸ñÀûÀç¿ø" ? "Purpose" : resource = "ÀÏ¹ÝÀç¿ø" ? "General" : "Work") . "Total"
        GuiControl, SSOKBudgetDashboard:, %control%, %summary%
    }
    SSOK_Expense_BudgetDashboard_UpdateSections()
    SSOK_Expense_BudgetDashboard_RefreshScope()
}

SSOK_Expense_BudgetDashboard_RefreshScope()
{
    global SSOK_BudgetDashboardParsed, SSOK_BudgetDashboardSearch, SSOK_BudgetDashboardManagerSearch
    p := SSOK_Expense_BudgetDashboard_FilterTotals()
    scope := "Á¶È¸ " . p.rowCount . " / ÀüÃ¼ " . SSOK_BudgetDashboardParsed.rowCount . "°Ç  |  ÃÊ°úÁýÇà " . p.overCount . "°Ç"
    if (Trim(SSOK_BudgetDashboardSearch) != "")
        scope .= "  |  »ç¾÷¸í: " . Trim(SSOK_BudgetDashboardSearch)
    if (Trim(SSOK_BudgetDashboardManagerSearch) != "")
        scope .= "  |  ´ã´çÀÚ: " . Trim(SSOK_BudgetDashboardManagerSearch)
    GuiControl, SSOKBudgetDashboard:, SSOK_BudgetDashboardScopeStatus, %scope%
}

SSOK_Expense_BudgetDashboard_SetKpi(title, total, detail)
{
    global SSOK_BudgetDashboardKpiControls
    handles := SSOK_BudgetDashboardKpiControls[title]
    GuiControl, SSOKBudgetDashboard:, % handles[1], %total%
    GuiControl, SSOKBudgetDashboard:, % handles[2], %detail%
    color := SubStr(total, 1, 1) = "-" ? "C62828" : handles[3]
    GuiControl, SSOKBudgetDashboard:+c%color%, % handles[1]
}

SSOK_Expense_BudgetDashboard_SetIcon(hwnd)
{
    static smallIcon := 0, largeIcon := 0
    if (!smallIcon)
    {
        iconPath := A_Temp . "\SSOK_BudgetIcon_" . DllCall("GetCurrentProcessId") . ".ico"
        FileInstall, ssok.ico, %iconPath%, 1
        if !ErrorLevel
        {
            smallIcon := DllCall("LoadImageW", "Ptr", 0, "WStr", iconPath, "UInt", 1, "Int", 16, "Int", 16, "UInt", 0x10, "Ptr")
            largeIcon := DllCall("LoadImageW", "Ptr", 0, "WStr", iconPath, "UInt", 1, "Int", 32, "Int", 32, "UInt", 0x10, "Ptr")
            FileDelete, %iconPath%
        }
    }
    if (smallIcon)
        DllCall("SendMessageW", "Ptr", hwnd, "UInt", 0x80, "Ptr", 0, "Ptr", smallIcon)
    if (largeIcon)
        DllCall("SendMessageW", "Ptr", hwnd, "UInt", 0x80, "Ptr", 1, "Ptr", largeIcon)
}

SSOK_Expense_BudgetDashboard_UpdateColors(listViewName, entries := "", category := "", viewMode := "")
{
    global SSOK_BudgetDashboardRowColors, SSOK_BudgetDashboardParsed, SSOK_BudgetDashboardHeaderSort
    GuiControlGet, handle, SSOKBudgetDashboard:Hwnd, %listViewName%
    Gui, SSOKBudgetDashboard:ListView, %listViewName%
    businessBalances := {}
    if (category = "±¹°í»ç¾÷" || category = "¸ñÀû»ç¾÷")
        for _, row in SSOK_Expense_BudgetDashboard_FilterRows()
            if (row.resourceType = "¸ñÀûÀç¿ø" && row.purposeCategory = category)
            {
                if !businessBalances.HasKey(row.business)
                    businessBalances[row.business] := 0
                businessBalances[row.business] += row.current - row.commit
            }
    styles := []
    Loop, % LV_GetCount()
    {
        index := A_Index
        style := {negative:{}, red:false, yellow:0}
        Loop, % LV_GetCount("Column")
        {
            column := A_Index
            LV_GetText(value, index, column)
            if RegExMatch(value, "^-\d[\d,]*(?:\.\d+)?%?$")
                style.negative[column] := true
        }
        if (listViewName = "SSOK_BudgetDashboardOverLV")
        {
            LV_GetText(value, index, 1)
            style.red := value = "ÃÊ°ú"
        }
        if ((category = "±¹°í»ç¾÷" || category = "¸ñÀû»ç¾÷") && viewMode != "¿ø°¡Åë°èºñ¸ñº°")
        {
            entry := entries[index]
            threshold := category = "±¹°í»ç¾÷" ? 10 : 100000
            if (businessBalances[entry.business] >= threshold)
                style.yellow := 9
        }
        styles.Push(style)
    }
    SSOK_BudgetDashboardRowColors[handle] := styles
    previous := SSOK_BudgetDashboardHeaderSort[handle]
    if (previous.column)
        SSOK_Expense_BudgetDashboard_SortColumn(listViewName, previous.column, true)
    GuiControl, SSOKBudgetDashboard:+Redraw, %listViewName%
}

SSOK_Expense_BudgetDashboard_CustomDraw(wParam, lParam, msg, hwnd)
{
    global SSOK_BudgetDashboardRowColors, SSOK_BudgetDashboardFonts, SSOK_BudgetDashboardHeaders
    if (NumGet(lParam + 0, A_PtrSize * 2, "Int") != -12)
        return
    handle := NumGet(lParam + 0, 0, "Ptr")
    if !SSOK_BudgetDashboardRowColors.HasKey(handle)
        return
    stageOffset := A_PtrSize = 8 ? 24 : 12
    stage := NumGet(lParam + 0, stageOffset, "UInt")
    if (stage = 1)
        return 0x20
    if (stage = 0x10001)
        return 0x20
    if (stage != 0x30001)
        return
    row := NumGet(lParam + 0, A_PtrSize = 8 ? 56 : 36, "UPtr") + 1
    column := NumGet(lParam + 0, A_PtrSize = 8 ? 88 : 56, "Int") + 1
    style := SSOK_BudgetDashboardRowColors[handle][row]
    textColor := style.negative.HasKey(column) ? 0x0000CC : 0x000000
    background := style.red ? 0xDADAFF : (style.yellow = column ? 0x99FFFF : 0xFFFFFF)
    NumPut(textColor, lParam + 0, A_PtrSize = 8 ? 80 : 48, "UInt")
    NumPut(background, lParam + 0, A_PtrSize = 8 ? 84 : 52, "UInt")
    if (style.negative.HasKey(column))
    {
        if !SSOK_BudgetDashboardFonts.HasKey(handle)
        {
            SendMessage, 0x31, 0, 0,, ahk_id %handle%
            VarSetCapacity(logfont, 92, 0)
            DllCall("GetObjectW", "Ptr", ErrorLevel, "Int", 92, "Ptr", &logfont)
            NumPut(700, logfont, 16, "Int")
            SSOK_BudgetDashboardFonts[handle] := DllCall("CreateFontIndirectW", "Ptr", &logfont, "Ptr")
        }
        hdc := NumGet(lParam + 0, A_PtrSize = 8 ? 32 : 16, "Ptr")
        DllCall("SelectObject", "Ptr", hdc, "Ptr", SSOK_BudgetDashboardFonts[handle])
        return 2
    }
    SendMessage, 0x31, 0, 0,, ahk_id %handle%
    hdc := NumGet(lParam + 0, A_PtrSize = 8 ? 32 : 16, "Ptr")
    DllCall("SelectObject", "Ptr", hdc, "Ptr", ErrorLevel)
    return 2
}

SSOK_Expense_BudgetDashboard_AddTopSummary(listViewName, widths, posX, posY, posW, posH)
{
    global SSOK_BudgetDashboardSummaryControls
    controls := []
    x := posX
    ...
    for index, width in widths
    {
        ; Stop at the visible table edge; preserve the numeric columns' alignment.
        visibleWidth := Min(width, posX + posW - x)
        if (visibleWidth <= 0)
            break
        alignment := index = 1 ? "" : "Right"
        Gui, SSOKBudgetDashboard:Add, Text, x%x% y%posY% w%visibleWidth% h26 +0x200 %alignment% c005BAC +Hwndhandle,
        controls.Push(handle)
        x += width
    }
    SSOK_BudgetDashboardSummaryControls[listViewName] := controls
    SSOK_Expense_BudgetDashboard_SetHeader(listViewName)
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
}

SSOK_Expense_BudgetDashboard_SetTopSummary(listViewName, firstAmountColumn, current, commit, paid, purpose := "", general := "")
{
    global SSOK_BudgetDashboardSummaryControls
    values := [SSOK_Expense_FormatNumber(current), SSOK_Expense_FormatNumber(commit)
        , SSOK_Expense_FormatNumber(paid), SSOK_Expense_FormatNumber(commit - paid)
        , SSOK_Expense_FormatNumber(current - commit), SSOK_Expense_BudgetDashboard_FormatRate(current, commit)]
    if (purpose != "" || general != "")
        values.Push(SSOK_Expense_FormatNumber(purpose), SSOK_Expense_FormatNumber(general))
    for index, handle in SSOK_BudgetDashboardSummaryControls[listViewName]
    {
        valueIndex := index - firstAmountColumn + 1
        value := index = 1 ? "Á¶È¸ ÇÕ°è" : (valueIndex >= 1 && valueIndex <= values.Length() ? values[valueIndex] : "")
        color := SubStr(value, 1, 1) = "-" ? "CC0000" : "005BAC"
        GuiControl, SSOKBudgetDashboard:+c%color%, %handle%
        GuiControl, SSOKBudgetDashboard:, %handle%, %value%
    }
}

SSOK_Expense_BudgetDashboard_CompactResourceText(label, current, commit, paid)
{
    return label . " A" . SSOK_Expense_FormatNumber(current)
        . " B" . SSOK_Expense_FormatNumber(commit) . "(" . SSOK_Expense_BudgetDashboard_FormatRate(current, commit) . ")"
        . " C" . SSOK_Expense_FormatNumber(paid) . "(" . SSOK_Expense_BudgetDashboard_FormatRate(current, paid) . ")"
        . " ÀÜ" . SSOK_Expense_FormatNumber(current - paid)
}

SSOK_Expense_BudgetDashboard_AddSplitKpi(x, y, w, title, totalText, generalText, purposeText, valueColor)
{
    global SSOK_BudgetDashboardKpiControls
    Gui, SSOKBudgetDashboard:Font, s8 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, GroupBox, x%x% y%y% w%w% h78, %title%
    valueX := x + 8
    valueY := y + 19
    valueW := w - 16
    Gui, SSOKBudgetDashboard:Font, s10 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, Text, x%valueX% y%valueY% w%valueW% h24 Center c%valueColor% +HwndtotalHandle, %totalText%
    detailY := y + 45
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    detailText := generalText . "`n" . purposeText
    Gui, SSOKBudgetDashboard:Add, Text, x%valueX% y%detailY% w%valueW% h30 Center c555555 +HwnddetailHandle, %detailText%
    SSOK_BudgetDashboardKpiControls[title] := [totalHandle, detailHandle, valueColor]
}
...
SSOK_Expense_BudgetDashboard_RefreshList()
{
    global SSOK_BudgetDashboardParsed, SSOK_BudgetDashboardSubtotal
    global SSOK_BudgetDashboardViewChoice, SSOK_BudgetDashboardSearch, SSOK_BudgetDashboardManagerSearch
    global SSOK_BudgetDashboardSortChoice, SSOK_BudgetDashboardResourceChoice
    global SSOK_BudgetDashboardHideZero, SSOK_BudgetDashboardOverOnly
    global SSOK_BudgetDashboardListStatus
    ...
    GuiControlGet, listHwnd, SSOKBudgetDashboard:Hwnd, SSOK_BudgetDashboardMainLV
    if (!listHwnd)
        return
    Gui, SSOKBudgetDashboard:Submit, NoHide
    ...
    entries := SSOK_Expense_BudgetDashboard_BuildEntries(SSOK_BudgetDashboardSubtotal ? "Àç¿ø±¸ºÐº°" : viewMode, resourceMode)
    SSOK_Expense_BudgetDashboard_SortEntries(entries, sortMode)

    Gui, SSOKBudgetDashboard:Default
    Gui, SSOKBudgetDashboard:ListView, SSOK_BudgetDashboardMainLV
    GuiControl, SSOKBudgetDashboard:-Redraw, SSOK_BudgetDashboardMainLV
    LV_Delete()
    ...
    shownPaid := 0
    shownOver := 0
    shownPurpose := 0, shownGeneral := 0

    for _, entry in entries
    {
        if (hideZero && entry.current = 0 && entry.commit = 0 && entry.paid = 0)
            continue
        if (overOnly && !(entry.commit > entry.current))
            continue

        LV_Add("", entry.business, entry.subItem, entry.calc, entry.cost
            , SSOK_Expense_FormatNumber(entry.current), SSOK_Expense_FormatNumber(entry.commit), SSOK_Expense_FormatNumber(entry.paid), SSOK_Expense_FormatNumber(entry.commit - entry.paid), SSOK_Expense_FormatNumber(entry.current - entry.commit), SSOK_Expense_BudgetDashboard_FormatRate(entry.current, entry.commit)
            , SSOK_Expense_FormatNumber(entry.settle)
            , SSOK_Expense_FormatNumber(entry.general), entry.manager)

        shownPurpose += entry.settle, shownGeneral += entry.general
        shown++
        shownCurrent += entry.current
        shownCommit += entry.commit
        shownPaid += entry.paid
        if (entry.commit > entry.current)
            shownOver++
    }

    SSOK_Expense_BudgetDashboard_SetTopSummary("SSOK_BudgetDashboardMainLV", 5, shownCurrent, shownCommit, shownPaid, shownPurpose, shownGeneral)
    widths := [120,120,165,100,100,100,100,105,110,85,85,85,140]
    for col, width in widths
        LV_ModifyCol(col, (col >= 5 ? "Right " : "") . width)

    GuiControl, SSOKBudgetDashboard:+Redraw, SSOK_BudgetDashboardMainLV
    status := "Ç¥½Ã " . shown . "°Ç"
    status .= "  |  ¿¹»êÇö¾× " . SSOK_Expense_FormatNumber(shownCurrent) . "¿ø"
    status .= "  |  ¿øÀÎÇàÀ§¾× " . SSOK_Expense_FormatNumber(shownCommit) . "¿ø"
    status .= "  |  ¿øÀÎÇàÀ§ ÁýÇà·ü " . SSOK_Expense_BudgetDashboard_FormatRate(shownCurrent, shownCommit)
    status .= "  |  ¿¹»êÀÜ¾× " . SSOK_Expense_FormatNumber(shownCurrent - shownCommit) . "¿ø"
    status .= "  |  Áö±Þ¾× " . SSOK_Expense_FormatNumber(shownPaid) . "¿ø"
    status .= "  |  Áö±Þ ÁýÇà·ü " . SSOK_Expense_BudgetDashboard_FormatRate(shownCurrent, shownPaid)
    status .= "  |  ÁöÃâÀÜ¾× " . SSOK_Expense_FormatNumber(shownCurrent - shownPaid) . "¿ø"
    status .= "  |  ÃÊ°ú " . shownOver . "°Ç"
    GuiControl, SSOKBudgetDashboard:, SSOK_BudgetDashboardListStatus, %status%
    SSOK_Expense_BudgetDashboard_UpdateColors("SSOK_BudgetDashboardMainLV")
}

SSOK_Expense_BudgetDashboard_BuildEntries(viewMode, resourceMode := "ÀüÃ¼", mergeLabor := false, sourceRows := "")
{
    global SSOK_BudgetDashboardParsed
    entries := []
    p := SSOK_BudgetDashboardParsed
    if !IsObject(sourceRows)
        sourceRows := SSOK_Expense_BudgetDashboard_FilterRows()

    if (viewMode = "»êÃâ³»¿ªº°")
    {
        for _, row in sourceRows
        {
            if (resourceMode != "ÀüÃ¼" && row.resourceType != resourceMode)
                continue
            managerText := (row.manager != "") ? row.manager : "´ã´çÀÚ ¾øÀ½"
            detailText := row.business . " / " . row.subItem . " / " . row.cost . " / " . managerText
            entries.Push({label:row.calc, business:row.business, subItem:row.subItem, calc:row.calc, cost:row.cost
                , manager:row.manager, detail:detailText
                , current:row.current
                , draft:row.draft
                , commit:row.commit
                , paid:row.paid
                ...
                , settlePaid:row.settlePaid
                , generalPaid:row.generalPaid})
        }
        return entries
    }
    ...
    for _, row in sourceRows
    {
        if (resourceMode != "ÀüÃ¼" && row.resourceType != resourceMode)
            continue
        if (viewMode = "Àç¿øº°" || viewMode = "Àç¿ø±¸ºÐº°")
        {
            label := row.resourceType
            if (viewMode = "Àç¿ø±¸ºÐº°")
                label .= " / " . (row.resourceType = "¸ñÀûÀç¿ø" ? row.purposeCategory : row.generalCategory)
            key := label
            detailBase := ""
        }
        else if (viewMode = "¼¼ºÎÇ×¸ñº°")
        {
            key := row.business . Chr(30) . row.subItem
            label := (row.subItem != "") ? row.subItem : "(¼¼ºÎÇ×¸ñ ¾øÀ½)"
            detailBase := row.business
        }
        else if (viewMode = "¿ø°¡Åë°èºñ¸ñº°")
        {
            label := (row.cost != "") ? row.cost : "(¿ø°¡Åë°èºñ¸ñ ¾øÀ½)"
            if (mergeLabor && (InStr(label, "ÀÎ°Çºñ") || InStr(label, "¹ýÁ¤ºÎ´ã±Ý")))
                label := "ÀÎ°Çºñ"
            key := label
            detailBase := ""
        }
        else if (viewMode = "¼¼ºÎÇ×¸ñ´ã´çÀÚº°")
        {
            key := (row.manager != "") ? row.manager : "´ã´çÀÚ ¾øÀ½"
            label := key
            detailBase := ""
        }
        else
        {
            key := row.business
            label := (row.business != "") ? row.business : "(¼¼ºÎ»ç¾÷ ¾øÀ½)"
            detailBase := ""
        }
        ...
        if !IsObject(groups[key])
            groups[key] := {label:label, business:(viewMode = "¿ø°¡Åë°èºñ¸ñº°" ? "" : (viewMode = "¼¼ºÎÇ×¸ñº°" ? row.business : label)), subItem:(viewMode = "¼¼ºÎÇ×¸ñº°" ? row.subItem : ""), calc:"", cost:(viewMode = "¿ø°¡Åë°èºñ¸ñº°" ? label : ""), detailBase:detailBase, detail:"", manager:"", current:0, draft:0, commit:0, paid:0, settle:0, general:0, settleCommit:0, generalCommit:0, settlePaid:0, generalPaid:0, count:0}

        g := groups[key]
        g.manager := SSOK_Expense_BudgetDashboard_AppendManager(g.manager, row.manager)
        ...
        g.settlePaid += row.settlePaid
        g.generalPaid += row.generalPaid
        g.count++
    }
    ...
    {
        if (g.detailBase != "")
            g.detail := g.detailBase . " / »êÃâ³»¿ª " . g.count . "°Ç"
        else
            g.detail := "»êÃâ³»¿ª " . g.count . "°Ç"
        entries.Push(g)
    }
...
SSOK_Expense_BudgetDashboard_ChoiceClick:
    SSOK_Expense_BudgetDashboard_ChooseButton(A_GuiControl)
return

SSOK_Expense_BudgetDashboard_AddChoiceButtons(control, labels, values, x, y, width, kind)
{
    static
    global SSOK_BudgetDashboardButtons, SSOK_BudgetDashboardButtonGroups
    names := StrSplit(labels, "|"), choices := StrSplit(values, "|")
    handles := []
    for index, label in names
    {
        buttonNumber := buttonNumber ? buttonNumber + 1 : 1
        buttonName := "SSOK_BudgetDashboardChoice" . buttonNumber
        Gui, SSOKBudgetDashboard:Add, Button, x%x% y%y% w%width% h26 v%buttonName% +Hwndhandle gSSOK_Expense_BudgetDashboard_ChoiceClick, %label%
        SSOK_BudgetDashboardButtons[handle] := {control:control, value:choices[index], kind:kind, label:label}
        SSOK_BudgetDashboardButtons[buttonName] := SSOK_BudgetDashboardButtons[handle]
        handles.Push(handle)
        x += width + 3
    }
    SSOK_BudgetDashboardButtonGroups[control] := handles
    SSOK_Expense_BudgetDashboard_MarkChoice(control)
}

SSOK_Expense_BudgetDashboard_MarkChoice(control)
{
    global SSOK_BudgetDashboardButtons, SSOK_BudgetDashboardButtonGroups
    GuiControlGet, selected, SSOKBudgetDashboard:, %control%
    for _, handle in SSOK_BudgetDashboardButtonGroups[control]
    {
        item := SSOK_BudgetDashboardButtons[handle]
        caption := (selected = item.value ? "¡Ü " : "") . item.label
        GuiControl, SSOKBudgetDashboard:, %handle%, %caption%
    }
}

SSOK_Expense_BudgetDashboard_ChooseButton(controlHandle)
{
    global SSOK_BudgetDashboardButtons
    ; Unnamed controls report their HWND through A_GuiControl.
    item := SSOK_BudgetDashboardButtons[controlHandle]
    if !IsObject(item)
        return
    control := item.control
    GuiControl, SSOKBudgetDashboard:ChooseString, %control%, % item.value
    SSOK_Expense_BudgetDashboard_MarkChoice(control)
    if (item.kind = "section")
        SSOK_Expense_BudgetDashboard_UpdateSections()
    else if (item.kind = "balanceView")
    {
        if (control = "SSOK_BudgetDashboardOverView")
            SSOK_Expense_BudgetDashboard_FillOver()
        else
            SSOK_Expense_BudgetDashboard_FillBalance(control = "SSOK_BudgetDashboardPendingView" ? "Pending" : "Unused")
    }
    else if (control = "SSOK_BudgetDashboardCostResource")
        SSOK_Expense_BudgetDashboard_FillCost()
    else if (control = "SSOK_BudgetDashboardViewChoice" || control = "SSOK_BudgetDashboardResourceChoice")
        SSOK_Expense_BudgetDashboard_RefreshList()
    else
        SSOK_Expense_BudgetDashboard_RefreshResourceControl(control)
}

SSOK_Expense_BudgetDashboard_HeaderClick:
    if (A_GuiEvent = "ColClick")
        SSOK_Expense_BudgetDashboard_SortColumn(A_GuiControl, A_EventInfo)
return

SSOK_Expense_BudgetDashboard_SortColumn(control, column, preserveDirection := false)
{
    global SSOK_BudgetDashboardRowColors, SSOK_BudgetDashboardHeaderSort
    Gui, SSOKBudgetDashboard:Default
    Gui, SSOKBudgetDashboard:ListView, %control%
    GuiControlGet, handle, SSOKBudgetDashboard:Hwnd, %control%
    previous := SSOK_BudgetDashboardHeaderSort[handle]
    descending := preserveDirection ? previous.descending : (previous.column = column ? !previous.descending : false)
    SSOK_BudgetDashboardHeaderSort[handle] := {column:column, descending:descending}
    entries := [], numeric := true
    Loop, % LV_GetCount()
    {
        row := A_Index, cells := []
        Loop, % LV_GetCount("Column")
        ...
        value := cells[column]
        if !(value = "" || value = "ÃÊ°ú" || RegExMatch(value, "^-?\d[\d,]*(?:\.\d+)?%?$"))
            numeric := false
        number := value = "ÃÊ°ú" ? 1.0e99 : StrReplace(StrReplace(value, ","), "%") + 0
        entries.Push({cells:cells, label:value, current:descending ? number : -number, style:SSOK_BudgetDashboardRowColors[handle][row]})
    }
    SSOK_Expense_BudgetDashboard_SortEntries(entries, numeric ? "Çö¾× Å«¼ø" : (descending ? "¹®ÀÚ ¿ª¼ø" : "ÀÌ¸§¼ø"))
    GuiControl, SSOKBudgetDashboard:-Redraw, %control%
    LV_Delete()
    styles := []
    for _, entry in entries
    {
...
SSOK_Expense_BudgetDashboard_SelectSection:
    SSOK_Expense_BudgetDashboard_UpdateSections()
return

SSOK_Expense_BudgetDashboard_UpdateSections()
{
    global SSOK_BudgetDashboardSummaryControls, SSOK_BudgetDashboardButtonGroups
    GuiControlGet, page, SSOKBudgetDashboard:, SSOK_BudgetDashboardTab
    GuiControlGet, purpose, SSOKBudgetDashboard:, SSOK_BudgetDashboardPurposeSubTab
    GuiControlGet, general, SSOKBudgetDashboard:, SSOK_BudgetDashboardGeneralSubTab
    GuiControlGet, work, SSOKBudgetDashboard:, SSOK_BudgetDashboardWorkSubTab
    groups := [["PurposeAll", "¸ñÀûÀç¿ø °ü¸®", "ÀüÃ¼", purpose]
        , ["GeneralAll", "ÀÏ¹ÝÀç¿ø °ü¸®", "ÀüÃ¼", general]
        , ["WorkAll", "¾÷¹«ÃßÁøºñ °ü¸®", "ÀüÃ¼", work]
        , ["PurposeProject", "¸ñÀûÀç¿ø °ü¸®", "¸ñÀû»ç¾÷", purpose]
        , ["PurposeNational", "¸ñÀûÀç¿ø °ü¸®", "±¹°íº¸Á¶±Ý", purpose]
        , ["PurposeUser", "¸ñÀûÀç¿ø °ü¸®", "¼öÀÍÀÚ", purpose]
        , ["Future", "ÀÏ¹ÝÀç¿ø °ü¸®", "¹Ì·¡", general]
        , ["Special", "ÀÏ¹ÝÀç¿ø °ü¸®", "Æ¯¼º", general]
        , ["Integrated", "ÀÏ¹ÝÀç¿ø °ü¸®", "ÅëÇÕ", general]
        , ["General", "ÀÏ¹ÝÀç¿ø °ü¸®", "ÀÏ¹Ý", general]
        , ["WorkGeneral", "¾÷¹«ÃßÁøºñ °ü¸®", "ÀÏ¹Ý", work]
        , ["WorkPurpose", "¾÷¹«ÃßÁøºñ °ü¸®", "¸ñÀû", work]]
    for _, group in groups
    {
        action := (page = group[2] && group[3] = group[4]) ? "Show" : "Hide"
        suffixes := InStr(group[1], "Work") = 1 ? ["LV", "Export"] : ["Label", "LV", "Export"]
        for _, suffix in suffixes
        {
            control := "SSOK_BudgetDashboard" . group[1] . suffix
            GuiControl, SSOKBudgetDashboard:%action%, %control%
        }
        for _, handle in SSOK_BudgetDashboardButtonGroups["SSOK_BudgetDashboard" . group[1] . "View"]
            GuiControl, SSOKBudgetDashboard:%action%, %handle%
        for _, handle in SSOK_BudgetDashboardSummaryControls["SSOK_BudgetDashboard" . group[1] . "LV"]
            GuiControl, SSOKBudgetDashboard:%action%, %handle%
    }
}
...
{
    configs := {}
    configs["PurposeAll"] := ["¸ñÀûÀç¿ø", "", "¸ñÀûÀç¿ø ÀüÃ¼"]
    configs["GeneralAll"] := ["ÀÏ¹ÝÀç¿ø", "", "ÀÏ¹ÝÀç¿ø ÀüÃ¼"]
    configs["PurposeProject"] := ["¸ñÀûÀç¿ø", "¸ñÀû»ç¾÷", "¸ñÀû»ç¾÷"]
    configs["PurposeNational"] := ["¸ñÀûÀç¿ø", "±¹°í»ç¾÷", "±¹°íº¸Á¶±Ý"]
    configs["PurposeUser"] := ["¸ñÀûÀç¿ø", "¼öÀÍÀÚ", "¼öÀÍÀÚ"]
    configs["Future"] := ["ÀÏ¹ÝÀç¿ø", "¹Ì·¡", "¹Ì·¡"]
    configs["Special"] := ["ÀÏ¹ÝÀç¿ø", "Æ¯¼º", "Æ¯¼º"]
    configs["Integrated"] := ["ÀÏ¹ÝÀç¿ø", "ÅëÇÕ", "ÅëÇÕ"]
    configs["General"] := ["ÀÏ¹ÝÀç¿ø", "ÀÏ¹Ý", "ÀÏ¹Ý"]
    for name, config in configs
    {
        base := "SSOK_BudgetDashboard" . name
        if (control != base . "View" && control != base . "Sort")
            continue
        GuiControlGet, view, SSOKBudgetDashboard:, %base%View
        SSOK_Expense_BudgetDashboard_FillResourceCategory(config[1], config[2], view, base . "LV", base . "Status", config[3])
        break
    }
}
...
{
    global SSOK_BudgetDashboardParsed
    global SSOK_BudgetDashboardPurposeProjectView, SSOK_BudgetDashboardPurposeNationalView, SSOK_BudgetDashboardPurposeUserView
    if !IsObject(SSOK_BudgetDashboardParsed)
        return

    global SSOK_BudgetDashboardPurposeAllView
    Gui, SSOKBudgetDashboard:Submit, NoHide
    SSOK_Expense_BudgetDashboard_FillResourceCategory("¸ñÀûÀç¿ø", "", SSOK_BudgetDashboardPurposeAllView
        , "SSOK_BudgetDashboardPurposeAllLV", "SSOK_BudgetDashboardPurposeAllStatus", "¸ñÀûÀç¿ø ÀüÃ¼")
    SSOK_Expense_BudgetDashboard_FillResourceCategory("¸ñÀûÀç¿ø", "¸ñÀû»ç¾÷", SSOK_BudgetDashboardPurposeProjectView
        , "SSOK_BudgetDashboardPurposeProjectLV", "SSOK_BudgetDashboardPurposeProjectStatus", "¸ñÀû»ç¾÷")
    SSOK_Expense_BudgetDashboard_FillResourceCategory("¸ñÀûÀç¿ø", "±¹°í»ç¾÷", SSOK_BudgetDashboardPurposeNationalView
        , "SSOK_BudgetDashboardPurposeNationalLV", "SSOK_BudgetDashboardPurposeNationalStatus", "±¹°íº¸Á¶±Ý")
    SSOK_Expense_BudgetDashboard_FillResourceCategory("¸ñÀûÀç¿ø", "¼öÀÍÀÚ", SSOK_BudgetDashboardPurposeUserView
        , "SSOK_BudgetDashboardPurposeUserLV", "SSOK_BudgetDashboardPurposeUserStatus", "¼öÀÍÀÚ")
}

SSOK_Expense_BudgetDashboard_RefreshGeneralLists()
{
    global SSOK_BudgetDashboardParsed
    global SSOK_BudgetDashboardFutureView, SSOK_BudgetDashboardSpecialView
    global SSOK_BudgetDashboardIntegratedView, SSOK_BudgetDashboardGeneralView
    if !IsObject(SSOK_BudgetDashboardParsed)
        return

    global SSOK_BudgetDashboardGeneralAllView
    Gui, SSOKBudgetDashboard:Submit, NoHide
    SSOK_Expense_BudgetDashboard_FillResourceCategory("ÀÏ¹ÝÀç¿ø", "", SSOK_BudgetDashboardGeneralAllView
        , "SSOK_BudgetDashboardGeneralAllLV", "SSOK_BudgetDashboardGeneralAllStatus", "ÀÏ¹ÝÀç¿ø ÀüÃ¼")
    SSOK_Expense_BudgetDashboard_FillResourceCategory("ÀÏ¹ÝÀç¿ø", "¹Ì·¡", SSOK_BudgetDashboardFutureView
        , "SSOK_BudgetDashboardFutureLV", "SSOK_BudgetDashboardFutureStatus", "¹Ì·¡")
    SSOK_Expense_BudgetDashboard_FillResourceCategory("ÀÏ¹ÝÀç¿ø", "Æ¯¼º", SSOK_BudgetDashboardSpecialView
        , "SSOK_BudgetDashboardSpecialLV", "SSOK_BudgetDashboardSpecialStatus", "Æ¯¼º")
    SSOK_Expense_BudgetDashboard_FillResourceCategory("ÀÏ¹ÝÀç¿ø", "ÅëÇÕ", SSOK_BudgetDashboardIntegratedView
        , "SSOK_BudgetDashboardIntegratedLV", "SSOK_BudgetDashboardIntegratedStatus", "ÅëÇÕ")
    SSOK_Expense_BudgetDashboard_FillResourceCategory("ÀÏ¹ÝÀç¿ø", "ÀÏ¹Ý", SSOK_BudgetDashboardGeneralView
        , "SSOK_BudgetDashboardGeneralLV", "SSOK_BudgetDashboardGeneralStatus", "ÀÏ¹Ý")
}

SSOK_Expense_BudgetDashboard_FillResourceCategory(resourceType, category, viewChoice, listViewName, statusControlName, displayCategory)
{
    viewMode := SSOK_Expense_BudgetDashboard_NormalizeManagementView(viewChoice)
    entries := SSOK_Expense_BudgetDashboard_BuildResourceManagementEntries(resourceType, category, viewMode)
    sortControl := StrReplace(listViewName, "LV", "Sort")
    GuiControlGet, sortMode, SSOKBudgetDashboard:, %sortControl%
    for _, entry in entries
        entry.label := entry.viewLabel
    SSOK_Expense_BudgetDashboard_SortEntries(entries, sortMode)
    ...
    for _, entry in entries
    {
        LV_Add("", viewMode = "¿ø°¡Åë°èºñ¸ñº°" ? "" : entry.business
            , (viewMode = "Ç×¸ñº°" || viewMode = "³»¿ªº°") ? entry.subItem : ""
            , viewMode = "³»¿ªº°" ? entry.calc : ""
            , (viewMode = "³»¿ªº°" || viewMode = "¿ø°¡Åë°èºñ¸ñº°") ? entry.cost : ""
            , SSOK_Expense_FormatNumber(entry.current), SSOK_Expense_FormatNumber(entry.commit), SSOK_Expense_FormatNumber(entry.paid), SSOK_Expense_FormatNumber(entry.commit - entry.paid), SSOK_Expense_FormatNumber(entry.current - entry.commit), SSOK_Expense_BudgetDashboard_FormatRate(entry.current, entry.commit)
            , entry.managers)
        totalBudget += entry.current
        totalCommit += entry.commit
        totalPaid += entry.paid
    }

    SSOK_Expense_BudgetDashboard_SetTopSummary(listViewName, 5, totalBudget, totalCommit, totalPaid)
    SSOK_Expense_BudgetDashboard_FormatResourceManagementColumns()
    GuiControl, SSOKBudgetDashboard:+Redraw, %listViewName%
    status := SSOK_Expense_BudgetDashboard_MakeDetailedSummary(displayCategory . " ¡¤ " . viewMode . " ÇÕ°è"
        , entries.Length(), totalBudget, totalCommit, totalPaid)
    GuiControl, SSOKBudgetDashboard:, %statusControlName%, %status%
    SSOK_Expense_BudgetDashboard_UpdateColors(listViewName, entries, category, viewMode)
}

SSOK_Expense_BudgetDashboard_NormalizeManagementView(viewChoice)
{
    if (viewChoice = "¼¼ºÎÇ×¸ñº°" || viewChoice = "Ç×¸ñº°")
        return "Ç×¸ñº°"
    if (viewChoice = "¼¼ºÎ³»¿ªº°" || viewChoice = "³»¿ªº°")
        return "³»¿ªº°"
    if (viewChoice = "¿ø°¡Åë°èºñ¸ñº°")
        return viewChoice
    return "»ç¾÷º°"
}
...
SSOK_Expense_BudgetDashboard_FormatResourceManagementColumns()
{
    widths := [130,130,200,115,100,105,100,105,110,100,170]
    for col, width in widths
        LV_ModifyCol(col, (col >= 5 && col <= 10 ? "Right " : "") . width)
}

SSOK_Expense_BudgetDashboard_BuildResourceManagementEntries(resourceType, category, viewMode)
{
    global SSOK_BudgetDashboardParsed
    groups := {}
    entries := []
    ...
    for _, row in SSOK_Expense_BudgetDashboard_FilterRows()
    {
        ...
        if (resourceType = "¸ñÀûÀç¿ø")
        {
            if (category != "" && row.purposeCategory != category)
                continue
        }
        else
        {
            if (category != "" && row.generalCategory != category)
                continue
        }

        business := (row.business != "") ? row.business : "(¼¼ºÎ»ç¾÷ ¾øÀ½)"
        subItem := (row.subItem != "") ? row.subItem : "(¼¼ºÎÇ×¸ñ ¾øÀ½)"
        calc := (row.calc != "") ? row.calc : "(»êÃâ³»¿ª ¾øÀ½)"

        if (viewMode = "³»¿ªº°")
        {
            key := business . Chr(30) . subItem . Chr(30) . row.cost . Chr(30) . calc
            viewLabel := business . " > " . subItem . " > " . calc . " > " . row.cost
        }
        else if (viewMode = "¿ø°¡Åë°èºñ¸ñº°")
        {
            key := row.cost != "" ? row.cost : "(¿ø°¡Åë°èºñ¸ñ ¾øÀ½)"
            viewLabel := key
        }
        else if (viewMode = "Ç×¸ñº°")
        {
            key := business . Chr(30) . subItem
            viewLabel := business . " > " . subItem
        }
        else
        {
            key := business
        ...
        if !IsObject(groups[key])
            groups[key] := {category:category
                , groupName:(resourceType = "ÀÏ¹ÝÀç¿ø" ? row.generalGroupName : category)
                , business:business
                , subItem:subItem
                , calc:calc
                , cost:row.cost
        ...
        g.commit += row.commit
        g.paid += row.paid
        g.managers := SSOK_Expense_BudgetDashboard_AppendManager(g.managers, row.manager)
    }
...
SSOK_Expense_BudgetDashboard_RefreshWorkExpenseList()
{
    ...
    SSOK_Expense_BudgetDashboard_FillWorkExpenseCategory("", "SSOK_BudgetDashboardWorkAllLV", "SSOK_BudgetDashboardWorkAllStatus")
    SSOK_Expense_BudgetDashboard_FillWorkExpenseCategory("ÀÏ¹ÝÀç¿ø", "SSOK_BudgetDashboardWorkGeneralLV", "SSOK_BudgetDashboardWorkGeneralStatus")
    SSOK_Expense_BudgetDashboard_FillWorkExpenseCategory("¸ñÀûÀç¿ø", "SSOK_BudgetDashboardWorkPurposeLV", "SSOK_BudgetDashboardWorkPurposeStatus")
}

SSOK_Expense_BudgetDashboard_FillWorkExpenseCategory(resourceType, listViewName, statusControlName)
{
    entries := SSOK_Expense_BudgetDashboard_BuildWorkExpenseEntries(resourceType)
    Gui, SSOKBudgetDashboard:Default
    Gui, SSOKBudgetDashboard:ListView, %listViewName%
    LV_Delete()
    ...
    label := resourceType = "" ? "¾÷¹«ÃßÁøºñ ÀüÃ¼ ÇÕ°è" : "¾÷¹«ÃßÁøºñ " . resourceType . " ÇÕ°è"
    status := SSOK_Expense_BudgetDashboard_MakeDetailedSummary(label, entries.Length(), totalBudget, totalCommit, totalPaid)
    GuiControl, SSOKBudgetDashboard:, %statusControlName%, %status%
    SSOK_Expense_BudgetDashboard_UpdateColors(listViewName)
}

SSOK_Expense_BudgetDashboard_BuildWorkExpenseEntries(resourceType := "")
{
    global SSOK_BudgetDashboardParsed
    entries := []
    ...
    for _, row in SSOK_Expense_BudgetDashboard_FilterRows()
    {
        if !InStr(row.cost, "¾÷¹«ÃßÁøºñ")
            continue
        if (resourceType != "" && row.resourceType != resourceType)
            continue
        entries.Push({cost:row.cost
            ...
            , commit:row.commit
            , paid:row.paid
            , manager:(row.manager != "" ? row.manager : "´ã´çÀÚ ¾øÀ½")})
    }
    return entries
}

SSOK_Expense_BudgetDashboard_MakeDetailedSummary(label, count, current, commit, paid)
{
    return label . " " . count . "°Ç"
        . "  |  ¿¹»êÇö¾× " . SSOK_Expense_FormatNumber(current) . "¿ø"
        . "  |  ¿øÀÎÇàÀ§¾× " . SSOK_Expense_FormatNumber(commit) . "¿ø"
        . "  |  ¿øÀÎÇàÀ§ ÁýÇà·ü " . SSOK_Expense_BudgetDashboard_FormatRate(current, commit)
        . "  |  ¿¹»êÀÜ¾× " . SSOK_Expense_FormatNumber(current - commit) . "¿ø"
        . "  |  Áö±Þ¾× " . SSOK_Expense_FormatNumber(paid) . "¿ø"
        . "  |  Áö±Þ ÁýÇà·ü " . SSOK_Expense_BudgetDashboard_FormatRate(current, paid)
        . "  |  ÁöÃâÀÜ¾× " . SSOK_Expense_FormatNumber(current - paid) . "¿ø"
}

SSOK_Expense_BudgetDashboard_AppendManager(currentText, manager)
{
    names := [], seen := {}
    for _, name in StrSplit(currentText . "," . manager, ",")
    {
        name := Trim(name)
...
SSOK_Expense_BudgetDashboard_SortResourceManagementEntries(ByRef entries)
{
    for _, entry in entries
        entry.label := entry.viewLabel != "" ? entry.viewLabel : entry.category . " / " . entry.groupName . " / " . entry.business . " / " . entry.subItem
    SSOK_Expense_BudgetDashboard_SortEntries(entries, "ÀÌ¸§¼ø")
}
...
SSOK_Expense_BudgetDashboard_SortEntries(ByRef entries, sortMode)
{
    ...
    while (width < count)
    {
        ...
        while (start <= count)
        {
            ...
            j := middle
            finish := Min(start + width * 2, count + 1)
            while (i < middle || j < finish)
            {
                if (j >= finish || (i < middle && !SSOK_Expense_BudgetDashboard_ShouldSwap(entries[i], entries[j], sortMode)))
...
SSOK_Expense_BudgetDashboard_ShouldSwap(a, b, sortMode)
{
    ...
    if (sortMode = "¹ÌÁöÃâ¾× Å«¼ø")
    {
        va := a.commit - a.paid, vb := b.commit - b.paid
        return va = vb ? (a.label > b.label) : (va < vb)
    }
    if (sortMode = "ºÒ¿ë¾× ¿ì¼±¼ø")
    {
        va := Max(0, SSOK_Expense_BudgetDashboard_RateSortValue(a.current, a.commit))
        vb := Max(0, SSOK_Expense_BudgetDashboard_RateSortValue(b.current, b.commit))
        if (va != vb)
            return va > vb
        va := a.current - a.commit, vb := b.current - b.commit
        return va = vb ? (a.label > b.label) : (va < vb)
    }
    if (sortMode = "Ç°ÀÇ¾× Å«¼ø")
    {
        va := a.draft
        vb := b.draft
        descending := true
    }
    else if (sortMode = "¿øÀÎÇàÀ§ Å«¼ø")
    {
        va := a.commit
        vb := b.commit
        descending := true
    }
    else if (sortMode = "ÁýÇà·ü ³ôÀº¼ø" || sortMode = "ÁýÇà·ü ³·Àº¼ø")
    {
        ...
        descending := (sortMode = "ÁýÇà·ü ³ôÀº¼ø")
    }
    else if (sortMode = "Áö±Þ¾× Å«¼ø" || sortMode = "ÀÜ¾× Å«¼ø")
    {
        va := sortMode = "Áö±Þ¾× Å«¼ø" ? a.paid : a.current - a.commit
        vb := sortMode = "Áö±Þ¾× Å«¼ø" ? b.paid : b.current - b.commit
        descending := true
    }
    else if (sortMode = "¸ñÀûÀç¿ø Å«¼ø")
    {
        va := a.settle
        vb := b.settle
        descending := true
    }
    else if (sortMode = "¸ñÀû ÁýÇà·ü ³ôÀº¼ø")
    {
    ...
    else if (sortMode = "ÀÏ¹ÝÀç¿ø Å«¼ø")
    {
        va := a.general
        vb := b.general
        descending := true
    }
    else if (sortMode = "ÀÏ¹Ý ÁýÇà·ü ³ôÀº¼ø")
    {
...
SSOK_Expense_BudgetDashboard_XmlText(value)
{
    value := RegExReplace(value, "[\x00-\x08\x0B\x0C\x0E-\x1F]", "")
    return StrReplace(StrReplace(StrReplace(StrReplace(value, "&", "&amp;"), "<", "&lt;"), ">", "&gt;"), """", "&quot;")
}

SSOK_Expense_BudgetDashboard_ExportXml(listViewName, headers, outputPath)
{
    Gui, SSOKBudgetDashboard:Default
    Gui, SSOKBudgetDashboard:ListView, %listViewName%
    columns := StrSplit(headers, "|")
    xml := "<?xml version=""1.0"" encoding=""UTF-8""?><?mso-application progid=""Excel.Sheet""?>"
        . "<Workbook xmlns=""urn:schemas-microsoft-com:office:spreadsheet"" xmlns:ss=""urn:schemas-microsoft-com:office:spreadsheet""><Worksheet ss:Name=""¿¹»êDashboard""><Table>"
    Loop, % LV_GetCount() + 1
    {
        row := A_Index - 1
        xml .= "<Row>"
        for column, header in columns
        {
            if (row = 0)
                ...
                LV_GetText(value, row, column)
            numeric := row > 0 && RegExMatch(header, "¾×$|ÀÜ¾×$|Àç¿ø$") && RegExMatch(value, "^-?\d[\d,]*(?:\.\d+)?$")
            if (numeric)
                value := StrReplace(value, ",")
            xml .= "<Cell><Data ss:Type=""" . (numeric ? "Number" : "String") . """>" . SSOK_Expense_BudgetDashboard_XmlText(value) . "</Data></Cell>"
        }
        xml .= "</Row>"
    }
    xml .= "</Table></Worksheet></Workbook>"
    file := FileOpen(outputPath, "w", "UTF-8")
    if !IsObject(file)
        throw Exception("ÆÄÀÏÀ» ÀúÀåÇÒ ¼ö ¾ø½À´Ï´Ù: " . outputPath)
    written := file.Write(xml)
    file.Close()
    if (!written)
        throw Exception("XML ÆÄÀÏ ¾²±â ½ÇÆÐ")
    return true
}

SSOK_Expense_BudgetDashboard_ExportListView(listViewName, exportName, headers, statusControlName := "")
{
    global SSOK_BudgetDashboardFilePath
    Gui, SSOKBudgetDashboard:Default
    Gui, SSOKBudgetDashboard:ListView, %listViewName%
    if (LV_GetCount() < 1)
    {
        MsgBox, 48, ¿¹»ê DashBoard, ÀÌ ÅÇ¿¡ Excel·Î ÃßÃâÇÒ ÀÚ·á°¡ ¾ø½À´Ï´Ù.
        return false
    }
    SplitPath, SSOK_BudgetDashboardFilePath,, sourceDir
    if (sourceDir = "")
        sourceDir := A_Desktop
    FormatTime, stamp,, yyyyMMdd-HHmmss
    safeName := RegExReplace(exportName, "[\\/:*?<>|""\x00-\x1F]", "_")
    outputPath := sourceDir . "\¿¹»êDashboard_" . safeName . "_" . stamp . "_" . A_MSec . ".xlsx"
    try
    {
        SSOK_Expense_BudgetDashboard_WriteXlsx(listViewName, headers, outputPath)
        Run, %outputPath%,, UseErrorLevel
        ToolTip, % "Excel ÃßÃâ ¿Ï·á: " . outputPath
        SetTimer, SSOKExpenseClearTip, -2000
        return true
    }
    catch e
    {
        MsgBox, 48, ¿¹»ê DashBoard, % "XLSX ÀúÀå ½ÇÆÐ:`n" . e.Message
        return false
    }
}
...
SSOK_Expense_BudgetDashboard_WriteXlsx(listViewName, headers, outputPath)
{
    ...
    for col, header in columns
        sheet .= "<col min=""" . col . """ max=""" . col . """ width=""" . (RegExMatch(header, "¾×|ÀÜ¾×|ÁýÇà·ü") ? 18 : 30) . """ customWidth=""1""/>"
    ...
    Loop, % rowCount + 1
    {
        ...
        for col, header in columns
        {
            if (row = 0)
                ...
                LV_GetText(value, row, col)
            style := SSOK_BudgetDashboardRowColors[handle][row]
            fill := style.red ? 1 : (style.yellow = col ? 2 : 0)
            negative := style.negative.HasKey(col) ? 1 : 0
            baseStyle := 4 + (fill * 2 + negative) * 3
            number := row > 0 && RegExMatch(value, "^-?\d[\d,]*(?:\.\d+)?%?$") && RegExMatch(header, "¾×|ÀÜ¾×|ÁýÇà·ü|Àç¿ø")
            if (number)
            {
                percent := InStr(value, "%")
                value := StrReplace(StrReplace(value, ","), "%") + 0
                if (percent)
                    value /= 100
                sheet .= "<c s=""" . (baseStyle + (percent ? 2 : 1)) . """><v>" . value . "</v></c>"
            }
            else
                sheet .= "<c t=""inlineStr"" s=""" . (row = 0 ? 3 : baseStyle) . """><is><t xml:space=""preserve"">" . SSOK_Expense_BudgetDashboard_XmlText(value) . "</t></is></c>"
        }
        sheet .= "</row>"
    }
    lastColumn := "", col := columns.Length()
    while (col > 0)
    {
        col--
        lastColumn := Chr(65 + Mod(col, 26)) . lastColumn
        col := Floor(col / 26)
    }
    sheet .= "</sheetData><autoFilter ref=""A1:" . lastColumn . (rowCount + 1) . """/></worksheet>"
    root := A_Temp . "\SSOK_BudgetXlsx_" . DllCall("GetCurrentProcessId") . "_" . A_TickCount
    package := root . "\package"
    FileCreateDir, %package%\_rels
    FileCreateDir, %package%\xl\_rels
    FileCreateDir, %package%\xl\worksheets
    parts := {}
    ...
    parts["_rels\.rels"] := "<Relationships xmlns=""http://schemas.openxmlformats.org/package/2006/relationships""><Relationship Id=""rId1"" Type=""http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument"" Target=""xl/workbook.xml""/></Relationships>"
    parts["xl\workbook.xml"] := "<workbook xmlns=""http://schemas.openxmlformats.org/spreadsheetml/2006/main"" xmlns:r=""http://schemas.openxmlformats.org/officeDocument/2006/relationships""><sheets><sheet name=""¿¹»êDashboard"" sheetId=""1"" r:id=""rId1""/></sheets></workbook>"
    parts["xl\_rels\workbook.xml.rels"] := "<Relationships xmlns=""http://schemas.openxmlformats.org/package/2006/relationships""><Relationship Id=""rId1"" Type=""http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet"" Target=""worksheets/sheet1.xml""/><Relationship Id=""rId2"" Type=""http://schemas.openxmlformats.org/officeDocument/2006/relationships/styles"" Target=""styles.xml""/></Relationships>"
    parts["xl\styles.xml"] := SSOK_Expense_BudgetDashboard_XlsxStyles()
    parts["xl\worksheets\sheet1.xml"] := sheet
    for path, content in parts
    {
        file := FileOpen(package . "\" . path, "w", "UTF-8")
        if !IsObject(file)
            throw Exception("ÀÓ½Ã ÆÄÀÏ »ý¼º ½ÇÆÐ: " . path)
        file.Write(content), file.Close()
    }
    ; Literal PowerShell paths: embedded single quotes are doubled, never interpolated as code.
    ps := "$ErrorActionPreference = 'Stop'`r`nAdd-Type -AssemblyName System.IO.Compression.FileSystem`r`n"
        . "[IO.Compression.ZipFile]::CreateFromDirectory('" . StrReplace(package, "'", "''") . "', '" . StrReplace(outputPath, "'", "''") . "')`r`n"
    script := root . "\pack.ps1"
    file := FileOpen(script, "w", "UTF-8")
    file.Write(ps), file.Close()
    executable := A_WinDir . "\System32\WindowsPowerShell\v1.0\powershell.exe"
    RunWait, % """" . executable . """ -NoProfile -NonInteractive -ExecutionPolicy Bypass -File """ . script . """",, Hide UseErrorLevel
    result := ErrorLevel
    ; root is a unique child of A_Temp assembled solely from our process ID and tick count.
    FileRemoveDir, %root%, 1
    if (result != 0 || !FileExist(outputPath))
        throw Exception("XLSX ¾ÐÃà ÀúÀå ½ÇÆÐ (" . result . "): " . outputPath)
    return true
}
...
SSOK_Expense_BudgetDashboard_CopyCurrentView()
{
    ...
    if (rowCount < 1)
    {
        MsgBox, 48, ¿¹»ê DashBoard, º¹»çÇÒ Ç¥½Ã ÇàÀÌ ¾ø½À´Ï´Ù.
        return
    }

    out := "¼¼ºÎ»ç¾÷`t¼¼ºÎÇ×¸ñ`t¼¼ºÎ³»¿ª`t¿ø°¡Åë°èºñ¸ñ`t¿¹»êÇö¾×`t°è¾à±Ý¾× (¿øÀÎÇàÀ§)`tÁö±Þ¾×`t¹ÌÁöÃâ¾× (Áö±Þ´ë±â)`t¿¹»êÀÜ¾× (ºÒ¿ë¿¹»ê¾×)`tÁýÇà·ü (¿øÀÎÇàÀ§)`t¸ñÀûÀç¿ø`tÀÏ¹ÝÀç¿ø`t´ã´çÀÚ`r`n"
    Loop, %rowCount%
    {
        rowNo := A_Index
        line := ""
    ...
    if ErrorLevel
    {
        MsgBox, 48, ¿¹»ê DashBoard, ÇöÀç Ç¥¸¦ Å¬¸³º¸µå¿¡ º¹»çÇÏÁö ¸øÇß½À´Ï´Ù.
        return
    }

    ToolTip, ÇöÀç ´ë½Ãº¸µå Ç¥¸¦ Excel ºÙ¿©³Ö±â Çü½ÄÀ¸·Î º¹»çÇß½À´Ï´Ù.
    SetTimer, SSOKExpenseClearTip, -1800
}

SSOK_Expense_BudgetDashboard_PickFile:
    FileSelectFile, chosen, 3,, ¿¹»ê Excel ÆÄÀÏ ¼±ÅÃ, Excel (*.xls; *.xlsx; *.xlsm)
    if (chosen != "")
        SSOK_Expense_BudgetDashboard_LoadFile(chosen)
return

SSOK_Expense_BudgetDashboard_ExportPending:
    SSOK_Expense_BudgetDashboard_ExportListView("SSOK_BudgetDashboardPendingLV", "¹ÌÁöÃâ¾×_°ü¸®", "¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|°è¾à±Ý¾× (¿øÀÎÇàÀ§)|Áö±Þ¾×|¹ÌÁöÃâ¾× (Áö±Þ´ë±â)|¿¹»êÀÜ¾× (ºÒ¿ë¿¹»ê¾×)|ÁýÇà·ü (¿øÀÎÇàÀ§)|´ã´çÀÚ")
return

SSOK_Expense_BudgetDashboard_ExportUnused:
    SSOK_Expense_BudgetDashboard_ExportListView("SSOK_BudgetDashboardUnusedLV", "ºÒ¿ë¾×_°ü¸®", "¼¼ºÎ»ç¾÷|¼¼ºÎÇ×¸ñ|¼¼ºÎ³»¿ª|¿ø°¡Åë°èºñ¸ñ|¿¹»êÇö¾×|°è¾à±Ý¾× (¿øÀÎÇàÀ§)|Áö±Þ¾×|¹ÌÁöÃâ¾× (Áö±Þ´ë±â)|¿¹»êÀÜ¾× (ºÒ¿ë¿¹»ê¾×)|ÁýÇà·ü (¿øÀÎÇàÀ§)|´ã´çÀÚ")
return

SSOK_Expense_BudgetDashboard_FillBalance(kind)
{
    name := "SSOK_BudgetDashboard" . kind . "LV"
    control := "SSOK_BudgetDashboard" . kind . "View"
    GuiControlGet, view, SSOKBudgetDashboard:, %control%
    qualifying := []
    for _, row in SSOK_Expense_BudgetDashboard_FilterRows()
    {
        amount := kind = "Pending" ? row.commit - row.paid : row.current - row.commit
        if (amount > 0)
            qualifying.Push(row)
    }
    entries := view = "»êÃâ³»¿ªº°" ? qualifying : SSOK_Expense_BudgetDashboard_BuildEntries(view, "ÀüÃ¼", false, qualifying)
    SSOK_Expense_BudgetDashboard_SortEntries(entries, kind = "Pending" ? "¹ÌÁöÃâ¾× Å«¼ø" : "ºÒ¿ë¾× ¿ì¼±¼ø")
    Gui, SSOKBudgetDashboard:Default
    Gui, SSOKBudgetDashboard:ListView, %name%
    LV_Delete()
    current := 0, commit := 0, paid := 0
    for _, row in entries
    {
        LV_Add("", view = "¿ø°¡Åë°èºñ¸ñº°" ? "" : row.business
            , view = "¼¼ºÎ»ç¾÷º°" || view = "¿ø°¡Åë°èºñ¸ñº°" ? "" : row.subItem
            , view = "»êÃâ³»¿ªº°" ? row.calc : ""
            , view = "»êÃâ³»¿ªº°" || view = "¿ø°¡Åë°èºñ¸ñº°" ? row.cost : ""
            , SSOK_Expense_FormatNumber(row.current), SSOK_Expense_FormatNumber(row.commit)
            , SSOK_Expense_FormatNumber(row.paid), SSOK_Expense_FormatNumber(row.commit - row.paid)
            , SSOK_Expense_FormatNumber(row.current - row.commit), SSOK_Expense_BudgetDashboard_FormatRate(row.current, row.commit), row.manager)
        current += row.current, commit += row.commit, paid += row.paid
    }
    SSOK_Expense_BudgetDashboard_FormatResourceManagementColumns()
    SSOK_Expense_BudgetDashboard_SetTopSummary(name, 5, current, commit, paid)
    SSOK_Expense_BudgetDashboard_UpdateColors(name)
}

SSOK_Expense_BudgetDashboard_SetHeader(name)
{
    global SSOK_BudgetDashboardHeaders, SSOK_BudgetDashboardFonts
    GuiControlGet, table, SSOKBudgetDashboard:Hwnd, %name%
    header := DllCall("SendMessageW", "Ptr", table, "UInt", 0x101F, "Ptr", 0, "Ptr", 0, "Ptr")
    font := DllCall("SendMessageW", "Ptr", table, "UInt", 0x31, "Ptr", 0, "Ptr", 0, "Ptr")
    VarSetCapacity(lf, 92, 0)
    DllCall("GetObjectW", "Ptr", font, "Int", 92, "Ptr", &lf)
    NumPut(-Round(28 * A_ScreenDPI / 96), lf, 0, "Int")
    tallFont := DllCall("CreateFontIndirectW", "Ptr", &lf, "Ptr")
    SSOK_BudgetDashboardFonts["header" . header] := tallFont
    static callback := RegisterCallback("SSOK_Expense_BudgetDashboard_HeaderProc", "", 4)
    original := DllCall("SetWindowLongPtrW", "Ptr", header, "Int", -4, "Ptr", callback, "Ptr")
    SSOK_BudgetDashboardHeaders[header] := {table:table, font:font, original:original}
    DllCall("SendMessageW", "Ptr", header, "UInt", 0x30, "Ptr", tallFont, "Ptr", 1)
    ; Recalculate the native header/list client layout using its new font height.
    GuiControl, SSOKBudgetDashboard:MoveDraw, %name%
}

SSOK_Expense_BudgetDashboard_HeaderProc(hwnd, msg, wParam, lParam)
{
    Critical
    global SSOK_BudgetDashboardHeaders
    info := SSOK_BudgetDashboardHeaders[hwnd]
    result := DllCall("CallWindowProcW", "Ptr", info.original, "Ptr", hwnd, "UInt", msg, "Ptr", wParam, "Ptr", lParam, "Ptr")
    if (msg != 0xF)
        return result
    hdc := DllCall("GetDC", "Ptr", hwnd, "Ptr")
    previous := DllCall("SelectObject", "Ptr", hdc, "Ptr", info.font, "Ptr")
    count := DllCall("SendMessageW", "Ptr", hwnd, "UInt", 0x1200, "Ptr", 0, "Ptr", 0)
    Loop, %count%
    {
        index := A_Index - 1
        VarSetCapacity(rect, 16, 0)
        DllCall("SendMessageW", "Ptr", hwnd, "UInt", 0x1207, "Ptr", index, "Ptr", &rect)
        DllCall("FillRect", "Ptr", hdc, "Ptr", &rect, "Ptr", DllCall("GetSysColorBrush", "Int", 15, "Ptr"))
        DllCall("DrawEdge", "Ptr", hdc, "Ptr", &rect, "UInt", 4, "UInt", 4)
        VarSetCapacity(text, 1024, 0)
        VarSetCapacity(column, 56, 0)
        NumPut(4, column, 0, "UInt")
        NumPut(&text, column, 16, "Ptr")
        NumPut(512, column, 24, "Int")
        DllCall("SendMessageW", "Ptr", info.table, "UInt", 0x105F, "Ptr", index, "Ptr", &column)
        label := StrReplace(StrGet(&text, "UTF-16"), " (", "`n(")
        NumPut(NumGet(rect, 1*4, "Int") + 4, rect, 1*4, "Int")
        DllCall("SetBkMode", "Ptr", hdc, "Int", 1)
        DllCall("SetTextColor", "Ptr", hdc, "UInt", 0x000000)
        DllCall("DrawTextW", "Ptr", hdc, "WStr", label, "Int", -1, "Ptr", &rect, "UInt", 0x811)
    }
...
SSOK_Expense_BudgetDashboard_OverViewRefresh:
    SSOK_Expense_BudgetDashboard_FillOver()
return

SSOK_Expense_BudgetDashboard_PendingViewRefresh:
    SSOK_Expense_BudgetDashboard_FillBalance("Pending")
return

SSOK_Expense_BudgetDashboard_UnusedViewRefresh:
    SSOK_Expense_BudgetDashboard_FillBalance("Unused")
return
...
SSOK_Expense_CardCompare_Reset:
    ...
    GuiControl, SSOKCardCompare:, SSOK_CardCompareUsePath, ¹ýÀÎÄ«µå»ç¿ëºÎ Excel ÆÄÀÏÀ» ¿©±â¿¡ ³õ¾ÆÁÖ¼¼¿ä
    GuiControl, SSOKCardCompare:, SSOK_CardCompareStatementPath, ¹ýÀÎÄ«µåÀÌ¿ë³»¿ª¼­ Excel ÆÄÀÏÀ» ¿©±â¿¡ ³õ¾ÆÁÖ¼¼¿ä
...
SSOK_Expense_CardCompare_Show()
{
    ...
    Gui, SSOKCardCompare:Destroy
    Gui, SSOKCardCompare:New, +ToolWindow, ¾÷¹«¿ëµµ±¸ - ¹ýÀÎÄ«µå³»¿ªºñ±³
    Gui, SSOKCardCompare:Margin, 20, 18
    Gui, SSOKCardCompare:Color, F7FBFF

    Gui, SSOKCardCompare:Font, s13 Bold, Malgun Gothic
    Gui, SSOKCardCompare:Add, Text, x20 y16 w760 h30 c005BAC, ¾÷¹«¿ëµµ±¸ - ¹ýÀÎÄ«µå³»¿ªºñ±³

    Gui, SSOKCardCompare:Font, s10 Bold, Malgun Gothic
    useDisplay := FileExist(SSOK_CardCompareUsePath) ? SSOK_CardCompareUsePath : "¹ýÀÎÄ«µå»ç¿ëºÎ Excel ÆÄÀÏÀ» ¿©±â¿¡ ³õ¾ÆÁÖ¼¼¿ä"
    ...
    Gui, SSOKCardCompare:Add, Text, x20 y154 w650 h24 c555555, ÀÚ·áÃâÃ³: ÇÐ±³È¸°è-ÁöÃâ°ü¸®-±âÅ¸°ü¸®-¹ýÀÎÄ«µå»ç¿ëºÎ
    ...
    Gui, SSOKCardCompare:Font, s10 Bold, Malgun Gothic
    statementDisplay := FileExist(SSOK_CardCompareStatementPath) ? SSOK_CardCompareStatementPath : "¹ýÀÎÄ«µåÀÌ¿ë³»¿ª¼­ Excel ÆÄÀÏÀ» ¿©±â¿¡ ³õ¾ÆÁÖ¼¼¿ä"
    ...
    Gui, SSOKCardCompare:Font, s9 Norm, Malgun Gothic
    Gui, SSOKCardCompare:Add, Text, x20 y286 w650 h24 c555555, ÀÚ·áÃâÃ³: ³óÇù ÀÎÅÍ³Ý¹ðÅ· - ¿ùº° ÀÌ¿ë³»¿ª¼­ ¿¢¼¿ ÀÚ·á ´Ù¿î·Îµå
    Gui, SSOKCardCompare:Font, s9 Underline, Malgun Gothic
    ...
    Gui, SSOKCardCompare:Font, s9 Norm, Malgun Gothic
    Gui, SSOKCardCompare:Add, Button, x20 y334 w150 h38 gSSOK_Expense_CardCompare_Reset, ÃÊ±âÈ­
    Gui, SSOKCardCompare:Add, Button, x184 yp w150 h38 gSSOK_Expense_CardCompare_Check Default, ÀÛ¾÷½ÇÇà

    Gui, SSOKCardCompare:Show, w800 h392 Center, ¾÷¹«¿ëµµ±¸ - ¹ýÀÎÄ«µå³»¿ªºñ±³
}
...
SSOK_Expense_WorkPublic_Check:
    ...
    GuiControl, SSOKWorkPublic:, SSOK_WorkPublicPrevDrop, Àü¿ù ¾÷¹«ÃßÁøºñ ÁýÇà³»¿ª Excel ÆÄÀÏÀ» ¿©±â¿¡ ³õ¾ÆÁÖ¼¼¿ä (¼±ÅÃ)
    GuiControl, SSOKWorkPublic:, SSOK_WorkPublicCurrentDrop, ¿øÀÎÇàÀ§ ÁýÇà½ÇÀû Excel ÆÄÀÏÀ» ¿©±â¿¡ ³õ¾ÆÁÖ¼¼¿ä (ÇÊ¼ö)
    GuiControl, SSOKWorkPublic:, SSOK_WorkPublicCardDrop, Ä«µå½ÂÀÎ³»¿ª Excel ÆÄÀÏÀ» ¿©±â¿¡ ³õ¾ÆÁÖ¼¼¿ä (¼±ÅÃ)
...
SSOK_Expense_WorkPublic_Show()
{
    ...
    Gui, SSOKWorkPublic:Destroy
    Gui, SSOKWorkPublic:New, +ToolWindow, ¾÷¹«¿ëµµ±¸ - ¾÷¹«ÃßÁøºñ°ø°³
    Gui, SSOKWorkPublic:Margin, 20, 18
    Gui, SSOKWorkPublic:Color, F7FBFF

    Gui, SSOKWorkPublic:Font, s13 Bold, Malgun Gothic
    Gui, SSOKWorkPublic:Add, Text, x20 y16 w820 h30 c005BAC, ¾÷¹«¿ëµµ±¸ - ¾÷¹«ÃßÁøºñ°ø°³

    Gui, SSOKWorkPublic:Font, s10 Bold, Malgun Gothic
    prevDisplay := FileExist(SSOK_WorkPublicPrevPath) ? SSOK_WorkPublicPrevPath : "Àü¿ù ¾÷¹«ÃßÁøºñ ÁýÇà³»¿ª Excel ÆÄÀÏÀ» ¿©±â¿¡ ³õ¾ÆÁÖ¼¼¿ä (¼±ÅÃ)"
    Gui, SSOKWorkPublic:Add, Text, x20 y58 w820 h76 +Border Center 0x200 vSSOK_WorkPublicPrevDrop c555555, %prevDisplay%

    currentDisplay := FileExist(SSOK_WorkPublicCurrentPath) ? SSOK_WorkPublicCurrentPath : "¿øÀÎÇàÀ§ ÁýÇà½ÇÀû Excel ÆÄÀÏÀ» ¿©±â¿¡ ³õ¾ÆÁÖ¼¼¿ä (ÇÊ¼ö)"
    ...
    Gui, SSOKWorkPublic:Add, Text, x20 y238 w820 h24 c555555, ÀÚ·áÃâÃ³ 1: ÇÐ±³È¸°è-ÁöÃâ°ü¸®-ÁöÃâÃ³¸®-¿øÀÎÇàÀ§¸ñ·Ï-¾÷¹«ÃßÁøºñ¸ñ·Ï
    Gui, SSOKWorkPublic:Add, Text, x20 y264 w820 h24 c555555, * »ó¼¼³»¿ª ÀÔ·ÂµÈ °Í¸¸ º¸±â ÇØÁ¦ ÈÄ Á¶È¸
    Gui, SSOKWorkPublic:Add, Text, x20 y290 w820 h24 c555555, ÀÚ·áÃâÃ³ 2: ÇÐ±³È¸°è-ÁöÃâ°ü¸®-ÁöÃâÀåºÎ-ÁöÃâ½ÇÀûÁ¶È¸-¿øÀÎÇàÀ§-¿¹»ê°Å·¡Ã³º°½ÇÀûÁ¶È¸
    Gui, SSOKWorkPublic:Add, Text, x20 y316 w820 h24 c555555, * ¼¼Ãâ¼¼¸ñ¸í: ÀÏ¹Ý¾÷¹«ÃßÁøºñ·Î Á¶È¸

    Gui, SSOKWorkPublic:Font, s10 Bold, Malgun Gothic
    cardDisplay := FileExist(SSOK_WorkPublicCardPath) ? SSOK_WorkPublicCardPath : "Ä«µå½ÂÀÎ³»¿ª Excel ÆÄÀÏÀ» ¿©±â¿¡ ³õ¾ÆÁÖ¼¼¿ä (¼±ÅÃ)"
    Gui, SSOKWorkPublic:Add, Text, x20 y352 w820 h76 +Border Center 0x200 vSSOK_WorkPublicCardDrop c555555, %cardDisplay%

    Gui, SSOKWorkPublic:Font, s9 Norm, Malgun Gothic
    Gui, SSOKWorkPublic:Add, Text, x20 y440 w820 h24 c555555, ÀÚ·áÃâÃ³: ÇÐ±³È¸°è-ÁöÃâ°ü¸®-±âÅ¸°ü¸®-Ä«µå½ÂÀÎ³»¿ªÁ¶È¸
    Gui, SSOKWorkPublic:Add, Text, x20 y470 w820 h1 Hidden vSSOK_WorkPublicStatus,

    Gui, SSOKWorkPublic:Add, Button, x20 y486 w150 h38 gSSOK_Expense_WorkPublic_Check, ÃÊ±âÈ­
    Gui, SSOKWorkPublic:Add, Button, x184 yp w150 h38 gSSOK_Expense_WorkPublic_Convert Default, ÀÛ¾÷½ÇÇà

    Gui, SSOKWorkPublic:Show, w860 h544 Center, ¾÷¹«¿ëµµ±¸ - ¾÷¹«ÃßÁøºñ°ø°³
}
...
SSOK_Expense_BudgetDashboard_XlsxStyles()
{
    xml := "<styleSheet xmlns=""http://schemas.openxmlformats.org/spreadsheetml/2006/main""><fonts count=""3""><font><sz val=""11""/><name val=""¸¼Àº °íµñ""/></font><font><b/><sz val=""11""/><name val=""¸¼Àº °íµñ""/></font><font><b/><color rgb=""FFCC0000""/><sz val=""11""/><name val=""¸¼Àº °íµñ""/></font></fonts><fills count=""4""><fill><patternFill patternType=""none""/></fill><fill><patternFill patternType=""gray125""/></fill><fill><patternFill patternType=""solid""><fgColor rgb=""FFFFDADA""/><bgColor indexed=""64""/></patternFill></fill><fill><patternFill patternType=""solid""><fgColor rgb=""FFFFFF99""/><bgColor indexed=""64""/></patternFill></fill></fills><borders count=""1""><border/></borders><cellStyleXfs count=""1""><xf/></cellStyleXfs><cellXfs count=""22""><xf xfId=""0""/><xf xfId=""0"" numFmtId=""3"" applyNumberFormat=""1""/><xf xfId=""0"" numFmtId=""10"" applyNumberFormat=""1""/><xf xfId=""0"" fontId=""1"" applyFont=""1""/>"
    Loop, 3
    {
        fill := A_Index = 1 ? 0 : A_Index
        Loop, 2
        {
            font := A_Index = 1 ? 0 : 2
            for _, format in [0, 3, 10]
                xml .= "<xf xfId=""0"" fontId=""" . font . """ fillId=""" . fill . """ numFmtId=""" . format . """ applyFont=""1"" applyFill=""1"" applyNumberFormat=""1""/>"
        }
    }
