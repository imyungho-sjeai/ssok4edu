...
DOC_FixQuickSymbols(line)
{
    ...
    line := RegExReplace(line, "^(\s*)a(\s+|$)", "$1¡à$2")
    line := RegExReplace(line, "^(\s*)¤²(\s+|$)", "$1¡á$2")
    line := RegExReplace(line, "^(\s*)¤·(\s+|$)", "$1¡Û$2")
    line := RegExReplace(line, "^(\s*)d(\s+|$)", "$1¡Û$2")
    line := RegExReplace(line, "^(\s*)¤¡(\s+|$)", "$1¡¤$2")
    line := RegExReplace(line, "^(\s*)¤µ(\s+|$)", "$1¡â$2")
    line := RegExReplace(line, "^(\s*)¤¸(\s+|$)", "$1¡Ø$2")
    line := RegExReplace(line, "(¡Ø\s*¿ìÃµ)\s+½Ã", "$1½Ã")

    ; ÁÙ ¸Ç ¾ÕÀÇ ¸¶Ä§Ç¥´Â ¸ñ·Ï¿ë °¡¿îµ¥Á¡À¸·Î º¯È¯
    ; ´Ü, .5 °°Àº ¼ýÀÚ Ç¥ÇöÀº °Çµå¸®Áö ¾ÊÀ½
    line := RegExReplace(line, "^(\s*)\.(?=\s|[°¡-ÆR]|$)", "$1¡¤")

    ; ÇÑ±Û°ú ÇÑ±Û »çÀÌÀÇ ¸¶Ä§Ç¥¸¸ °¡¿îµ¥Á¡À¸·Î º¯È¯
    ; ¼ýÀÚ°¡ ÇÑÂÊ¿¡¶óµµ ºÙÀº Á¡°ú ¹®Àå ³¡ ¸¶Ä§Ç¥´Â ±×´ë·Î À¯Áö
    line := RegExReplace(line, "([°¡-ÆR])\.(?=\s*[°¡-ÆR])", "$1¡¤")
