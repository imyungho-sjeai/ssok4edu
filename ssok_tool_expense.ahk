...
; - Çö¾×¡¤Ç°ÀÇ¾×¡¤¿øÀÎÇàÀ§¾×¡¤ÁýÇà·ü¡¤ÁýÇàÀÜ¾× Ç¥½Ã
; - Á¤»êÀç¿ø / ÀÏ¹ÝÀç¿ø º°µµ °ü¸® ¹× °ü¸®¿ë ÁýÇà·ü Ç¥½Ã
...
SSOK_Expense_BudgetDashboard_Show()
{
    ...
    Gui, SSOKBudgetDashboard:Destroy
    Gui, SSOKBudgetDashboard:New, +AlwaysOnTop +ToolWindow +HwndSSOK_BudgetDashboardHwnd, ¾÷¹«¿ëµµ±¸ - ¿¹»ê DashBoard
    ...
    try
    {
        ...
        if !IsObject(selectedSheet)
        {
            errMsg := "»ç¾÷°ü¸®Ä«µå(Çö¾×) Ç¥ ¸Ó¸®±ÛÀ» Ã£Áö ¸øÇß½À´Ï´Ù.`n`n"
                . "±Ù°ÅÀÚ·á: ÇÐ±³È¸°è-»ç¾÷°ü¸®-»ç¾÷°ü¸®Ä«µå-»ç¾÷°ü¸®Ä«µåÇö¾×`n"
...
SSOK_Expense_BudgetDashboard_ReadSheet(ws, headerRow, lastRow, colBusiness, colSubItem, colCost, colCalc, colCurrent, colDraft, colCommit, colSettle, colGeneral, colManager, ByRef parsed, ByRef errMsg)
{
    ...
    Loop, %dataCount%
    {
        ...
        fg.commit += commit
        fg.rowCount++
        ; »ç¾÷°ü¸®Ä«µåÀÇ Á¤»êÀç¿ø/ÀÏ¹ÝÀç¿øÀº °°Àº ¼¼ºÎÇ×¸ñ¿¡¼­ ¹Ýº¹µÇ¹Ç·Î ÇÑ °ª¸¸ º¸°üÇÕ´Ï´Ù.
    ...
    ; ¿øº»¿¡´Â ½ÇÁ¦ ÁöÃâÀÌ ¾î´À Àç¿ø¿¡¼­ ºüÁ³´ÂÁö º°µµ Ç¥½Ã°¡ ¾ø½À´Ï´Ù.
    ; °ü¸®¿ëÀ¸·Î Á¤»êÀç¿øÀ» ¸ÕÀú ÁýÇàÇÑ °ÍÀ¸·Î °¡Á¤ÇÏ°í Àç¿øº° ÁýÇà¾×À» °è»êÇÕ´Ï´Ù.
    ...
    ; ¼¼ºÎÇ×¸ñ ´ÜÀ§ Àç¿ø ±Ý¾×À» »êÃâ³»¿ª Çö¾× ºñÀ²·Î ¹èºÐÇÏ¿© ¾î¶² º¸±â¿¡¼­µµ Áßº¹ ¾øÀÌ ÇÕ»êµÇ°Ô ÇÕ´Ï´Ù.
    for _, row in rows
    {
    ...
    if (colSettle && colGeneral)
    {
        if (Abs(resourceTotal - totalCurrent) < 0.5)
            resourceVerifyText := "Àç¿øÇÕ°è ÀÏÄ¡"
        else
            resourceVerifyText := "Àç¿øÇÕ°è È®ÀÎ ÇÊ¿ä"
    }
    else
    {
        resourceVerifyText := "Àç¿ø¿­ ¾øÀ½"
    }
...
SSOK_Expense_BudgetDashboard_ShowDashboard()
{
    ...
    SSOK_BudgetDashboardViewChoice := "¼¼ºÎ»ç¾÷º°"
    ...
    Gui, SSOKBudgetDashboard:Destroy
    Gui, SSOKBudgetDashboard:New, +AlwaysOnTop +ToolWindow +HwndSSOK_BudgetDashboardHwnd, ¿¹»ê DashBoard
    Gui, SSOKBudgetDashboard:Margin, 18, 14
    Gui, SSOKBudgetDashboard:Color, F7FBFF

    Gui, SSOKBudgetDashboard:Font, s13 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, Text, x18 y12 w500 h28 c005BAC, ¿¹»ê DashBoard
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, Text, x560 y14 w920 h20 Right c555555, % "»êÃâ³»¿ª " . p.rowCount . "Çà  |  ÃÊ°úÁýÇà " . p.overCount . "°Ç  |  " . p.verifyText . "  |  " . p.resourceVerifyText
    Gui, SSOKBudgetDashboard:Add, Text, x560 y34 w920 h18 Right c777777, %sourceName%

    SSOK_Expense_BudgetDashboard_AddKpi(18, 58, 280, "¿¹»êÇö¾×", SSOK_Expense_FormatNumber(p.current) . "¿ø", "005BAC")
    SSOK_Expense_BudgetDashboard_AddKpi(310, 58, 280, "ÁöÃâÇ°ÀÇ¾×", SSOK_Expense_FormatNumber(p.draft) . "¿ø", "3A6EA5")
    SSOK_Expense_BudgetDashboard_AddKpi(602, 58, 280, "¿øÀÎÇàÀ§¾×", SSOK_Expense_FormatNumber(p.commit) . "¿ø", "6A5ACD")
    SSOK_Expense_BudgetDashboard_AddKpi(894, 58, 280, "ÁýÇà·ü(B/A)", SSOK_Expense_BudgetDashboard_FormatRate(p.current, p.commit), "00856A")
    balanceColor := (p.balance < 0) ? "C62828" : "555555"
    SSOK_Expense_BudgetDashboard_AddKpi(1186, 58, 294, "ÁýÇàÀÜ¾×(A-B)", SSOK_Expense_FormatNumber(p.balance) . "¿ø", balanceColor)

    settleText := "Çö¾× " . SSOK_Expense_FormatNumber(p.settle) . "¿ø   |   °ü¸®ÁýÇà " . SSOK_Expense_FormatNumber(p.settleCommit) . "¿ø   |   " . SSOK_Expense_BudgetDashboard_FormatRate(p.settle, p.settleCommit)
    generalText := "Çö¾× " . SSOK_Expense_FormatNumber(p.general) . "¿ø   |   °ü¸®ÁýÇà " . SSOK_Expense_FormatNumber(p.generalCommit) . "¿ø   |   " . SSOK_Expense_BudgetDashboard_FormatRate(p.general, p.generalCommit)
    SSOK_Expense_BudgetDashboard_AddResourceKpi(18, 136, 718, "Á¤»êÀç¿ø", settleText, "7A4E00")
    SSOK_Expense_BudgetDashboard_AddResourceKpi(748, 136, 732, "ÀÏ¹ÝÀç¿ø", generalText, "3F6B4F")

    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, Text, x18 y199 w1462 h18 c777777, ¡Ø Àç¿øº° ÁýÇà¾×/ÁýÇà·üÀº ¿øº»¿¡ ½ÇÁ¦ »ç¿ëÀç¿ø ±¸ºÐÀÌ ¾ø¾î 'Á¤»êÀç¿ø ¿ì¼± ÁýÇà' ±âÁØÀ¸·Î °è»êÇÑ °ü¸®¿ë °ªÀÔ´Ï´Ù.
    ...
    Gui, SSOKBudgetDashboard:Add, Text, x18 y247 w36 h24 +0x200, º¸±â
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, DropDownList, x54 y245 w180 vSSOK_BudgetDashboardViewChoice gSSOK_Expense_BudgetDashboard_Refresh, ¼¼ºÎ»ç¾÷º°||¼¼ºÎÇ×¸ñº°|¿ø°¡Åë°èºñ¸ñº°|»êÃâ³»¿ªº°|¼¼ºÎÇ×¸ñ´ã´çÀÚº°

    Gui, SSOKBudgetDashboard:Font, s8 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, Text, x246 y247 w36 h24 +0x200, °Ë»ö
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, Edit, x282 y245 w250 h24 vSSOK_BudgetDashboardSearch gSSOK_Expense_BudgetDashboard_Refresh

    Gui, SSOKBudgetDashboard:Font, s8 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, Text, x544 y247 w36 h24 +0x200, Á¤·Ä
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, DropDownList, x580 y245 w190 vSSOK_BudgetDashboardSortChoice gSSOK_Expense_BudgetDashboard_Refresh, Çö¾× Å«¼ø||Ç°ÀÇ¾× Å«¼ø|¿øÀÎÇàÀ§ Å«¼ø|ÁýÇà·ü ³ôÀº¼ø|ÁýÇà·ü ³·Àº¼ø|Á¤»êÀç¿ø Å«¼ø|Á¤»ê ÁýÇà·ü ³ôÀº¼ø|ÀÏ¹ÝÀç¿ø Å«¼ø|ÀÏ¹Ý ÁýÇà·ü ³ôÀº¼ø|ÀÌ¸§¼ø

    Gui, SSOKBudgetDashboard:Font, s8 Bold, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, Text, x782 y247 w36 h24 +0x200, Àç¿ø
    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, DropDownList, x818 y245 w105 vSSOK_BudgetDashboardResourceChoice gSSOK_Expense_BudgetDashboard_Refresh, ÀüÃ¼||Á¤»êÀç¿ø|ÀÏ¹ÝÀç¿ø

    Gui, SSOKBudgetDashboard:Add, CheckBox, x936 y247 w100 h24 vSSOK_BudgetDashboardHideZero gSSOK_Expense_BudgetDashboard_Refresh, 0¿ø Çà Á¦¿Ü
    Gui, SSOKBudgetDashboard:Add, CheckBox, x1040 y247 w105 h24 vSSOK_BudgetDashboardOverOnly gSSOK_Expense_BudgetDashboard_Refresh, ÃÊ°úÁýÇà¸¸
    Gui, SSOKBudgetDashboard:Add, Button, x1160 y242 w86 h30 gSSOK_Expense_BudgetDashboard_CopyView, Ç¥ º¹»ç
    Gui, SSOKBudgetDashboard:Add, Button, x1252 y242 w96 h30 gSSOK_Expense_BudgetDashboard_BackToInput, ´Ù¸¥ ÆÄÀÏ
    Gui, SSOKBudgetDashboard:Add, Button, x1354 y242 w126 h30 gSSOKBudgetDashboardGuiClose, ´Ý±â

    Gui, SSOKBudgetDashboard:Font, s8 Norm, Malgun Gothic
    Gui, SSOKBudgetDashboard:Add, ListView, x18 y282 w1462 h452 Grid -Multi, ±¸ºÐ|»ó¼¼|Çö¾×|Ç°ÀÇ¾×|¿øÀÎÇàÀ§¾×|ÁýÇà·ü|ÁýÇàÀÜ¾×|Á¤»êÀç¿ø|Á¤»ê ÁýÇà·ü|ÀÏ¹ÝÀç¿ø|ÀÏ¹Ý ÁýÇà·ü
    Gui, SSOKBudgetDashboard:Add, Text, x18 y741 w1462 h24 +0x200 vSSOK_BudgetDashboardListStatus c555555,
    Gui, SSOKBudgetDashboard:Add, Text, x18 y770 w1462 h20 c777777, ´Ù¸¥ »ç¾÷°ü¸®Ä«µå ÆÄÀÏµµ ÀÌ Ã¢¿¡ ¹Ù·Î ²ø¾î ³õÀ¸¸é Áï½Ã ´Ù½Ã ºÐ¼®ÇÕ´Ï´Ù.

    Gui, SSOKBudgetDashboard:Show, w1498 h805 Center, ¿¹»ê DashBoard
    SSOK_Expense_BudgetDashboard_RefreshList()
}
...
SSOK_Expense_BudgetDashboard_RefreshList()
{
    ...
    for _, entry in entries
    {
        ...
        if (resourceMode = "Á¤»êÀç¿ø" && entry.settle <= 0)
            continue
        if (resourceMode = "ÀÏ¹ÝÀç¿ø" && entry.general <= 0)
            continue
    ...
    status := "Ç¥½Ã " . shown . "°Ç"
    status .= "  |  Çö¾× " . SSOK_Expense_FormatNumber(shownCurrent) . "¿ø"
    status .= "  |  ¿øÀÎÇàÀ§ " . SSOK_Expense_FormatNumber(shownCommit) . "¿ø"
    status .= "  |  ÁýÇà·ü " . SSOK_Expense_BudgetDashboard_FormatRate(shownCurrent, shownCommit)
    status .= "  |  Á¤»ê " . SSOK_Expense_FormatNumber(shownSettle) . "¿ø / " . SSOK_Expense_BudgetDashboard_FormatRate(shownSettle, shownSettleCommit)
    status .= "  |  ÀÏ¹Ý " . SSOK_Expense_FormatNumber(shownGeneral) . "¿ø / " . SSOK_Expense_BudgetDashboard_FormatRate(shownGeneral, shownGeneralCommit)
...
SSOK_Expense_BudgetDashboard_ShouldSwap(a, b, sortMode)
{
    ...
    else if (sortMode = "Á¤»êÀç¿ø Å«¼ø")
    {
        va := a.settle
        vb := b.settle
        descending := true
    }
    else if (sortMode = "Á¤»ê ÁýÇà·ü ³ôÀº¼ø")
    {
...
SSOK_Expense_BudgetDashboard_CopyCurrentView()
{
    ...
    out := "±¸ºÐ`t»ó¼¼`tÇö¾×`tÇ°ÀÇ¾×`t¿øÀÎÇàÀ§¾×`tÁýÇà·ü`tÁýÇàÀÜ¾×`tÁ¤»êÀç¿ø`tÁ¤»ê ÁýÇà·ü`tÀÏ¹ÝÀç¿ø`tÀÏ¹Ý ÁýÇà·ü`r`n"
    Loop, %rowCount%
    {
        rowNo := A_Index
        line := ""
